...
bool readEurocCalibration(string caliFilePath) {
  ...
  if (cameraNode.isSeq() && cameraNode.size() > 0) {
    ...
    for (cv::FileNodeIterator it = cameraNode.begin(); it != cameraNode.end();
      ...
      if ((*it).isMap() && (*it)["T_SC"].isSeq() &&
        ...
        ROS_INFO("Found calibration in configuration file for camera %d",
                 camIdx);
        gotCalibration = true;
      } else {
        return false;
  ...
  if (gotCalibration) {
    ...
    for (cv::FileNodeIterator it = cameraNode.begin(); it != cameraNode.end();
      ...
      printf("%lf %lf\n", principal_point[camIdx][0],
             principal_point[camIdx][1]);
    }
  }
  ...
  cv::stereoRectify(K0, distortion_coefficients[0], K1,
                    distortion_coefficients[1], imageSize, R01, t01, R0, R1, P0,
                    P1, Q, cv::CALIB_ZERO_DISPARITY, 0, imageSize, &validRoi[0],
                    &validRoi[1]);

  cv::initUndistortRectifyMap(K0, distortion_coefficients[0], R0, P0, imageSize,
                              CV_16SC2, map00_, map01_);
  cv::initUndistortRectifyMap(K1, distortion_coefficients[1], R1, P1, imageSize,
                              CV_16SC2, map10_, map11_);
...
*/
  ...
  printf("fx=%f fy=%f cx=%f cy=%f\n", calib_par.fx[0], calib_par.fy[0],
         calib_par.cx[0], calib_par.cy[0]);
...
//                   &calib_par.d[0], &calib_par.d[1], &calib_par.d[2],
//                   &calib_par.d[3]) != 8 )
//    {
//        puts("calibration file format error 1") ;
//        return false ;
//    }
//    if(std::sscanf(l2.c_str(), "%d %d", &calib_par.width, &calib_par.height )
//    != 2)
//    {
//        puts("calibration file format error 2") ;
//        return false ;
//    }
//    printf("fx=%f fy=%f cx=%f cy=%f\n", calib_par.fx, calib_par.fy,
//    calib_par.cx, calib_par.cy ) ; printf("height=%d width=%d\n",
...
void fun( ros::NodeHandle& nh)
{
    ...
    ros::Publisher pub_img0 =
nh.advertise<sensor_msgs::Image>("/cam0/image_raw", 10); ros::Publisher pub_img1
= nh.advertise<sensor_msgs::Image>("/cam1/image_raw", 10); sensor_msgs::Image
msg ; for( int i = 0 ; i < 60 ; i++ )
    {
        msg.header.stamp = ros::Time::now() ;

        sprintf(path,
"/home/ygling2008/visensor_calibraion/calibrationdata/left-%04d.png", i ) ;
...
image_height, image_width, image_width, img0.data ); pub_img0.publish(msg) ;
        imshow("img0", img0) ;

        cv::waitKey(50) ;

        sprintf(path,
"/home/ygling2008/visensor_calibraion/calibrationdata/right-%04d.png", i ) ;
        ...
        msg.width = image_width ;
        sensor_msgs::fillImage(msg, sensor_msgs::image_encodings::MONO8,
image_height, image_width, image_width, img1.data ); printf("%s\n", path ) ;
...
int main(int argc, char **argv) {
  ...
  dynamic_reconfigure::Server<edge_imu_bias::ParamsConfig> srv(
      ros::NodeHandle("~"));
  ...
  nh.param("intrinsics_calibration_file", intrinsics_calibration_file,
           packagePath + "/calib/combine2_icra.yml");
  nh.param("extrinsics_calibration_file", extrinsics_calibration_file,
           packagePath + "/calib/visensor.yml");
  nh.param("euroc_calibration_file", euroc_calibration_file,
           packagePath + "/calib/config_euroc.yaml");
