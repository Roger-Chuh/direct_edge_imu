...
SE3 SE3Tracker::trackFrameOnPermaref(Frame *reference, Frame *frame,
  ...
  callOptimized(calcResidualAndBuffers, (reference->permaRef_posData,
                                         ...
                                         reference->permaRefNumPts, frame,
                                         referenceToFrame, QUICK_KF_CHECK_LVL));
  ...
  for (int iteration = 0; iteration < settings.maxItsTestTrack; iteration++) {
    ...
    while (true) {
      ...
      calcResidualAndBuffers(reference->permaRef_posData,
                             reference->permaRef_colorAndVarData, 0,
                             reference->permaRefNumPts, frame,
                             new_referenceToFrame, QUICK_KF_CHECK_LVL);
      ...
      if (buf_warped_size < MIN_GOODPERALL_PIXEL_ABSMIN *
        ...
        //                std::endl ; std::cout <<
        //                referenceToFrameOrg.translation() << std::endl ;
        diverged = true;
        trackingWasGood = false;
        return SE3();
      }
