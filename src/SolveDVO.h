...
//#define __PRINT_POSE_EACH_ITERATION 0 //display the data only for this level
// in runIterations #define __PRINT_GRAD_DIRECTION_EACH_ITERATION
...
// note: If you do not scale you cannot really view the overlay on the DT. Not
// scaling gives better pose estimates
...
//      Note: This is not ensuring that dvo does not listen to streams (need to
//      take care of this by yourself) Note: Cannot be used together with
//      __TF_GT__ because the xml files do not contain GT data
...
//#define __DATA_SKIP_FACTOR 4 //use 1 for processing every frame. 2 will give
// every alternate frame, 3 will gv every 3rd frame and so on
...
/// Defines the class to handle streaming images and compute camera pose from
/// RGBD images ie. dense visual odometry (DVO)
class SolveDVO {
...
public:
  ...
  std::vector<Eigen::MatrixXf> rcvd_framemono,
      rcvd_depth; ///< Received image pyramid
  bool isFrameAvailable;

  //
  // Previously set now frames (added 14th Aug, 2015)
  std::vector<Eigen::MatrixXf> p_now_framemono,
      p_now_depth; ///< previously set now image pyramid
  bool isPrevFrameAvailable;
  //    void setPrevFrameAsRefFrame(); // sets (p_now_framemono, p_now_depth) as
  //    ref frame
  ...
  // void computeJacobian(int level, JacobianList &J, ImCordList &imC,
  // SpaceCordList &spC, IntensityList &I, Eigen::MatrixXi &refROI );
  float runIterations(STATE *keyFrame, STATE *currentFrame,
                      CALIBRATION_PAR *cali, int level, int maxIterations,
                      Eigen::Matrix3d &cR, Eigen::Vector3d &cT,
  ...
  void updateEstimates(Eigen::Matrix3f &cR, Eigen::Vector3f &cT,
                       Eigen::Matrix3f &xRot, Eigen::Vector3f &xTrans);

  float optimizeGaussianNewton(STATE *keyFrame, STATE *currentFrame,
  ...
  // helpers
  void imageGradient(Eigen::MatrixXf &image, Eigen::MatrixXf &gradientX,
  ...
  void exponentialMap(
      Eigen::VectorXf &psi, Eigen::Matrix3f &outR,
      Eigen::Vector3f &outT); // this is not in use. It is done with Sophus
  void sOverlay(Eigen::MatrixXf eim, Eigen::MatrixXi mask, cv::Mat &outIm,
                cv::Vec3b color);
  ...
  void visualizeEnergyProgress(Eigen::VectorXf energyAtEachIteration,
                               int bestEnergyIndex, int XSPACING);
  // void visualizeResidueHeatMap( Eigen::MatrixXf& eim, Eigen::MatrixXf&
  // residueAt );
  void visualizeDistanceResidueHeatMap(cv::Mat &tmpIm,
                                       Eigen::MatrixXi &reprojectionMask,
                                       Eigen::MatrixXf &nowDistMap);

  //    void visualizeReprojectedDepth( Eigen::MatrixXf& eim, Eigen::MatrixXf&
  //    reprojDepth );

  // debugging variable
  Eigen::MatrixXi
      __now_roi_reproj;       // this is a `binary` mask of reprojected points
  Eigen::VectorXf __residues; // list of residue values, -1 ==> this pixel is
                              // not visible in reprojected frame
  Eigen::MatrixXf __now_roi_reproj_values; // this is a mask with float values
                                           // (= residues at that point)
  Eigen::MatrixXf __reprojected_depth; // mask of float values denoting depth
                                       // values (in mm) in now frames
  ...
  //    void visualizeDistanceResidueHeatMap(Eigen::MatrixXf& eim,
  //    Eigen::MatrixXi& reprojectionMask, Eigen::MatrixXf& nowDistMap ); void
  //    visualizeEnergyProgress( Eigen::VectorXf energyAtEachIteration, int
  //    bestEnergyIndex, int XSPACING=1 );
  ...
  //
  // Forward formulation (distance energy function) related function (8th July,
  // 2015) selectedPts //< already declared above
  std::vector<SpaceCordList> _ref_edge_3d; ///< Stores the edge points of the
                                           ///< reference frame in 3d ie. `E_i`
  std::vector<ImCordList>
      _ref_edge_2d; ///< Stores edge image co-ordinates (2d) ie. `e_i`
  std::vector<Eigen::MatrixXi>
      _ref_roi_mask; ///< Reference selected edge points

  // void gaussNewton <-- defined above
  void enlistRefEdgePts(int level, Eigen::MatrixXi &refEdgePtsMask,
                        SpaceCordList &_3d, ImCordList &_2d);
  // void preProcessRefFrame();
  void computeJacobianOfNowFrame(STATE *keyFrame, STATE *currentFrame,
                                 CALIBRATION_PAR *cali, int level,
                                 Eigen::Matrix3f &cR, Eigen::Vector3f &cT,
                                 JacobianLongMatrix &Jcbian);
  float getReprojectedEpsilons(STATE *keyFrame, STATE *currentFrame,
                               CALIBRATION_PAR *cali, int level);

  void cordList_2_mask(Eigen::MatrixXf &list,
                       Eigen::MatrixXi &mask); // make sure mask is preallocated
  ...
  void tfTransform2EigenMat(tf::StampedTransform &tr, Eigen::Matrix3f &R,
                            Eigen::Vector3f &T);
  ...
  float psiNormTerminationThreshold; // terminate iteration if norm of psi falls
                                     // below this value
  float trustRegionHyperSphereRadius; // trust region to scale the direction of
                                      // descent
