...
public:
  ...
  cv::Mat labels[PYRAMID_LEVELS];
  std::vector<Eigen::Vector2i> label2xy[PYRAMID_LEVELS];
  ...
  Eigen::MatrixXf edge_2d_cur[PYRAMID_LEVELS];
        ...
        for (int x = 0; x < width; x++) {
          ...
          image[i].at<uchar>(y, x) =
              ((int)image[i - 1].at<uchar>(y << 1, x << 1) +
               ...
               (int)image[i - 1].at<uchar>((y << 1) + 1, (x << 1) + 1)) /
                  4 +
              0.5;
        }
      }
      ...
      for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
          ...
          if (var > 0) {
            ...
            ivarSumsSum += ivar;
            idepthSumsSum +=
                ivar * idepth[i - 1].at<float>(y << 1, (x << 1) + 1);
            num++;
          }
          ...
          if (var > 0) {
            ...
            ivarSumsSum += ivar;
            idepthSumsSum +=
                ivar * idepth[i - 1].at<float>((y << 1) + 1, x << 1);
            num++;
          }
          ...
          if (var > 0) {
            ...
            ivarSumsSum += ivar;
            idepthSumsSum +=
                ivar * idepth[i - 1].at<float>((y << 1) + 1, (x << 1) + 1);
            num++;
          }
  ...
  (float*)Eigen::internal::aligned_malloc(w*h*sizeof(float));
          ...
          for( int y = 0 ; y < h ; y++ )
          {
              for ( int x = 0 ; x < w ; x++ )
              {
                  if ( edge[i].at<uchar>(y, x) > 0 && iVar[i].at<float>(y, x) >
  0 )
                  {
                      Z = 1.0 / idepth[i].at<float>(y, x) ;
                      X = ( x - cali->cx[i] ) / cali->fx[i] * Z ;
  ...
  */
  void initEdgePyramid() {
    ...
    for (int i = 0; i < optimizedPyramidLevel; i++) {
      ...
      if (adaptiveCannyThreshold) {
        ...
        cv::Canny(image[i], edge[i], std::max(0, (int)threshold - 10),
                  std::min(255, (int)threshold + 10), 3, true);
      } else {
        cv::Canny(image[i], edge[i], cannyThreshold1, cannyThreshold2, 3, true);
      }
      //            cv::Canny(image[i], edge[i], max(0, cannyThreshold1,
      //            cannyThreshold2, 3, true ) ; printf("cannyThreshold1 = %d",
      //            cannyThreshold1 ) ; printf("cannyThreshold2 = %d",
      //            cannyThreshold2 ) ;
      // edge[i] = 255 - edge[i] ;
  ...
  void initDistanceTransformPyramid() {
    ...
    cv::Mat gradientY;
    for (int i = 0; i < optimizedPyramidLevel; i++) {
      // 输入图像中 0 = 边缘，非零 = 背景。因为 cv::distanceTransform
      // 计算的是每个非零像素到最近的零像素的距离
      labels[i] = cv::Mat::zeros(edge[i].size(), CV_32S);
      cv::Mat inverted = 255 - edge[i];
      cv::distanceTransform(inverted, distanceTransformMap[i], labels[i],
                            cv::DIST_L2, cv::DIST_MASK_PRECISE,
                            cv::DIST_LABEL_PIXEL);
      // 建立 label → (x,y) 映射
      int maxLabel = cv::countNonZero(edge[i]);
      label2xy[i].resize(maxLabel + 1);
      edge_2d_cur[i] = Eigen::MatrixXf::Zero(2, maxLabel + 1);
      for (int r = 0; r < edge[i].rows; r++) {
        for (int c = 0; c < edge[i].cols; c++) {
          if (edge[i].at<uchar>(r, c) > 0) {
            // 是边缘像素
            label2xy[i][labels[i].at<int>(r, c)] = Eigen::Vector2i(c, r);
            edge_2d_cur[i].col(labels[i].at<int>(r, c)) = Eigen::Vector2f(c, r);
          }
        }
      }

      // 使用时：
      int label = labels[i].at<int>(20, 10);
      Eigen::Vector2i nearestPt = label2xy[i][label]; // 最近边缘点的像素坐标
