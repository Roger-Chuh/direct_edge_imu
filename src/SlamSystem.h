...
public:
  ...
  const Matrix3d acc_cov = std::pow(acc_density * std::sqrt(update_rate), 2.0) *
                           Matrix3d::Identity(); // 0.014
  const Matrix3d gyr_cov = std::pow(gyr_density * std::sqrt(update_rate), 2.0) *
                           Matrix3d::Identity(); // 0.0012
  const Matrix2d pts_cov =
      (0.5 / FOCAL_LENGTH) * (0.5 / FOCAL_LENGTH) * Matrix2d::Identity();
  ...
  lsd_slam::TrackingReference
      *trackingReference; // tracking reference for current keyframe. only used
                          // by tracking.
  lsd_slam::SE3Tracker *tracker;
  lsd_slam::TrackingReference
      *trackingReferenceConstraint; // tracking reference for current keyframe.
                                    // only used by tracking.
