...
void MarginalizationFactor::addResidualBlockInfo(
...
void MarginalizationFactor::marginalize() {
  ...
  ROS_INFO("marginalization, pos: %d, m: %d, n: %d, size: %lu", pos, m, n,
           parameter_block_idx.size());
  ...
  ROS_INFO("summing up costs %f ms, error: %f, %f", t_summing.toc(),
           (A).maxCoeff(), (A).minCoeff());
...
std::vector<double *> MarginalizationFactor::getParameterBlocks(
...
bool MarginalizationFactor::Evaluate(double const *const *parameters,
  ...
  for (int i = 0; i < static_cast<int>(keep_block_size.size()); i++) {
    ...
    Eigen::VectorXd x0 =
        Eigen::Map<const Eigen::VectorXd>(keep_block_data[i], size);
    if (size != 7)
