...
void LiveSLAMWrapper::popAndSetGravity(int skipFrameNum) {
  ...
  while (nh.ok()) {
    ...
    while (imuQueue.size() < 1 || imuQueue.rbegin()->header.stamp < tImage) {
      // cout << "currentIMU_iter->header.stamp " <<
      // imuQueue.rbegin()->header.stamp  << "\n" ;
      imu_queue_mtx.unlock();
      r.sleep();
      ros::spinOnce();
      imu_queue_mtx.lock();
  ...
  Sophus::Matrix3f K_sophus;
  K_sophus << fx, 0.0, cx, 0.0, fy, cy, 0.0, 0.0, 1.0;
  Odometry->pKF->frame.reset(new lsd_slam::Frame(0, width, height, K_sophus,
                                                 tImage.toSec(), image0.data));
  // puts("1") ;
  ...
  if (printDebugInfo) { /*
                           ...
                           sensor_msgs::image_encodings::MONO8, height, width,
                           width, Odometry->states[0]->edge[0].data );
  ...
  //            pub_odometry, pub_pose, pub_nav_Odometry, 0,
  //            Eigen::Matrix3d::Identity(), true, tImage );
...
void LiveSLAMWrapper::pubCameraLink() {
  ...
  //            cv::circle(linkListMap, locations[i], 6, cv::Scalar(255, 0, 0),
  //            5);
  //        }
  //        else{
  //            cv::circle(linkListMap, locations[i], 6, cv::Scalar(0, 0, 255),
  //            5);
  //        }
  //        if ( i == Odometry->head ){
  //            cv::circle(linkListMap, locations[i], 6, cv::Scalar(0, 255, 0),
  //            5);
  //        }
  ...
  //        Odometry->slidingWindow[idx].cameraLinkList.begin(); for (; iter !=
  //        Odometry->slidingWindow[idx].cameraLinkList.end(); iter++ )
  //        {
  //            int linkID = *iter;
  //            cv::line(linkListMap, locations[idx], locations[linkID],
  //            cv::Scalar(0, 255, 255), 3); cnt++ ;
  //        }
  //    }
...
//    Odometry->currentKeyFrame->idepthVar(level); Eigen::Vector3f posDataPT ;
//    Eigen::Vector3f posDataOutput ;
//    Eigen::Matrix3f R_output ;
//    R_output << R_vi_2_odometry(0, 0), R_vi_2_odometry(0, 1),
//    R_vi_2_odometry(0, 2),
//            R_vi_2_odometry(1, 0), R_vi_2_odometry(1, 1), R_vi_2_odometry(1,
//            2), R_vi_2_odometry(2, 0), R_vi_2_odometry(2, 1),
//            R_vi_2_odometry(2, 2) ;
...
void LiveSLAMWrapper::BALoop() {
  ...
  while (nh.ok()) {
    ...
    //        if (
    //        Odometry->frameInfoList[Odometry->frameInfoListHead].keyFrameFlag
    //        ){
    //            puts("k") ;
    //        }
    //        else{
    ...
    ros::Time imageTimeStamp =
        Odometry->frameInfoList[Odometry->frameInfoListHead].t;
    Odometry->frameInfoList_mtx.unlock();
    ...
    if (printDebugInfo) {
      ...
      cv::Mat showDT;
      cv::normalize(nowFrame->distanceTransformMap[0], showDT, 0, 255,
                    CV_MINMAX, CV_8U);
    ...
    { // consistent check
      ...
      Vector3d tmp =
          Utility::R2ypr(Odometry->Rs[Odometry->frame_count].transpose() *
                         Odometry->Rs[Odometry->lastKF->state_id] *
                         Odometry->frameInfoList[Odometry->frameInfoListHead]
                             .R_k_2_c.transpose());
      double errorAngle = tmp.norm();
      Vector3d errorT =
          Odometry->Rs[Odometry->frame_count].transpose() *
              (Odometry->Ps[Odometry->lastKF->state_id] -
               Odometry->Ps[Odometry->frame_count]) -
          Odometry->frameInfoList[Odometry->frameInfoListHead].T_k_2_c;
      double errorTranslation = errorT.norm();

      //            std::cout << "Rotation-IMU Prior: " <<
      //            Odometry->Rs[Odometry->frame_count].transpose()*Odometry->Rs[Odometry->lastKF->state_id]
      //            << endl; std::cout << "Visual: " <<
      //            Odometry->frameInfoList[Odometry->frameInfoListHead].R_k_2_c.transpose()
      //            << endl; std::cout << "diff: " << errorQ.vec().transpose()
      //            << endl ;

      //            std::cout << "Translation-IMU Prior: " <<
      //            (Odometry->Rs[Odometry->frame_count].transpose()
      //                    *(Odometry->Ps[Odometry->lastKF->state_id] -
      //                    Odometry->Ps[Odometry->frame_count])).transpose() <<
      //                    endl;
      //            std::cout << "Visual: " <<
      //            Odometry->frameInfoList[Odometry->frameInfoListHead].T_k_2_c.transpose()
      //            << endl; std::cout << "diff: " << errorT.transpose() << endl
      //            ;
      ...
      if (IMUorNot && errorAngle > errorAngleThreshold) {
        ROS_WARN("errorAngle=%f errorAngleThreshold=%f", errorAngle,
                 errorAngleThreshold);
        cout << "IMU Prior: "
             << Utility::R2ypr(Odometry->Rs[Odometry->frame_count].transpose() *
                               Odometry->Rs[Odometry->lastKF->state_id])
      ...
      if (IMUorNot && errorTranslation > errorTranslationThreshold) {
        ROS_WARN("errorTranslation=%f errorTranslationThreshold=%f",
                 errorTranslation, errorTranslationThreshold);
        cout << "IMU Prior: "
             << (Odometry->Rs[Odometry->frame_count].transpose() *
                 (Odometry->Ps[Odometry->lastKF->state_id] -
                  Odometry->Ps[Odometry->frame_count]))
    ...
    if (Odometry->frameInfoList[Odometry->frameInfoListHead].keyFrameFlag) {
      ...
      if (printDebugInfo) {
        if (denseOrNot) {
          ...
          sensor_msgs::fillImage(msg, sensor_msgs::image_encodings::MONO8,
                                 height, width, width, nowFrame->edge[0].data);
          pub_gradientMapForDebug.publish(msg);
        }
      }
      ...
      if (enable_LoopClosure &&
          (imageTimeStamp - lastLoopClorsureTime).toSec() >
              loopClosureInterval) {
        ...
        ROS_WARN("loop closure link cost time: %f",
    ...
    if (Odometry->frameInfoList[Odometry->frameInfoListHead].trust) {
      ...
      //                          );
      Odometry->insertCameraLink(
          Odometry->lastKF, nowFrame,
          Odometry->frameInfoList[Odometry->frameInfoListHead].R_k_2_c,
          Odometry->frameInfoList[Odometry->frameInfoListHead].T_k_2_c,
          Odometry->frameInfoList[Odometry->frameInfoListHead].lastestATA);

      // Vector3d tmp =
      // Utility::R2ypr(Odometry->frameInfoList[Odometry->frameInfoListHead].R_k_2_c)
      // ; ROS_WARN("[insert Frame R] %lf %lf %lf", tmp(0), tmp(1), tmp(2) ) ;

      //            cout << "T " <<
      //            -Odometry->frameInfoList[Odometry->frameInfoListHead].T_k_2_c.transpose()
      //            << endl ; Quaterniond
      //            dq(Odometry->frameInfoList[Odometry->frameInfoListHead].R_k_2_c)
      //            ; cout << "R " << dq.x() << " " << dq.y() << " " << dq.z()
      //            << endl ;
    }
    ...
    //        nowFrame->T_bk_2_b0.transpose() << endl; cout << "[-BA]current
    //        Velocity: " << nowFrame->v_bk.transpose() << endl;
    ...
    cout << "[BA-]current Position: "
         << Odometry->Ps[Odometry->frame_count].transpose() << endl;
    cout << "[BA-]current Velocity: "
         << Odometry->Vs[Odometry->frame_count].transpose() << endl;
    cout << "[BA-]current bias_acc: "
         << Odometry->Bas[Odometry->frame_count].transpose() << endl;
    cout << "[BA-]current bias_gyr: "
         << Odometry->Bgs[Odometry->frame_count].transpose() << endl;
    ...
    //      if ( Odometry->frame_count >= WINDOW_SIZE-1 )
    {
      if (onUAV) {
        pubOdometry(
            ...
            Odometry->Rs[Odometry->frame_count], pub_odometry, pub_pose,
            pub_nav_Odometry, control_flag, R_vi_2_odometry,
            Odometry->frameInfoList[Odometry->frameInfoListHead].keyFrameFlag,
            imageTimeStamp);
      } else {
        pubOdometry(
            ...
            Odometry->Vs[Odometry->frame_count],
            Odometry->Rs[Odometry->frame_count], pub_odometry, pub_pose,
            pub_nav_Odometry, control_flag, Eigen::Matrix3d::Identity(),
            Odometry->frameInfoList[Odometry->frameInfoListHead].keyFrameFlag,
            imageTimeStamp);
      }
    }
    ...
    if (printDebugInfo) {
      ...
      //            Odometry->frameInfoList[Odometry->frameInfoListHead].keyFrameFlag
      //            ; if (
      //            Odometry->frameInfoList[Odometry->frameInfoListHead].trust
      //            == false ){
      //                colorFlag = 2 ;
      //            }
      ...
      if (onUAV) {
        pubPath(Odometry->Ps[Odometry->frame_count], colorFlag, path_line,
                pub_path, R_vi_2_odometry);
      } else {
        pubPath(Odometry->Ps[Odometry->frame_count], colorFlag, path_line,
                pub_path, Eigen::Matrix3d::Identity());
      }
    }
    ...
    if (enable_pubTF) {
      ...
      // Vector3d t_translation = R_vi_2_odometry *
      // Odometry->Ps[Odometry->frame_count] ;
      ...
      tf::Quaternion q;
      // Quaterniond tt_q(R_vi_2_odometry * Odometry->Rs[Odometry->frame_count]
    ...
    //        if ( noUse ){
    //            Odometry->frameInfoList_mtx.lock();
    //            int ttt = (Odometry->frameInfoListTail -
    //            Odometry->frameInfoListHead); if ( ttt < 0 ){
    //                ttt += frameInfoListSize ;
    //            }
    //            if ( ttt >= 1 )
    //            {
    ...
    //        }
    //        Vector3d tt_dist =
    //        (Odometry->slidingWindow[Odometry->tail]->T_bk_2_b0 -
    //                Odometry->slidingWindow[preIndex]->T_bk_2_b0) ;
    //        Matrix3d tt_rotate =
    //        Odometry->slidingWindow[Odometry->tail]->R_bk_2_b0.transpose() *
...
void LiveSLAMWrapper::Loop() {
  ...
  while (nh.ok()) {
    ...
    if (denseOrNot) {
      ...
      Odometry->trackFrameEdge(image0, image1, imageSeqNumber, imageTimeStamp,
                               deltaR);
    }

    gettimeofday(&tv_end, NULL);
    float msTrackFrame = (tv_end.tv_sec - tv_start.tv_sec) * 1000.0f +
...
void LiveSLAMWrapper::initRosPub() {
  ...
  pub_odometry =
      nh.advertise<quadrotor_msgs::Odometry>("/denseVO/odometry", 1000);
  ...
  ;
  pub_distanceTransform =
      nh.advertise<sensor_msgs::Image>("denseVO/distanceTransform", 100);
  ;
  ...
  pub_nav_Odometry =
      nh.advertise<nav_msgs::Odometry>("/denseVO/navOdometry", 100);
