 ...
 * [1] Yonggen Ling, Manohar Kuse, and Shaojie Shen, "Direct Edge
 * Alignment-Based Visual-Inertial Fusion for Tracking of Aggressive Motions",
 * Autonomous Robots, 2017. [2] Yonggen Ling and Shaojie Shen, "Aggresive
 * Quadrotor Flight Using Dense Visual-Inertial Fusion", in Proc. of the IEEE
 * Intl. Conf. on Robot. and Autom., 2016. [3] Yonggen Ling and Shaojie Shen,
 * "Dense Visual-Inertial Odometry for Tracking of Aggressive Motions", in Proc.
 * of the IEEE International Conference on Robotics and Biomimetics 2015.
 *
 *
...
SolveDVO::SolveDVO() {
  ...
  //    //sub = nh.subscribe( "odometry/rgbd", 2,
  //    &SolveDVO::imageArrivedCallBack, this ); sub = nh.subscribe(
  //    "Xtion/rgbdPyramid", 10, &SolveDVO::imageArrivedCallBack, this );
...
void SolveDVO::setCameraMatrix(const char *calibFile) {
  ...
  ROS_INFO(
      "[SolveDVO::setCameraMatrix] Camera Matrix & Distortion Coif Loaded");
...
bool SolveDVO::loadFromFile(const char *xmlFileName) {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    ROS_DEBUG_STREAM("loaded `" << matmonoName << "` : " << eframemono.rows()
                                << ", " << eframemono.cols());
    ROS_DEBUG_STREAM("loaded `" << matdepthName << "` : " << edframe.rows()
                                << ", " << edframe.cols());
...
/// @brief Given a mask of reference edge points, lists the edge points (as 3xN
/// & 2xN)
...
void SolveDVO::enlistRefEdgePts(int level, Eigen::MatrixXi &refEdgePtsMask,
                                SpaceCordList &_3d, ImCordList &_2d) {
    ...
    for (int yy = 0; yy < refEdgePtsMask.rows(); yy++) {
      if (refEdgePtsMask(yy, xx) > 0) {
        ...
        float Z = _refDepth(yy, xx) /
                  1000.0f; // converting mm to m for numerical conditioning
...
//        //
//        /// Enlist these selected pts (edge pts in ref) as 3d points using the
//        depth-image obtained from Xtion SpaceCordList _3d =
//        Eigen::MatrixXf::Zero(3, nSelectedPts); ImCordList _2d =
//        Eigen::MatrixXf::Zero(2, nSelectedPts); enlistRefEdgePts( level,
//        _roi_ref, _3d, _2d );
...
void SolveDVO::computeJacobianOfNowFrame(STATE *keyFrame, STATE *currentFrame,
                                         CALIBRATION_PAR *cali, int level,
                                         Eigen::Matrix3f &cR,
                                         Eigen::Vector3f &cT,
                                         JacobianLongMatrix &Jcbian) {
  /// J_{at a pixel} = [ 2x1 grad of now distance ] x [ 2x3 projection jacobian
  /// ] x [ del_p / del_psi ]
  ...
  // keyFrame->id(), level ) ;
  igl::repmat(cT, 1, _3d.cols(),
              cTRep); // repeat cT col-wise (uses IGL library)
  Eigen::MatrixXf _3d_transformed = cR.transpose() * (_3d - cTRep);

  /// "Step 3" : Corresponding reprojected pts. Project the transformed 3d
  /// points onto the image.
  int numOfPoints = _3d_transformed.cols();
  reprojections = Eigen::MatrixXf::Zero(2, numOfPoints);
  for (int i = 0; i < numOfPoints; i++) {
    reprojections(0, i) =
        cali->fx[level] * _3d_transformed(0, i) / _3d_transformed(2, i) +
        cali->cx[level];
    reprojections(1, i) =
        cali->fy[level] * _3d_transformed(1, i) / _3d_transformed(2, i) +
        cali->cy[level];
  }
  ...
  /// "Step 4" : Loop over each 3d/2d edge pt of the (cR,cT) frame of reference
  /// (ie. ref Transformed) to compute jacobian at those pts
  int notJ = 0;
  const int nCols = _nowDist.cols() - 3;
  const int nRows = _nowDist.rows() - 3;

  for (int i = 0; i < numOfPoints; i++) {
    if (reprojections(0, i) < 2 || reprojections(0, i) > nCols ||
        reprojections(1, i) < 2 || reprojections(1, i) > nRows) {
      notJ++;
      continue;
    }
    ...
    // G
    // printf("yy=%d xx=%d\n", yy, xx) ;
    G(0) = getInterpolatedElementEigen(dGx, reprojections(0, i),
                                       reprojections(1, i));
    G(1) = getInterpolatedElementEigen(dGy, reprojections(0, i),
                                       reprojections(1, i));
    //        G(0) = dGx(yy,xx);
    //        G(1) = dGy(yy,xx);
  ...
  ROS_INFO("Jacobians computed at %d of %d edge locations", _3d.cols() - notJ,
           _3d.cols());
  ROS_INFO("[computeJacobianOfNowFrame()] Visibility ratio : %2.2f",
           (float)(_3d.cols() - notJ) / (float)_3d.cols());
#endif
}

/// @brief Given the reprojection mask, get corresponding epsilons and also
/// weight as a vector Reads the epsilon (error) values at the reprojected
/// location (not computed here). Errors-vals here are the distance-transform
/// values of the now frame at reprojected locations
...
/// @param[out] weights : corresponding weights (for solving equation with Huber
/// norm)
/// @returns : Ratio of visible points to the total points tracked
/// \see computeJacobianOfNowFrame
float SolveDVO::getReprojectedEpsilons(STATE *keyFrame, STATE *currentFrame,
                                       CALIBRATION_PAR *cali, int level) {
  Eigen::MatrixXf &_nowDist = currentFrame->distanceTransformEigen[level];
  ...
  for (int i = 0; i < reprojections.cols(); i++) {
    ...
    if (reprojections(0, i) < 2 || reprojections(0, i) > w ||
        reprojections(1, i) < 2 || reprojections(1, i) > h) {
      notJ++;
      // epsilon(i) = 200;
      // note : On having this one stange thing that happens is as points go out
      // of scene the energy will increase even though the estimates
      // are getting better. Need a better way to take care of decreasing energy
      // due to points going out of scene.
      continue;
    }

    //#ifdef __INTERPOLATE_DISTANCE_TRANSFORM
    //        epsilon(i) = interpolate(_nowDist,reprojections(1,i),
    //        reprojections(0,i) );
    //#else
    //        epsilon(i) = _nowDist( int(reprojections(1,i)+0.5),
    //        int(reprojections(0,i)+0.5) );
    //#endif
    epsilon(i) = getInterpolatedElementEigen(_nowDist, reprojections(0, i),
                                             reprojections(1, i));

    weights(i) = getWeightOf(epsilon(i));
  }
#ifdef __SHOW_REPROJECTIONS_EACH_ITERATION__
  ROS_INFO("Epsilon computed at %d of %d reprojected points",
           reprojections.cols() - notJ, reprojections.cols());
  ROS_INFO("[getReprojectedEpsilons()] Visibility ratio : %2.2f",
           (float)(reprojections.cols() - notJ) / (float)reprojections.cols());
#endif

  float ratio_of_visible_pts =
      (float)(reprojections.cols() - notJ) / (float)reprojections.cols();

  return ratio_of_visible_pts;
}

void SolveDVO::visualizeDistanceResidueHeatMap(
    cv::Mat &tmpIm, Eigen::MatrixXi &reprojectionMask,
    Eigen::MatrixXf &nowDistMap) {
  //    assert( (eim.rows() == reprojectionMask.rows()) && "Image and mask rows
  //    must match"); assert( (eim.cols() == reprojectionMask.cols()) && "Image
  //    and mask cols must match");
...
void SolveDVO::cordList_2_mask(Eigen::MatrixXf &list, Eigen::MatrixXi &mask) {
  ...
  for (int i = 0; i < list.cols(); i++) {
    if (list(0, i) < 0 || list(0, i) >= mask.cols() || list(1, i) < 0 ||
        list(1, i) >= mask.rows()) {
      continue;
    }
...
void SolveDVO::visualizeEnergyProgress(Eigen::VectorXf energyAtEachIteration,
                                       int bestEnergyIndex, int XSPACING) {
  ...
  cv::Mat energyPlot = cv::Mat::zeros(500, len * XSPACING + 40, CV_8UC3) +
                       cv::Scalar(255, 255, 255);

  // draw axes
  cv::line(energyPlot, cv::Point(20, 0), cv::Point(20, energyPlot.rows),
           cv::Scalar(0, 0, 0)); // y-axis
  cv::line(energyPlot, cv::Point(0, energyPlot.rows - 20),
           cv::Point(XSPACING * len + 40, energyPlot.rows - 20),
           cv::Scalar(0, 0, 0)); // x-axis

  // mark axes
  for (int p = 0; p < len; p += 10) { // plot's x-axis
    char str[40];
    sprintf(str, "%d", p);
    cv::circle(energyPlot, cv::Point(XSPACING * p + 20, energyPlot.rows - 20),
               2, cv::Scalar(0, 0, 255), -1);
    cv::putText(energyPlot, str,
                cv::Point(XSPACING * p + 20, energyPlot.rows - 10),
  ...
  for (float p = 0.0; p < 100000.0; p += 5000.0) { // plot y-axis labels
    ...
    cv::circle(energyPlot, cv::Point(20, energyPlot.rows - 20 - p / 200.0), 2,
               cv::Scalar(0, 0, 255), -1);
    cv::putText(energyPlot, str, cv::Point(3, energyPlot.rows - 25 - p / 200.0),
                cv::FONT_HERSHEY_COMPLEX_SMALL, .5, cv::Scalar(0, 0, 0));
  }
  ...
  for (int i = 0; i < energyAtEachIteration.rows(); i++) {
    ...
    cv::line(
        energyPlot, cv::Point(XSPACING * i + 20, energyPlot.rows - 20),
        cv::Point(XSPACING * i + 20, energyPlot.rows - 20 - energy / 200.0),
        cv::Scalar(255, 0, 0));
  }
  ...
  if (bestEnergyIndex >= 0) {
    ...
    cv::line(energyPlot,
             cv::Point(XSPACING * bestEnergyIndex + 20, energyPlot.rows - 20),
             cv::Point(XSPACING * bestEnergyIndex + 20,
                       energyPlot.rows - 20 - energy / 200.0),
             cv::Scalar(0, 255, 0));
  }
...
float SolveDVO::runIterations(STATE *keyFrame, STATE *currentFrame,
                              CALIBRATION_PAR *cali, int level,
                              int maxIterations, Eigen::Matrix3d &cR,
                              Eigen::Vector3d &cT,
                              Eigen::VectorXf &energyAtEachIteration,
                              Eigen::VectorXf &finalEpsilons,
                              Eigen::MatrixXf &finalReprojections,
                              int &bestEnergyIndex, float &finalVisibleRatio) {

  bool waitPerItr = true;
  assert(level >= 0 && level <= 3);
  assert(maxIterations > 0);
  // assert( isRefFrameAvailable && isNowFrameAvailable &&
  // isCameraIntrinsicsAvailable );
  ...
  ROS_DEBUG("-*-*-*-*- Start of Gauss-Newton Iterations (level=%d) -*-*-*-*-",
            level);
  ...
  for (int itr = 0; itr < maxIterations; itr++) {
    ...
    //
    // Compute Jacobian of the **NOW** frame (not the ref frame) (this is
    // forward formulation)
    //          Be careful here since computeJacobian() function computes
    //          jacobiann at zero of the reference frame. Here we have to
    //          compute jacobian at
    //                    **R_cap**,    **T_cap**.
    // SpaceCordList _refEdge = _ref_edge_3d[level];
    JacobianLongMatrix Jcbian =
        Eigen::MatrixXf::Zero(keyFrame->edge_3d[level].cols(), 6);
    ...
    //        }
    computeJacobianOfNowFrame(keyFrame, currentFrame, cali, level, cR_32, cT_32,
                              Jcbian);
    if (level == 0) {
...
#ifdef __SHOW_REPROJECTIONS_EACH_ITERATION__
    ROS_INFO("size of reprojection : %d %d", reprojections.rows(),
             reprojections.cols());
#endif
    ...
    float ratio_of_visible_pts =
        getReprojectedEpsilons(keyFrame, currentFrame, cali, level);
...
#ifdef __SHOW_REPROJECTIONS_EACH_ITERATION__
    ROS_INFO("Best iteration so far is #%d. Total epsilon : %f", bestItrNumber,
             bestTotalEpsilon);
#endif
    ...
    // PVec<<factorBig, factorBig, factorBig   , factorSmall, factorSmall,
    // factorSmall;
    ...
    // Square summable but not summable seq of step-length (as recommended in
    // Boyd's documents)
    stepLength = 9.0 * 1.0E-2 /
                 ((itr > 5) ? (double)(itr - 4)
                            : 1.0); // dont turn down the rate too soon
    ...
    // Mike Shuster's Advice. Increase stepLen when grads point in same
    // direction
    bool gradPointsInSameDirection = true;
    if (((g.transpose() * g_prev) > 0)) { // same direction
      //            stepLength =(stepLength>1000)?stepLength: stepLength * 1.2f;
      //            //done to avoid floating point problems
      gradPointsInSameDirection = true;
    } else {
      //            stepLength =(stepLength<1E-9)?stepLength: stepLength / 2.0f;
      //            //done to avoid floating point problems
    ...
    char xcttt[500];
    sprintf(xcttt, "raw grad (%.4lf, %.4lf %.4lf %s dirn)", g_norm, s_norm,
            s_norm / g_norm, gradPointsInSameDirection ? "same" : "opposite");
...
#ifdef __SHOW_REPROJECTIONS_EACH_ITERATION__
      ROS_INFO_STREAM("psi : (after trust region constraint) [ "
                      << psi.transpose() << "]\n"
                      << "|psi| : " << psi.norm());
...
#ifdef __SHOW_REPROJECTIONS_EACH_ITERATION__
      ROS_INFO("Early Termination. psi.norm() {%f} smaller than %f", psi.norm(),
               psiNormTerminationThreshold);
#endif
      break;
    }

    // if( (itr - bestItrNumber) > 10 ) //if no betterment in function value for
    // X iterations then give up
    //    break;
    ...
    if (waitPerItr) {
      if (__REPROJECTION_LEVEL == level) {
        Eigen::MatrixXi reprojectedMask =
            Eigen::MatrixXi::Zero(_now.rows(), _now.cols());
        ...
        sOverlay(im_r[level], ref_edge_map[level], outRef,
                 cv::Vec3b(0, 0, 255));
        ...
        visualizeDistanceResidueHeatMap(im_n[level], reprojectedMask,
                                        now_distance_transform[level]);
        visualizeEnergyProgress(energyAtEachIteration, bestItrNumber,
                                (energyAtEachIteration.rows() < 300) ? 4 : 1);

#if !defined(__PRINT_POSE_EACH_ITERATION) &&                                   \
    !defined(__PRINT_GRAD_DIRECTION_EACH_ITERATION)
...
#ifdef __SHOW_REPROJECTIONS_EACH_ITERATION__DISPLAY_ONLY
  ROS_DEBUG("Best Energy Achieved (%f) in iteration #%d with visibility-ratio "
            "of %2.2f",
            bestTotalEpsilon, bestItrNumber, finalVisibleRatio);
  ROS_DEBUG_STREAM("final R :\n" << cR);
  ROS_DEBUG_STREAM("final T :\n" << cT.transpose());
  ROS_INFO("-*-*-*-*- End of Gauss-Newton Iterations (level=%d) -*-*-*-*- ",
           level);
#endif
...
float SolveDVO::optimizeGaussianNewton(
    STATE *keyFrame, STATE *currentFrame, CALIBRATION_PAR *cali, int level,
    int maxIterations, Eigen::Matrix3d &cR, Eigen::Vector3d &cT,
    Eigen::VectorXf &energyAtEachIteration, Eigen::VectorXf &finalEpsilons,
    Eigen::MatrixXf &finalReprojections, int &bestEnergyIndex,
    float &finalVisibleRatio) {
  ...
  for (int itr = 0; itr < maxIterations; itr++) {
    ...
    igl::repmat(repmatT, 1, _3d.cols(),
                cTRep); // repeat cT col-wise (uses IGL library)
    Eigen::MatrixXf _3d_transformed =
        (cR.cast<float>()).transpose() * (_3d - cTRep);
    Eigen::RowVector2f G;
    Eigen::Matrix<float, 2, 3> A1;
    ...
    for (int i = 0; i < numOfPoints; i++) {
      ...
      reprojections(1, i) =
          ...
          cali->cy[level];
      if (reprojections(0, i) < 2 || reprojections(0, i) > nCols ||
          reprojections(1, i) < 0 || reprojections(1, i) > nRows) {
        notJ++;
        continue;
      }
#ifdef MY_SSE
      buf_warped_residual[idx] = getInterpolatedElementEigen(
          _nowDist, reprojections(0, i), reprojections(1, i));
      ...
      buf_warped_dx[idx] = getInterpolatedElementEigen(dGx, reprojections(0, i),
                                                       reprojections(1, i));
      buf_warped_dy[idx] = getInterpolatedElementEigen(dGy, reprojections(0, i),
                                                       reprojections(1, i));
      idx++;
#else
      epsilon(i) = getInterpolatedElementEigen(_nowDist, reprojections(0, i),
      ...
      oneByThree(0, 2) = -G(0) * cali->fx[level] * X / (Z * Z) -
                         G(1) * cali->fy[level] * Y / (Z * Z);
      //            A1(0,0) = cali->fx[level]/Z;
      //            A1(0,1) = 0.;
      //            A1(0,2) = -cali->fx[level]*X/(Z*Z);
      //            A1(1,0) = 0.;
    ...
    for (int i = 0; i < buf_warped_size - 3; i += 4) {
      ...
      if (i + 3 < buf_warped_size) {
        ...
        for (int k = 0; i + k < buf_warped_size; k++) {
          ...
          ls.update(v6, *(buf_warped_residual + i + k),
                    *(buf_weight_p + i + k));
        }
      }
      // ls.updateSSE(J61, J62, J63, J64, J65, J66,
      // _mm_load_ps(buf_warped_residual+i), _mm_load_ps(buf_weight_p+i));
    }
    ls.finish();
#endif
    ...
    float ratio_of_visible_pts =
        (float)(reprojections.cols() - notJ) / (float)reprojections.cols();
    float currentTotalEpsilon =
        sqrt(sumOfError) / (reprojections.cols() - notJ);
    energyAtEachIteration[itr] = currentTotalEpsilon;
    ...
    // Early Termination ?
    if (psi.norm() < psiNormTerminationThreshold) {
      // ROS_INFO( "Early Termination. psi.norm() {%f} smaller than %f",
      // psi.norm(), psiNormTerminationThreshold );
      break;
    }
    Sophus::SE3d mat = Sophus::SE3d::exp(psi);
...
/// @brief Updates the current estimates of R_{t}, T_{t} using R_h, T_h ie. the
/// best deltas from previous iteration
void SolveDVO::updateEstimates(Eigen::Matrix3f &cR, Eigen::Vector3f &cT,
                               Eigen::Matrix3f &R_h, Eigen::Vector3f &T_h) {
  ///////////////////////////////////////////////// 2nd Innings
  //////////////////////////////////
...
/// Computation of Image gradient along X and along Y. For now the input image
/// (in Eigen format) is converted to Opencv. The gradient is computed using
/// opencv functions and results are converted back to Eigen format.
...
void SolveDVO::imageGradient(Eigen::MatrixXf &image, Eigen::MatrixXf &gradientX,
                             Eigen::MatrixXf &gradientY) {
  ...
  cv::Mat kernX = (cv::Mat_<float>(3, 3) << 0, 0, 0, -0.5, 0.0, .5, 0, 0, 0);
  cv::Mat kernY = (cv::Mat_<float>(3, 3) << 0, -0.5, 0, 0, 0.0, 0, 0, 0.5, 0);
...
void SolveDVO::exponentialMap(Eigen::VectorXf &psi, Eigen::Matrix3f &outR,
                              Eigen::Vector3f &outT) {
  ...
  Eigen::Matrix3f xR = I3 + sin(theta) / theta * wx +
                       (1.0 - cos(theta)) / (theta * theta) * wx * wx;

  Eigen::Matrix3f xV = I3 + (1 - cos(theta)) / (theta * theta) * wx +
  ...
  ROS_INFO("out Quaternion : %f %f %f %f", quat.x(), quat.y(), quat.z(),
           quat.w());

  // outTr << xR, xV*t, zer_row, 1.0;
}

void SolveDVO::sOverlay(Eigen::MatrixXf eim, Eigen::MatrixXi mask, cv::Mat &xim,
                        cv::Vec3b color) {
...
//    assert( (_refEdge.rows() == _refEdge.rows())  &&  (_refEdge.cols() ==
//    _refEdge.cols()) );
...
/// The solution is to enforce singular values to be either +-1 in place og
/// 0.999999.
...
/// @brief Gives the value of the matrix F at position (y,x). y:rowIndx.
/// x:colIndx. Uses Bilinear interpolation
float SolveDVO::interpolate(Eigen::MatrixXf &F, float ry, float rx) {
  ...
  // In the following d_xdyd_xuyd means it is a linear combination of 2 terms
  // viz.
  // 1. first term from down of x (xd), down of y (yd)
  // 2. 2nd term from up of x (xu), down of y (yd)
  float f_xdyd_xuyd = sqrt((1.0f - inc_x) * F(ry_d, rx_d) * F(ry_d, rx_d) +
                           (inc_x)*F(ry_d, rx_u) * F(ry_d, rx_u));
  float f_xdyu_xuyu = sqrt((1.0f - inc_x) * F(ry_u, rx_d) * F(ry_u, rx_d) +
                           (inc_x)*F(ry_u, rx_u) * F(ry_u, rx_u));

  // Interpolate the above obtained intermediate terms
  float f_xy = sqrt((1.0f - inc_y) * f_xdyd_xuyd * f_xdyd_xuyd +
                    inc_y * f_xdyu_xuyu * f_xdyu_xuyu);
...
void SolveDVO::printRT(Eigen::Matrix3f &fR, Eigen::Vector3f &fT,
                       const char *msg) {
  //#ifdef __SHOW_REPROJECTIONS_EACH_ITERATION__
  //  ROS_INFO( "____%s____", msg );
  // ROS_INFO( "fR\n[" << fR << "]" );
  Eigen::Quaternionf quat(fR);
  ROS_INFO("6-DOF (%s) : %-.4f %-.4f %-.4f %-.4f : %-.4f %-.4f %-.4f", msg,
           quat.x(), quat.y(), quat.z(), quat.w(), fT(0), fT(1), fT(2));
  // ROS_INFO_STREAM( "fT : [" << fT.transpose() << "]" );
  // ROS_INFO( "____" );
  //#endif
}

void SolveDVO::printRT(Eigen::Matrix3d &fR, Eigen::Vector3d &fT,
                       const char *msg) {
  Eigen::Quaterniond quat(fR);
  ROS_INFO("6-DOF (%s) : %-.4lf %-.4lf %-.4lf %-.4lf : %-.4lf %-.4lf %-.4lf",
           msg, quat.x(), quat.y(), quat.z(), quat.w(), fT(0), fT(1), fT(2));
}

void SolveDVO::printPose(geometry_msgs::Pose &rospose, const char *msg,
                         std::ostream &stream) {
  ROS_INFO("%s : [ %.4lf %.4lf %.4lf %.4lf :: %.4lf %.4lf %.4lf ]", msg,
           rospose.orientation.x, rospose.orientation.y, rospose.orientation.z,
           rospose.orientation.w, rospose.position.x, rospose.position.y,
           rospose.position.z);
...
void SolveDVO::printDescentDirection(Eigen::VectorXd d, const char *msg) {
  ...
  ROS_INFO(
      "Direction (%s) : %-4.4lf %-4.4lf %-4.4lf ::: %-4.4lf %-4.4lf %-4.4lf",
      msg, d(0), d(1), d(2), d(3), d(4), d(5));
}

void SolveDVO::printDescentDirection(Eigen::VectorXf d, const char *msg) {
  assert(d.rows() == 6);

  ROS_INFO("Direction (%s) : %-4.4f %-4.4f %-4.4f ::: %-4.4f %-4.4f %-4.4f",
           msg, d(0), d(1), d(2), d(3), d(4), d(5));
}

float SolveDVO::getDriftFromPose(geometry_msgs::Pose &p1,
...
void SolveDVO::analyzeDriftVector(std::vector<float> &v) {
  ...
  // ROS_INFO( "Mean : %2.4f :::: Median : %2.4f :::: RMS : %2.4f", mean,
  // median, rms );

  float perSec = (float)v.size() / 30.0f;
  ROS_INFO("Mean : %2.4f :::: Median : %2.4f :::: RMS : %2.4f", mean / perSec,
           median / perSec, rms / perSec);
}

/// @brief Compute and visualize histogram of the residues
/// @param[in] residi : Residue at pixel location. Nx1
/// @param[in] quite : `true` will suppress the display. By default the display
/// is off
/// @returns : Return the `b` param of the MLE laplacian distribution
/// @note : This is customized to show integer valued bins only. 0 to 200 on
/// x-axis. 0 to 2500 on y-axis
float SolveDVO::processResidueHistogram(Eigen::VectorXf &residi,
                                        bool quite = true) {
  ...
  for (int i = 0; i < residi.rows(); i++) {
    ...
    // +1 is to take care of the '-1' in the absolute-residues which means the
    // residue at this point is not calculated since the re-projected point is
    // not visible in now frame
  }
  histogram /=
      residi.rows(); // normalized the histogram, now histogram \in [0,1]

  cv::Mat histPlot;
  if (quite == false) {
    histPlot = cv::Mat::zeros(500, 450, CV_8UC3) + cv::Scalar(255, 255, 255);
    // red-dots on vertical
    cv::line(histPlot, cv::Point(1, histPlot.rows - 1), cv::Point(1, 0),
             cv::Scalar(0, 0, 0));
    for (float mag = 0; mag < .95f; mag += 0.05f) {
      cv::circle(histPlot, cv::Point(1, histPlot.rows - 20 - int(mag * 500.0)),
                 2, cv::Scalar(0, 0, 255), -1);
      char toS[20];
      sprintf(toS, "%.2f", mag);
      cv::putText(histPlot, toS,
                  cv::Point(10, histPlot.rows - 20 - int(mag * 500.0)),
                  cv::FONT_HERSHEY_COMPLEX_SMALL, .7, cv::Scalar(0, 0, 0));
    }
  }

  //    char tmpS[20];
  //    sprintf( tmpS, "%f", histogram(i) );
  //    cv::putText(histPlot, tmpS, cv::Point(250,20),
  ...
  if (quite == false) {
    ...
    for (int i = 0; i < 256; i++) {
      ...
      cv::line(histPlot, cv::Point(2 * i, histPlot.rows - 20),
               cv::Point(2 * i, histPlot.rows - 20 - int(mag * 500.0)),
               cv::Scalar(255, 0, 0));
      if ((2 * (i - 1)) % 50 == 0) {
        cv::circle(histPlot, cv::Point(2 * i, histPlot.rows - 20), 2,
                   cv::Scalar(0, 0, 255), -1);
        char toS[20];
        sprintf(toS, "%d", i - 1);

        cv::putText(histPlot, toS, cv::Point(2 * i, histPlot.rows - 5),
                    cv::FONT_HERSHEY_COMPLEX_SMALL, .7, cv::Scalar(0, 0, 0));
      }
    }
  }
  ...
  if (quite == false) {
    ...
    cv::putText(histPlot, toS, cv::Point(40, 20),
                cv::FONT_HERSHEY_COMPLEX_SMALL, .5, cv::Scalar(0, 0, 0));
    // Plot the estimated laplacian distribution
    for (int i = 1; i < 256; i++) {
      float mag = 1 / (2 * b_cap) * exp(-(i - 1) / b_cap);
      cv::circle(histPlot, cv::Point(2 * i, histPlot.rows - 20 - mag * 500.), 2,
...
// void SolveDVO::visualizeResidueHeatMap(Eigen::MatrixXf &eim, Eigen::MatrixXf
// &residueAt)
//{
//    assert( (eim.rows() == residueAt.rows()) && "Image and mask rows must
//    match"); assert( (eim.cols() == residueAt.cols()) && "Image and mask cols
//    must match");
...
// void SolveDVO::visualizeDistanceResidueHeatMap(Eigen::MatrixXf &eim,
// Eigen::MatrixXi &reprojectionMask, Eigen::MatrixXf &nowDistMap )
//{
//    assert( (eim.rows() == reprojectionMask.rows()) && "Image and mask rows
//    must match"); assert( (eim.cols() == reprojectionMask.cols()) && "Image
//    and mask cols must match");
...
//    // draw axes
//    cv::line( energyPlot, cv::Point(20,0), cv::Point(20,energyPlot.rows),
//    cv::Scalar(0,0,0) ); //y-axis cv::line( energyPlot,
//    cv::Point(0,energyPlot.rows-20), cv::Point(XSPACING*len+40,
//    energyPlot.rows-20), cv::Scalar(0,0,0) ); //x-axis

//    // mark axes
//    for( int p=0 ; p<len ; p+=10 ) { // plot's x-axis
//        char str[40];
//        sprintf( str, "%d", p );
//        cv::circle( energyPlot, cv::Point(XSPACING*p+20,
//        energyPlot.rows-20),2, cv::Scalar(0,0,255), -1 ); cv::putText(
//        energyPlot, str, cv::Point(XSPACING*p+20, energyPlot.rows-10),
//        cv::FONT_HERSHEY_COMPLEX_SMALL, .5, cv::Scalar(0,0,0));
//    }
//    for( float p=0.0 ; p<100000.0 ; p+=5000.0 ) { //plot y-axis labels
//        char str[40];
//        sprintf( str, "%2.0f00", (int) p/1000.0 );
//        cv::circle( energyPlot, cv::Point(20,energyPlot.rows - 20 -
//        p/200.0),2, cv::Scalar(0,0,255), -1); cv::putText( energyPlot, str,
//        cv::Point(3, energyPlot.rows - 25 - p/200.0),
//        cv::FONT_HERSHEY_COMPLEX_SMALL, .5, cv::Scalar(0,0,0) );
//    }
...
//    {
//        float energy = energyAtEachIteration(i);
//        cv::line( energyPlot, cv::Point(XSPACING*i+20, energyPlot.rows-20),
//        cv::Point(XSPACING*i+20, energyPlot.rows - 20 - energy/200.0),
//        cv::Scalar(255,0,0));
//    }
...
//    if( bestEnergyIndex >= 0 ) {
//    float energy = energyAtEachIteration(bestEnergyIndex);
//    cv::line( energyPlot, cv::Point(XSPACING*bestEnergyIndex+20,
//    energyPlot.rows-20), cv::Point(XSPACING*bestEnergyIndex+20,
//    energyPlot.rows - 20 - energy/200.0), cv::Scalar(0,255,0));
//    }

//    cv::imshow( "energy progress", energyPlot );
//}

// void SolveDVO::visualizeReprojectedDepth(Eigen::MatrixXf &eim,
// Eigen::MatrixXf &reprojDepth)
//{
//    assert( (eim.rows() == reprojDepth.rows()) && "Image and mask rows must
//    match"); assert( (eim.cols() == reprojDepth.cols()) && "Image and mask
//    cols must match");
...
///// @brief Computes the distance transform of the reference frame (all levels)
/// and stores in std::vector
// void SolveDVO::computeDistTransfrmOfRef()
//{
//    assert( isRefFrameAvailable && "Reference frames need to be available in
//    order to compute dist transform on them" );
...
//        cv::distanceTransform( refEdge, refDistTransCvMat, CV_DIST_L2,
//        CV_DIST_MASK_PRECISE );

//#ifdef __SCALE_NORMALIZE_DISTANCE_TRANFROM
//        cv::normalize(refDistTransCvMat, refDistTransCvMat, 0.0, 255.0,
//        cv::NORM_MINMAX);
//#endif

////        double min, max;
////        cv::minMaxLoc(refDistTransCvMat, &min, &max);
//        //ROS_INFO_STREAM( "min : "<< min << " max : "<< max << " dataTYpe :
//        "<< cvMatType2str(refDistTransCvMat.type()) );
...
//{
//    assert( isNowFrameAvailable && "Now frames need to be available in order
//    to compute dist transform on them" );
...
//        cv::distanceTransform( nowEdge, nowDistTransCvMat, CV_DIST_L2,
//        CV_DIST_MASK_PRECISE );

//#ifdef __SCALE_NORMALIZE_DISTANCE_TRANFROM
//        cv::normalize(nowDistTransCvMat, nowDistTransCvMat, 0.0, 255.0,
//        cv::NORM_MINMAX);
//#endif

////        double min, max;
////        cv::minMaxLoc(nowDistTransCvMat, &min, &max);
//        //ROS_INFO_STREAM( "(Now)min : "<< min << " max : "<< max << "
//        dataTYpe : "<< cvMatType2str(nowDistTransCvMat.type()) );
//        ///// END DISTANCE TRANSFORM
...
///// @brief Only loop through the images received (now frame) and optionally
/// GT. No DVO estimation.
...
//            break;
//        }
//        sprintf( frameFileName, "%s/framemono_%04d.xml",
//        __DATA_FROM_XML_FILES__, iDataFrameNum ); bool flag =
//        loadFromFile(frameFileName ); if( flag == false ) {
...
//        try{
//            tflis.lookupTransform("/world", "/openni_rgb_optical_frame",
...
///// @brief The event loop. Basically an ever running while with
/// ros::spinOnce()
///// This is a re-implementation taking into care the memory scopes and also
/// processing only points with higher image gradients
// void SolveDVO::loop()
//{
...
//    //
//    // =================================  Capture the 1st frame (as ref-frame)
//    unconditionally ============================== while( ros::ok() )
//    {
//        ros::spinOnce();
//#ifdef __DATA_FROM_XML_FILES__
//        int iDataFrameNum = __DATA_FROM_XML_FILES__START + nFrame;
...
//        tf::StampedTransform transform;
//        try{
//             //block until it is available. maximum wait for 1 sec for tf to
//             be active
//            tflis.waitForTransform("/world", "/openni_rgb_optical_frame",
...
//        }
//        catch(tf::TransformException &ex) {
//            ROS_ERROR("NO TF Message. Will set one of the subsequent frame as
//            the 1st frame. Everything might be slightly off");
//            ROS_ERROR("%s",ex.what());
//        }

//        if( _tf_GT_1_frame_available==false ) //make a blue sphere in case of
//        slight delay in tf
...
//            nFrame++;
//            ROS_INFO( "Done setting 1st received frame as reference..now will
//            continue processing frames" ); signalGetNewRefImage = false;
//            rate.sleep();
//            break;
//        }
//        rate.sleep();
...
//        ros::spinOnce();
//#ifdef __DATA_FROM_XML_FILES__
//        int iDataFrameNum = __DATA_FROM_XML_FILES__START +
//        __DATA_SKIP_FACTOR*nFrame; if( iDataFrameNum >
//        __DATA_FROM_XML_FILES__END )
//        {
//            ROS_ERROR( "Done with all files...Quitting..." );
//            break;
//        }
...
////        runIterations(2, 50, cR, cT, energyAtEachIteration, epsilonVec,
/// reprojections, bestEnergyIndex, visibleRatio  ); /        runIterations(1,
/// 100, cR, cT, energyAtEachIteration, epsilonVec, reprojections,
/// bestEnergyIndex, visibleRatio ); /        runIterations(0, 300, cR, cT,
/// energyAtEachIteration, epsilonVec, reprojections, bestEnergyIndex,
/// visibleRatio );
...
//            {
//                ROS_INFO( "Run %d iterations @ level %d", iterationsConfig[f],
//                f ); runIterations(f, iterationsConfig[f], cR_64, cT_64,
//                energyAtEachIteration, epsilonVec, reprojections,
//                bestEnergyIndex, visibleRatio );
//            }
//        }

//        ros::Duration jdur = ros::Time::now() - jstart;
//        iterationsComputeTime = jdur.toSec();
//        avgIterationsTime = avgIterationsTime*(nFrame-1)/nFrame +
//        iterationsComputeTime/nFrame; ROS_INFO( "Iterations done in %lf ms",
//        iterationsComputeTime*1000 );

//        //mviz.publishFullPointCloud();

//        // This was a critical-patch. Fixed a critical bug. The exit
//        conditions need to be checked on the f_best and not all the epsilons.
//        // Before this was done in gaussNewtonIteration which was obiviously
//        not correct as it checkd the threshold for the last iteration
//        // and not the best iteration. Similar fix need to be done with
...
//        // ================== Change KeyFrame ? ====================
//        //   Check if the estimation was bad and if there is a need to change
//        key-frame

//        /*
//        // These 3 following IFs will check if it is time to change the
//        reference frame if( b_cap > laplacianThreshExitCond ) {
//            ROS_ERROR( "Fitted laplacian b: %.2f. Laplacian b_thresh : %.2f.
//            Signal change of reference frame", b_cap, laplacianThreshExitCond
//            ); snprintf( signalGetNewRefImageMsg, 450, "Fitted laplacian b:
//            %.2f. Laplacian b_thresh : %.2f. Signal change of reference
//            frame", b_cap, laplacianThreshExitCond ); signalGetNewRefImage =
//            true; reasonForChange=2;
//        }

//        if( visibleRatio < ratio_of_visible_pts_thresh ) {
//            ROS_ERROR( "Only %.2f of tracked points visible. Required %.2f.
//            Signal change of reference frame", visibleRatio,
//            ratio_of_visible_pts_thresh); snprintf(signalGetNewRefImageMsg,
//            450, "Only %.2f of tracked points visible. Required %.2f. Signal
//            change of reference frame", visibleRatio,
//            ratio_of_visible_pts_thresh ); signalGetNewRefImage = true;
//            reasonForChange=3;
//        }

//        if( reprojections.cols() < 50 ) //this was put in here after
//        experience from TUM-RGBD/kidnap sequence
//        {
...
//        // the 2nd condition here says that if 'n-1'th frame was already a
//        reference frame than turn a blind eye to the warning on
//        // cR, cT being bad
...
//            //rerun iterations
////            runIterations(2, 50, cR, cT, energyAtEachIteration, epsilonVec,
/// reprojections, bestEnergyIndex, visibleRatio  ); / runIterations(1, 100, cR,
/// cT, energyAtEachIteration, epsilonVec, reprojections, bestEnergyIndex,
/// visibleRatio ); /            runIterations(0, 300, cR, cT,
/// energyAtEachIteration, epsilonVec, reprojections, bestEnergyIndex,
/// visibleRatio );
...
//                if( iterationsConfig[f] > 0 )
//                {
//                    ROS_INFO( "re-Run %d iterations @ level %d",
//                    iterationsConfig[f], f ); runIterations(f,
//                    iterationsConfig[f], cR_64, cT_64, energyAtEachIteration,
...
//        else
//        {
//            //Currently estimated pose was acceptable wrt to lastRefFrame so
//            push as ordinary
////            cR_64 = cR.cast<double>();
////            cT_64 = cT.cast<double>();
//            gop.pushAsOrdinaryFrame(nFrame, cR_64, cT_64);
...
//#ifdef __WRITE_EST_POSE_TO_FILE
//        printPose(__currentEstimatedPose, "main/__currentEstPose ",
//        estPoseFile );
//#else
...
//        float drift = getDriftFromPose( __currentGTPose,
//        __currentEstimatedPose ); drifts.push_back(drift); analyzeDriftVector(
//        drifts );
//#endif //__TF_GT__
...
//            int xlevel = 0;
//            Eigen::MatrixXi reprojectedMask =
//            Eigen::MatrixXi::Zero(im_n[xlevel].rows(), im_n[xlevel].cols());
//            cordList_2_mask(reprojections, reprojectedMask);
//            cv::Mat outIm,outIm2, outImGray, outRef;
//            sOverlay(now_distance_transform[xlevel], reprojectedMask, outIm,
//            cv::Vec3b(0,255,0));
...
//            sOverlay(im_n[xlevel], reprojectedMask, outImGray,
//            cv::Vec3b(0,255,0));

//            sOverlay(im_r[xlevel], ref_edge_map[xlevel], outRef,
//            cv::Vec3b(0,0,255));

//            cv::imshow( "reprojection with cR,cT (on now dist-tran", outIm);
//            //cv::imshow( "reprojection with cR,cT (on now gray", outImGray);
//            cv::imshow( "selected edges on ref", outRef);
//            visualizeDistanceResidueHeatMap(im_n[xlevel], reprojectedMask,
//            now_distance_transform[xlevel] );

//            //printFrameIndex2Scratch(debugScratchBoard, nFrame, lastRefFrame,
//            lastRefFrameComputeTime, gaussNewtonIterationsComputeTime, true );
...
//        cv::Mat outRef;
//        sOverlay(im_r[xlevel], ref_edge_map[xlevel], outRef,
//        cv::Vec3b(0,0,255)); cv::imshow( "selected edges on ref", outRef);

//        visualizeDistanceResidueHeatMap(im_n[xlevel], reprojectedMask,
//        now_distance_transform[xlevel] );

//        visualizeEnergyProgress( energyAtEachIteration, bestEnergyIndex,
//        (energyAtEachIteration.rows() < 300)?2:1 );

//        char ch = cv::waitKey(__MINIMAL_DISPLAY);
//        if( ch == 27 ){ // ESC
//            ROS_ERROR( "ESC pressed quitting...");
//            ROS_ERROR( "Average iterations time per frame : %lf ms",
//            avgIterationsTime*1000.0 ); exit(1);
//        }
...
//        isFrameAvailable = false; //this is put in place to process at-max the
//        arrival rate.
...
//    // Run iterations
//    runIterations(0, 100, cR_64, cT_64, energyAtEachIteration, epsilonVec,
//    reprojections, bestEnergyIndex, visibleRatio );
...
/// This is a re-implementation taking into care the memory scopes and also
processing only points with higher image gradients void SolveDVO::loopFromFile()
{
...
//            gaussNewtonIterations(2, 20, cR, cT, energyAtEachIteration,
epsilonVec, reprojections, bestEnergyIndex, visibleRatio  );
//            gaussNewtonIterations(1, 20, cR, cT, energyAtEachIteration,
epsilonVec, reprojections, bestEnergyIndex, visibleRatio ); runIterations(0,
500, cR, cT, energyAtEachIteration, epsilonVec, reprojections, bestEnergyIndex,
visibleRatio );
            }
        ...
        //{
            ...
            ros::Time jstart = ros::Time::now();
            runIterations(0, 500, cR, cT, energyAtEachIteration, epsilonVec,
reprojections, bestEnergyIndex, visibleRatio ); ros::Duration jdur =
ros::Time::now() - jstart; gaussNewtonIterationsComputeTime = jdur.toSec();
            ROS_INFO( "Iterations done in %lf ms",
gaussNewtonIterationsComputeTime*1000 );
...
not correct as it checkd the threshold for the last iteration
            // and not the best iteration. Similar fix need to be done with
%-visibility condition. #ifdef __ENABLE_DISPLAY__ float b_cap =
processResidueHistogram( epsilonVec, false ); #else float b_cap =
processResidueHistogram( epsilonVec, true ); #endif //__ENABLE_DISPLAY__



            if( b_cap > laplacianThreshExitCond ) {
                ROS_ERROR( "Fitted laplacian b: %.2f. Laplacian b_thresh : %.2f.
Signal change of reference frame", b_cap, laplacianThreshExitCond ); snprintf(
signalGetNewRefImageMsg, 450, "Fitted laplacian b: %.2f. Laplacian b_thresh :
%.2f. Signal change of reference frame", b_cap, laplacianThreshExitCond );
                signalGetNewRefImage = true;
            }
            ...
            if( visibleRatio < ratio_of_visible_pts_thresh ) {
                ROS_ERROR( "Only %.2f of tracked points visible. Required %.2f.
Signal change of reference frame", visibleRatio, ratio_of_visible_pts_thresh);
                snprintf(signalGetNewRefImageMsg, 450, "Only %.2f of tracked
points visible. Required %.2f. Signal change of reference frame", visibleRatio,
ratio_of_visible_pts_thresh ); signalGetNewRefImage = true;
            }
...
Eigen::MatrixXi::Zero(im_n[xlevel].rows(), im_n[xlevel].cols());
            ...
            cv::Mat outIm, outImGray, outRef;
            sOverlay(now_distance_transform[xlevel], reprojectedMask, outIm,
cv::Vec3b(0,255,0)); sOverlay(im_n[xlevel], reprojectedMask, outImGray,
cv::Vec3b(0,255,0));
...
(energyAtEachIteration.rows() < 300)?4:1 );
            ...
            if( iFrameNum == START ) { //only attempt to move windows in 1st
iteration cv::moveWindow("reprojection with cR,cT (on now dist-tran", 800, 600
...
now_distance_transform[xlevel] );
                ...
                visualizeEnergyProgress( energyAtEachIteration, bestEnergyIndex,
(energyAtEachIteration.rows() < 300)?4:2 );
...
/// @brief Given the input transform in tf format, returns the Eigen R,T
/// matrices
void SolveDVO::tfTransform2EigenMat(tf::StampedTransform &tr,
                                    Eigen::Matrix3f &R, Eigen::Vector3f &T) {
  Eigen::Quaternionf Qf(tr.getRotation().getW(), tr.getRotation().getX(),
                        tr.getRotation().getY(), tr.getRotation().getZ());
