...
void readParameters(ros::NodeHandle &n) {
  ...
  GRADIENT_THRESHOLD =
      readParam<double>(n, "gradient_threshold") / FOCAL_LENGTH;
  ...
  MIN_PARALLAX_POINT =
      readParam<double>(n, "min_parallax_point") / FOCAL_LENGTH;
  ...
  for (int i = 0; i < NUM_OF_CAM; i++) {
    RIC_OK.push_back(
        readParam<bool>(n, std::string("ric_ok") + std::to_string(i)));
    if (RIC_OK.back()) {
    ...
    TIC_OK.push_back(
        readParam<bool>(n, std::string("tic_ok") + std::to_string(i)));
    if (TIC_OK.back()) {
