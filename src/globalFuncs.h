...
inline float getInterpolatedElementEigen(const Eigen::MatrixXf &mat,
  ...
  float dy = y - iy;
  float dxdy = dx * dy;
  float res = dxdy * mat(iy + 1, ix + 1) + (dy - dxdy) * mat(iy + 1, ix) +
              (dx - dxdy) * mat(iy, ix + 1) +
...
inline cv::Mat getDepthRainbowPlot(const float *idepth, const float *idepthVar,
...
inline cv::Mat getVarRedGreenPlot(const float *idepthVar, const float *gray,
...
inline void calculateInvDepthImage(cv::Mat disparity, cv::Mat &iDepthImage,
                                   cv::Mat &iVar, float baseline, float f) {
