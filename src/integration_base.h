...
template <class Derived> class IntegrationBase {
public:
  ...
  void init_linearization_point(const Eigen::Vector3d &_linearized_ba,
                                const Eigen::Vector3d &_linearized_bg) {
  ...
  void push_back(double _dt, const Eigen::Vector3d &_acc_1,
  ...
  void repropagate(const std::vector<double> &dt_buf,
                   const std::vector<Eigen::Vector3d> &acc_buf,
                   const std::vector<Eigen::Vector3d> &gyr_buf) {
  ...
  struct functor {
    ...
    template <typename T>
    bool operator()(const T *const p, const T *const q, const T *const v,
                    const T *const ba, const T *const bg, const T *const a_0,
                    const T *const g_0, const T *const a_1, const T *const g_1,
                    T *e) const {
      ...
      derived->propagate_implementation(
          tmp_delta_p, tmp_delta_q, tmp_delta_v, tmp_linearized_ba,
          tmp_linearized_bg, tmp_acc_0, tmp_gyr_0, tmp_acc_1, tmp_gyr_1,
          delta_p, delta_q, delta_v, linearized_ba, linearized_bg);

      Eigen::Map<Eigen::Matrix<T, 3, 1>>(e + 0) = delta_p;
      Eigen::Map<Eigen::Matrix<T, 4, 1>>(e + 3) = Eigen::Matrix<T, 4, 1>{
  ...
  void propagate(double _dt, const Eigen::Vector3d &_acc_1,
    ...
    // derived->propagate_implementation(tmp_delta_p, tmp_delta_q, tmp_delta_v,
    // tmp_linearized_ba, tmp_linearized_bg,
    //                                  delta_p, delta_q, delta_v,
    //                                  linearized_ba, linearized_bg);

    ceres::CostFunction *cost_function =
        new ceres::AutoDiffCostFunction<functor, 16, 3, 4, 3, 3, 3, 3, 3, 3, 3>(
            new functor(derived));

    Eigen::Matrix<double, 4, 1> tmp_q{tmp_delta_q.w(), tmp_delta_q.x(),
                                      tmp_delta_q.y(), tmp_delta_q.z()};
    const std::vector<const double *> parameters{
        tmp_delta_p.data(),       tmp_q.data(),
        tmp_delta_v.data(),       tmp_linearized_ba.data(),
        tmp_linearized_bg.data(), tmp_acc_0.data(),
        tmp_gyr_0.data(),         tmp_acc_1.data(),
        tmp_gyr_1.data()};
    ...
    std::vector<double *> jacobians{
        de_dp.data(),  de_dq.data(),  de_dv.data(),
        de_dba.data(), de_dbg.data(), de_da0.data(),
        de_dg0.data(), de_da1.data(), de_dg1.data()};

    cost_function->Evaluate(parameters.data(), residuals.data(),
                            jacobians.data());

    delta_p = residuals.segment<3>(0);
    delta_q = Eigen::Quaterniond(residuals(3), residuals(4), residuals(5),
                                 residuals(6));
    ...
    noise.block<3, 3>(0, 0) = noise.block<3, 3>(6, 6) =
        (ACC_N * ACC_N) * Eigen::Matrix3d::Identity() / dt;
    noise.block<3, 3>(3, 3) = noise.block<3, 3>(9, 9) =
        (ACC_N * GYR_N) * Eigen::Matrix3d::Identity() / dt;
    ...
    m_covariance = A * m_covariance * A.transpose() +
                   B * noise * B.transpose() + C * walk * C.transpose();
  ...
  void getJacobianAndCovariance() {
    if (!has_jaco) {
      ...
      L.block<3, 4>(3, 3) =
          2 * Utility::Qleft(delta_q.inverse()).template bottomRows<3>();
      ...
      R.block<4, 3>(3, 3) = 0.5 * Utility::Qleft(Eigen::Quaterniond::Identity())
                                      .template rightCols<3>();
