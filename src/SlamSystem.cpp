...
SlamSystem::SlamSystem(int w, int h, Eigen::Matrix3f K, CALIBRATION_PAR *cali) {
  ...
  //        PoseLocalParameterization(); problem.AddParameterBlock(para_Pose[i],
  //        SIZE_POSE, local_parameterization);
...
void SlamSystem::generateDubugMap(STATE *currentFrame,
...
void SlamSystem::initGravity(const Eigen::Vector3d &gravity_b0) {
  ...
  ROS_INFO_STREAM("origin gravity "
                  << (Rs[0].transpose() * gravity_new).transpose());
  ROS_INFO_STREAM("R0:\n" << Rs[0]);
  ...
  //    ROS_INFO_STREAM("origin gravity " << (Rs[0].transpose() *
  //    g).transpose()); ROS_INFO_STREAM("R0:\n" << Rs[0]);
}
...
void SlamSystem::insertFrame(ros::Time time, cv::Mat img) {
  ...
  states[frame_count]->frame.reset(new lsd_slam::Frame(
      frame_count, width, height, K, time.toSec(), img.data));

  //    Ps[frame_count] = Ps[frame_count-1]
  //            +
  //            Vs[frame_count-1]*imu_factors[frame_count]->pre_integration.sum_dt
  //            - 0.5 * GRAVITY *
  //            SQ(imu_factors[frame_count]->pre_integration.sum_dt)
  ...
  if (IMUorNot) {
    ...
    // std::cout<<"==================== Vs[frame_count]:
    // "<<Vs[frame_count].transpose()<<std::endl;

    Rs[frame_count] =
        Rs[frame_count - 1] * imu_factors[frame_count]->pre_integration.delta_q;
  ...
  //    cout << "imu_factors[frame_count]->sum_dt " <<
  //    imu_factors[frame_count]->pre_integration.sum_dt << "\n" ; cout <<
  //    "imu_factors[frame_count]->delta_p " <<
  //    imu_factors[frame_count]->pre_integration.delta_p  << "\n" ; cout <<
  //    "imu_factors[frame_count]->delta_v "<<
  //    imu_factors[frame_count]->pre_integration.delta_v  << "\n" ; cout <<
  //    "imu_factors[frame_count]->delta_q "<<
  //    imu_factors[frame_count]->pre_integration.delta_q.toRotationMatrix()  <<
  //    "\n" ;
}

void SlamSystem::insertCameraLink(
    STATE *keyFrame, STATE *currentFrame, const Eigen::Matrix3d &R_k_2_c,
    const Eigen::Vector3d &T_k_2_c,
  ...
  ROS_INFO("insert camera link %d %d", keyFrame->state_id,
           currentFrame->state_id);
}
...
void SlamSystem::setReprojectionListRelateToLastestKeyFrameEdge() {
  ...
  for (int i = 0; i < currentKFID; i++) {
    if (states[i]->keyFrameFlag == false || trackFrameCnt > 10) {
      continue;
    }
    ...
    cT_64 = -cR_64 * T_i_2_j;
    for (int ith = optimizedPyramidLevel - 1; ith >= optimizedPyramidLevel - 1;
         ith--) {
      error = dvoLoop.optimizeGaussianNewton(
          pKF, states[i], calib_par, ith, gaussianNewtonTrackingIterationNum,
          cR_64, cT_64, energyAtEachIteration, epsilonVec, reprojections,
    ...
    for (int ith = optimizedPyramidLevel - 1; ith >= optimizedPyramidLevel - 1;
         ith--) {
      error = dvoLoop.optimizeGaussianNewton(
          states[i], pKF, calib_par, ith, gaussianNewtonTrackingIterationNum,
          cR_64, cT_64, energyAtEachIteration, epsilonVec, reprojections,
          bestEnergyIndex, visibleRatio);
    }
    if (visibleRatio < 0.7 || error > trustTrackingErrorThreshold) {
    ...
    Eigen::Vector3d final_T =
        R_i_2_c.transpose() * (r_i_2_j * T_i_2_c + t_i_2_j) -
    ...
    ATA.setIdentity();
    insertCameraLink(states[i], pKF, final_R, final_T, ATA * visualWeight);
    break;
  }
}
...
void SlamSystem::optimizeWithEdgeAlignment(STATE *keyFrame, STATE *currentFrame,
  ...
  double pose[7] = {t_i_2_j(0),  t_i_2_j(1),  t_i_2_j(2), q_i_2_j.x(),
                    q_i_2_j.y(), q_i_2_j.z(), q_i_2_j.w()};
  ...
  for (int i = 0, sz = keyFrame->edge_3d[level].cols(); i < sz; i++) {
    EdgeAlignmentFactor *f = new EdgeAlignmentFactor(
        keyFrame->edge_3d[level](0, i), keyFrame->edge_3d[level](1, i),
        keyFrame->edge_3d[level](2, i),
        currentFrame->distanceTransformEigen[level],
        currentFrame->distanceTransformGradientX[level],
        currentFrame->distanceTransformGradientY[level], cali->fx[level],
        cali->fy[level], cali->cx[level], cali->cy[level], width, height);
  ...
  const int col_offset = currentFrame->distanceTransformEigen[level].cols();
  ...
  cv::Mat host_img = keyFrame->image[level].clone();
  cv::Mat target_img = currentFrame->image[level].clone();
  cv::Mat show;
  cv::hconcat(host_img, target_img, show);
  cv::hconcat(show, target_img, show);
  cv::cvtColor(show, show, cv::COLOR_GRAY2BGR);
  for (int i = 0, sz = currentFrame->edge_2d_cur[level].cols(); i < sz; i++) {
    double u0 = currentFrame->edge_2d_cur[level].col(i)[0];
    double v0 = currentFrame->edge_2d_cur[level].col(i)[1];
    if (level == 0) {
      cv::circle(show, cv::Point(u0 + (double)col_offset * 2.0, v0), 1,
                 cv::Scalar(0, 0, 255), -1);
    }
  }
  int maxLabel = cv::countNonZero(currentFrame->edge[level]);
  for (int i = 0, sz = keyFrame->edge_3d[level].cols(); i < sz; i++) {
    p2 = R_i_2_j * keyFrame->edge_3d[level].col(i).cast<double>() + t_i_2_j;
    double u0 = keyFrame->edge_2d[level].col(i)[0];
    double v0 = keyFrame->edge_2d[level].col(i)[1];
    ...
    if (level == 0 && i % 10 == 0) {
      // std::cout << "labels type: " << currentFrame->labels[level].type() << "
      // (CV_32S=" << CV_32S << ", CV_32F=" << CV_32F << ")" << std::endl;
      // std::cout << "labels size: " << currentFrame->labels[level].rows << "x"
      // << currentFrame->labels[level].cols << std::endl; std::cout << "access
      // pos: (" << (int)v_ << ", " << (int)u_ << ")" << std::endl;
      int label = currentFrame->labels[level].at<int>((int)v_, (int)u_);
      // std::cout << "label: " << label << ", maxLabel: " << maxLabel << ",
      // proj_uv: " << u_ <<", " << v_ << std::endl;
      Eigen::Vector2f nearestPt = currentFrame->label2xy[level][label]
                                      .cast<float>(); // 最近边缘点的像素坐标
      // std::cout << "nearestPt: " << nearestPt[0] << ", " << nearestPt[1] <<
      // std::endl;
      cv::line(show, cv::Point2f(u0, v0),
               cv::Point2f(u_ + (double)col_offset, v_), cv::Scalar(255, 0, 0),
               1);
      cv::circle(show, cv::Point(u0, v0), 2, cv::Scalar(0, 0, 255), -1);
      cv::circle(show, cv::Point(u_ + (double)col_offset, v_), 2,
                 cv::Scalar(0, 255, 0), -1);
      cv::circle(
          show,
          cv::Point(nearestPt[0] + (double)col_offset * 2.0, nearestPt[1]), 3,
          cv::Scalar(0, 255, 255), -1);
      cv::circle(show, cv::Point(u_ + (double)col_offset * 2.0, v_), 2,
                 cv::Scalar(0, 255, 0), -1);
      cv::line(
          show,
          cv::Point2f(nearestPt[0] + (double)col_offset * 2.0, nearestPt[1]),
          cv::Point2f(u_ + (double)col_offset * 2.0, v_), cv::Scalar(255, 0, 0),
          1);
    }
    cnt++;

    total_error += getInterpolatedElementEigen(
        currentFrame->distanceTransformEigen[level], u_, v_);
  }
  ratio = (double)cnt / keyFrame->edge_3d[level].cols();
  aveError = total_error / cnt;
  if (level == 0) {
    cv::imshow("reproj error", show);
    cv::waitKey(1);
  }
}
...
void SlamSystem::setReprojectionListRelateToLastestKeyFrameEdgeCeres() {
  ...
  for (int i = 0; i < currentKFID; i++) {
    ...
    for (int ith = optimizedPyramidLevel - 1; ith >= optimizedPyramidLevel - 1;
         ith--) {
      optimizeWithEdgeAlignment(pKF, states[i], cR_64, cT_64, calib_par, ith,
                                visibleRatio, error);
    }
    if (visibleRatio < 0.7 || error > trustTrackingErrorThreshold) {
      continue;
    ...
    for (int ith = optimizedPyramidLevel - 1; ith >= optimizedPyramidLevel - 1;
         ith--) {
      optimizeWithEdgeAlignment(states[i], pKF, cR_64, cT_64, calib_par, ith,
                                visibleRatio, error);
    }
    if (visibleRatio < 0.7 || error > trustTrackingErrorThreshold) {
      continue;
...
void SlamSystem::setReprojectionListRelateToLastestKeyFrameDense() {
  ...
  for (int i = 0; i < currentKFID; i++) {
    ...
    trackerConstraint->trackFrameOnPermaref(pKF->frame.get(),
                                            states[i]->frame.get(), c2f_init);
    ...
    trackerConstraint->trackFrameOnPermaref(states[i]->frame.get(),
                                            pKF->frame.get(), c2f_init);
    if (trackerConstraint->trackingWasGood == false) {
    ...
    float tracking_lastGoodPerTotal =
        trackerConstraint->lastGoodCount /
        (pKF->frame->width(SE3TRACKING_MIN_LEVEL) *
         pKF->frame->height(SE3TRACKING_MIN_LEVEL));
    Eigen::Vector3d dist =
        RefToFrame.translation() * states[i]->frame->meanIdepth;
    float minVal = 1.0f;
...
void SlamSystem::processIMU(double dt, const Vector3d &linear_acceleration,
  ...
  printf("====== entering processIMU, frame_count: %d, !imu_factors[%d]: %d, "
         "acc_0: [%0.5f, %0.5f, %0.5f] ...... ",
         frame_count, frame_count, !imu_factors[frame_count], acc_0(0),
         acc_0(1), acc_0(2));
  if (!imu_factors[frame_count]) {
    ROS_WARN("processIMU new frame_count = %d", frame_count);
    // std::cout<<"acc_0: "<<acc_0.transpose()<<std::endl;
    // std::cout<<"gyro_0: "<<gyr_0.transpose()<<std::endl;
    // std::cout<<"Bas[frame_count]: "<<Bas[frame_count].transpose()<<std::endl;
    // std::cout<<"Bgs[frame_count]: "<<Bgs[frame_count].transpose()<<std::endl;
    imu_factors[frame_count] =
        new IMUFactor_t{acc_0, gyr_0, Bas[frame_count], Bgs[frame_count]};
  }
  imu_factors[frame_count]->push_back(dt, linear_acceleration,
                                      angular_velocity);
  printf("imu_factors[%d]->parameter_block_sizes().size(): %d\n", frame_count,
         imu_factors[frame_count]->parameter_block_sizes().size());
...
void SlamSystem::para2eigen() {
  ...
  for (int i = 0; i <= frame_count; i++) {
    // Vector3d tmp = Utility::R2ypr(Quaterniond(para_Pose[i][6],
    // para_Pose[i][3], para_Pose[i][4],
    // para_Pose[i][5]).normalized().toRotationMatrix()); ROS_WARN("before R %d
    // = %lf %lf %lf", i, tmp(0), tmp(1), tmp(2) ) ;
...
#endif
  if (states[frame_count - 1]->keyFrameFlag &&
      frame_count == WINDOW_SIZE) { // marginalize old
    for (int i = 0; i < frame_count; i++) {
      Rs[i].swap(Rs[i + 1]);
    ...
    printf("moveData, frame_count: %d, states[frame_count-1]->keyFrameFlag == "
           "false: %d...\n",
           frame_count, states[frame_count - 1]->keyFrameFlag == false);
    ...
    } else // drop
    {
      ...
      for (unsigned int i = 0; i < sz; i++) {
        imu_factors[frame_count - 1]->push_back(
            imu_factors[frame_count]->pre_integration.dt_buf[i],
  ...
  if (states[frame_count - 1]->keyFrameFlag &&
    ...
    { // added visual factor
      for (std::list<CAMERALINK>::iterator iter = states[0]->cameraLink.begin();
        ...
        RelativePoseFactor *f = new RelativePoseFactor(
            iter->R_bi_2_bj, iter->T_bi_2_bj, iter->P_inv);
        ResidualBlockInfo *residual_block_info = new ResidualBlockInfo(
            f, NULL, vector<double *>{para_Pose[0], para_Pose[linkID]},
            vector<int>{0});
        marginalization_factor->addResidualBlockInfo(residual_block_info);
      }
    }
    ...
    vector<double *> parameter_blocks =
        marginalization_factor->getParameterBlocks(addr_shift);
    last_marginalization_factor = marginalization_factor;
    last_marginalization_parameter_blocks = parameter_blocks;
  } else if (states[frame_count - 1]->keyFrameFlag == false
             // && frame_count == WINDOW_SIZE
  ) {
    printf("marging states[frame_count-1]->keyFrameFlag == false: %d... \n",
           states[frame_count - 1]->keyFrameFlag == false);
    ...
    //             )
    {
      ...
      if (frontMarginalization) {
        ...
        if (IMUorNot && imu_factors[frame_count - 1] != nullptr) {
          ResidualBlockInfo *residual_block_info = new ResidualBlockInfo(
              imu_factors[frame_count - 1], NULL,
              vector<double *>{
                  para_Pose[frame_count - 2], para_SpeedBias[frame_count - 2],
        ...
        for (std::list<CAMERALINK>::iterator iter = lastKF->cameraLink.begin();
          ...
          ResidualBlockInfo *residual_block_info = new ResidualBlockInfo(
              f, NULL,
              vector<double *>{para_Pose[lastKF->state_id],
                               para_Pose[frame_count - 1]},
              vector<int>{1});
          marginalization_factor->addResidualBlockInfo(residual_block_info);
...
#define DEBUG_CERES
  ...
  bool print_imu_factors = true; // false;
  //#define DEBUG_CERES
  // init ceres
  while (ceres_frame_count < frame_count) {
    ...
    PoseLocalParameterization *local_parameterization =
        new PoseLocalParameterization();
    problem.AddParameterBlock(para_Pose[ceres_frame_count], SIZE_POSE,
                              local_parameterization);
    if (IMUorNot) {
      ceres::SubsetParameterization *sub_par =
          new ceres::SubsetParameterization(SIZE_SPEEDBIAS,
                                            vector<int>{3, 4, 5, 6, 7, 8});
      problem.AddParameterBlock(para_SpeedBias[ceres_frame_count],
                                SIZE_SPEEDBIAS);
      // problem.AddParameterBlock(para_SpeedBias[ceres_frame_count],
      // SIZE_SPEEDBIAS, sub_par);
    }
...
#ifdef DEBUG_CERES
      double **para =
          new double *[last_marginalization_parameter_blocks.size()];
      for (int i = 0;
           i < static_cast<int>(last_marginalization_parameter_blocks.size());
           i++)
  ...
  if (IMUorNot) { // imu factor
    ...
    for (int i = 0; i < frame_count; i++) {
      ...
      if (imu_factors[j] == nullptr) {
        //  if ( (imu_factors[j] == nullptr)  ||
        //  (imu_factors[j]->parameter_block_sizes().size() != 4)){
        continue;
      }
#if 1
      std::cout << "cost_function->parameter_block_sizes().size(): "
                << imu_factors[j]->parameter_block_sizes().size() << std::endl;
      std::cout << "j: " << j << ", imu_factors[j]: " << imu_factors[j]
                << std::endl;
      std::cout << "i: " << i << ", para_Pose[i]: " << para_Pose[i][0]
                << std::endl;
      // std::cout<<"para_SpeedBias[i]: "<<para_SpeedBias[i][0]<<std::endl;
      // std::cout<<"para_Pose[j]: "<<para_Pose[j][0]<<std::endl;
      // std::cout<<"para_SpeedBias[j]: "<<para_SpeedBias[j][0]<<std::endl;
#endif
      problem.AddResidualBlock(imu_factors[j], NULL, para_Pose[i],
  ...
  if (print_imu_factors) {
    printf("~~~~~ 1\n");
    printf("frame_count: %d \n", frame_count);
    for (int i = 0; i <= WINDOW_SIZE; i++) {
      if (imu_factors[i] != nullptr) {
        printf("----- imu_factors[%d]->parameter_block_sizes().size(): %d \n",
               i, imu_factors[i]->parameter_block_sizes().size());
      }
    }
  }
  ceres::Solver::Summary summary;
  ...
  // printf("residual_set.size(): %d \n",residual_set.size());

#if 1
  IMUFactor_t *imu_factors_bak[WINDOW_SIZE + 1];
  for (int i = 0; i <= WINDOW_SIZE; i++) {

    imu_factors_bak[i] = nullptr;
  }
#endif
#if 1
  // imu_factors_bak = imu_factors;
  for (int i = 0; i <= WINDOW_SIZE; i++) {
    imu_factors_bak[i] = imu_factors[i];
  }
  // IMUFactor_t *imu_factors_bak[WINDOW_SIZE + 1];
  for (int i = 0; i <= frame_count; i++) {
    if (imu_factors[i] != nullptr) {
      memcpy(imu_factors_bak[i], imu_factors[i], sizeof(IMUFactor_t *));
    } else {
      imu_factors_bak[i] = nullptr;
    }
  }
  ...
  if (print_imu_factors) {
    ...
    printf("frame_count: %d \n", frame_count);
    for (int i = 0; i <= WINDOW_SIZE; i++) {
      if (imu_factors_bak[i] != nullptr) {
        printf("----- imu_factors[%d]->parameter_block_sizes().size(): %d \n",
               i, imu_factors_bak[i]->parameter_block_sizes().size());
      } else {
        imu_factors_bak[i] = nullptr;
      }
    }
  ...
  // auto imu_factors_bak = imu_factors;
  // printf("frame_count: %d", frame_count);
  // imu_factors = imu_factors_bak;
  ...
  for (int i = 0; i <= frame_count; i++) {
    ...
    if (imu_factors_bak[i] != nullptr) {
#if 0
            imu_factors[i] = imu_factors_bak[i];
#else
      if (print_imu_factors) {
        printf("before memcpy ----- "
               "imu_factors[%d]->parameter_block_sizes().size(): %d \n",
               i, imu_factors[i]->parameter_block_sizes().size());
        printf("before memcpy ----- "
               "imu_factors[%d]->parameter_block_sizes().size(): %d \n",
               i, imu_factors_bak[i]->parameter_block_sizes().size());
      }
      memcpy(imu_factors[i], imu_factors_bak[i], sizeof(IMUFactor_t *));
      if (print_imu_factors) {
        printf("after memcpy ----- "
               "imu_factors[%d]->parameter_block_sizes().size(): %d \n",
               i, imu_factors[i]->parameter_block_sizes().size());
      ...
      // printf("----- imu_factors[%d]->parameter_block_sizes().size(): %d \n",
      // i,
      //      imu_factors[i]->parameter_block_sizes().size());
    } else {
      imu_factors[i] = nullptr;
    }
  }
...
void SlamSystem::trackFrameEdge(cv::Mat image0, cv::Mat image1,
                                unsigned int frameID, ros::Time imageTimeStamp,
  ...
  if (pKF->edge_3d[0].cols() <
      MINIMUM_3DPOINTSFORTRACKING) { // not enough points for tracking
    ...
    // ROS_WARN("Edge detection time: %f", ((double)cvGetTickCount() - t) /
    // (cvGetTickFrequency() * 1000) );

    //        std::shared_ptr<lsd_slam::Frame> trackingNewFrame(
    //                    new lsd_slam::Frame( frameID, width, height, K,
    //                    imageTimeStamp.toSec(), image0.data ) );

    // ROS_WARN("keyFrame=%d currentFrame=%d\n", currentKeyFrame->edgeNum[0],
    // currentFrame.edgeNum[0] ) ;
    for (int i = 0; i < optimizedPyramidLevel - 1; i++) {
      if (fabs(pKF->edgeNum[i] - currentFrame.edgeNum[i]) /
              double(min(pKF->edgeNum[i], currentFrame.edgeNum[i])) >
          edgeProportion) {
        // ROS_WARN("keyFrame=%d currentFrame=%d", currentKeyFrame->edgeNum[i],
        // currentFrame.edgeNum[i] ) ;
        createNewKeyFrame = true;
      }
    }
    ...
    if (createNewKeyFrame) {
      ...
      // ROS_WARN("distance transform time: %f", ((double)cvGetTickCount() - t)
      // / (cvGetTickFrequency() * 1000) );
      ...
      //                calculateInvDepthImage(disparity, iDepth, iVar, 0.11,
      //                calib_par->fx[0] ); currentFrame.insertDepth(iDepth,
      //                iVar ); currentFrame.initPointsPyramid(calib_par);
      //            }

      // track
      // printf("currentKeyFrame = %d 3d() = %d\n", currentKeyFrame->id(),
      // currentKeyFrame->edge_3d[0].cols() ) ;

      for (int i = optimizedPyramidLevel - 1; i >= 0; i--) {
        optimizeWithEdgeAlignment(pKF, &currentFrame, cR_64, cT_64, calib_par,
                                  i, visibleRatio, bestTotalEpsilon);

        //                bestTotalEpsilon = dvo.optimizeGaussianNewton( pKF,
        //                &currentFrame, calib_par,
        //                                                               i,
        //                                                               gaussianNewtonTrackingIterationNum,
        //                                                               cR_64,
        //                                                               cT_64,
        //                                                               energyAtEachIteration,
        //                                                               epsilonVec,
        //                                                               reprojections,
        //                                                               bestEnergyIndex,
        //                                                               visibleRatio
      ...
      if (visualizeTrackingDebug) {
        // dvo.visualizeEnergyProgress( energyAtEachIteration, bestEnergyIndex,
        // 2 ); Eigen::MatrixXi reprojectedMask = Eigen::MatrixXi::Zero(
        // currentFrame.image[0].rows , currentFrame.image[0].cols );
        ...
        // dvo.visualizeDistanceResidueHeatMap( currentFrame.image[0],
        // reprojectedMask, currentFrame.distanceTransformEigen[0] );
        ...
        cv::normalize(currentFrame.distanceTransformMap[0], displayDT, 0.0, 1.0,
                      cv::NORM_MINMAX);
      ...
      // Keyframe selection
      if (visibleRatio < 0.7 ||
          bestTotalEpsilon > trustTrackingErrorThreshold) {
        createNewKeyFrame = true;
        nTrackFrame = 0;
      } else {
        createNewKeyFrame = false;
...
void SlamSystem::trackFrameDense(cv::Mat image0, unsigned int frameID,
                                 ...
                                 Eigen::Matrix3d deltaR) {
  std::shared_ptr<lsd_slam::Frame> trackingNewFrame(new lsd_slam::Frame(
      frameID, width, height, K, imageTimeStamp.toSec(), image0.data));
  ...
  //    R_k_2_c.transpose(); Vector3d T_bk1_2_b0 =
  //    trackingReference->keyframe->T_bk_2_b0 + R_bk1_2_b0*T_k_2_c ;
