...
//    Rs[frame_count] = Rs[frame_count-1]*imu_factors[frame_count]->R_k1_k ;
    ...
    if ( IMUorNot ){
        ...
        // std::cout<<"==================== Vs[frame_count]: "<<Vs[frame_count].transpose()<<std::endl;
...
{
    printf("====== entering processIMU, frame_count: %d, !imu_factors[%d]: %d, acc_0: [%0.5f, %0.5f, %0.5f] ...... ",frame_count, frame_count, !imu_factors[frame_count], acc_0(0), acc_0(1), acc_0(2));
...
printf("imu_factors[%d]->parameter_block_sizes().size(): %d\n", frame_count, imu_factors[frame_count]->parameter_block_sizes().size());
...
void SlamSystem::moveData()
{
    ...
    {
        printf("moveData, frame_count: %d, states[frame_count-1]->keyFrameFlag == false: %d...\n",frame_count, states[frame_count-1]->keyFrameFlag == false);
    ...
    {//imu factor
        ...
        for (int i = 0; i < frame_count; i++)
        {
            ...
            if ( imu_factors[j] == nullptr ){
              //  if ( (imu_factors[j] == nullptr)  || (imu_factors[j]->parameter_block_sizes().size() != 4)){
                continue ;
            }
#if 1
            std::cout<<"cost_function->parameter_block_sizes().size(): "<<imu_factors[j]->parameter_block_sizes().size()<<std::endl;
            std::cout<<"j: "<<j<<", imu_factors[j]: "<<imu_factors[j]<<std::endl;
            std::cout<<"i: "<<i<<", para_Pose[i]: "<<para_Pose[i][0]<<std::endl;
    ...
    if(print_imu_factors) {
        ...
        for (int i = 0; i <= WINDOW_SIZE; i++) {
            if (imu_factors_bak[i] != nullptr) {
                printf("----- imu_factors[%d]->parameter_block_sizes().size(): %d \n", i,
                       imu_factors_bak[i]->parameter_block_sizes().size());
            }
            else
            {
                imu_factors_bak[i] = nullptr;
    ...
    for (int i = 0; i <= frame_count; i++){
       // for (int i = 0; i <= WINDOW_SIZE; i++){
        //if (true) {
...
#else
            if(print_imu_factors) {
                printf("before memcpy ----- imu_factors[%d]->parameter_block_sizes().size(): %d \n", i,
                       imu_factors[i]->parameter_block_sizes().size());
                printf("before memcpy ----- imu_factors[%d]->parameter_block_sizes().size(): %d \n", i,
                       imu_factors_bak[i]->parameter_block_sizes().size());
            }
            ...
            if(print_imu_factors) {
                printf("after memcpy ----- imu_factors[%d]->parameter_block_sizes().size(): %d \n", i,
                       imu_factors[i]->parameter_block_sizes().size());
                printf("after memcpy ----- imu_factors[%d]->parameter_block_sizes().size(): %d \n", i,
                       imu_factors_bak[i]->parameter_block_sizes().size());
            }
