...
public:
  ...
  RelativePoseFactor(const Eigen::Matrix3d &R, const Eigen::Vector3d &T,
                     const Eigen::Matrix<double, 6, 6> &P_inv) {
    R_k_2_c = R;
    T_k_2_c = T;
    Cov_inv = P_inv;
  }
  ...
  bool Evaluate(double const *const *parameters, double *residuals,
    ...
    // Vector3d r_q = Quaterniond(R_k_2_c.transpose() * Rj.transpose() *
    // Ri).vec() * 2.0 ;
    ...
    Eigen::Matrix<double, 6, 6> sqrt_info =
        Eigen::LLT<Eigen::Matrix<double, 6, 6>>(tmpP_inv).matrixL().transpose();
    ...
    if (jacobians) {
      if (jacobians[0]) {
        ...
        jacobian_pose_i.block<3, 3>(O_R, O_R) =
            Utility::Qleft(q_i_2_j.inverse() * Qj.inverse() * Qi)
      ...
      if (jacobians[1]) {
        ...
        // jacobian_pose_j.block<3, 3>(O_P, O_R) = math.Skew(
        // Rj.transpose()*(Pi-Pj) );
        jacobian_pose_j.block<3, 3>(O_P, O_R) =
            Utility::skewSymmetric(Qj.inverse() * (Pi - Pj));
        // jacobian_pose_j.block<3, 3>(O_R, O_R) = -Ri.transpose()*Rj;
        jacobian_pose_j.block<3, 3>(O_R, O_R) =
            -(Utility::Qleft(Qi.inverse() * Qj) * Utility::Qright(q_i_2_j))
                 .bottomRightCorner<3, 3>();
