...
template <typename DerivedV, typename DerivedF, typename Derivedc,
          typename Derivedvol>
IGL_INLINE void igl::centroid(const Eigen::PlainObjectBase<DerivedV> &V,
  ...
  // loop over faces
  for (int f = 0; f < m; f++) {
    // "Calculating the volume and centroid of a polyhedron in 3d" [Nuernberg
    // 2013] http://www2.imperial.ac.uk/~rn/centroid.pdf rename corners
    ...
    cen.array() += (1. / 24. * n.array() *
                    ((a + b).array().square() + (b + c).array().square() +
                     (c + a).array().square())
                        .array());
  }
  cen *= 1. / (2. * vol);
...
template void igl::centroid<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
