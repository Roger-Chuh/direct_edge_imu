...
// separate the vector set field into N vector fields, where the separation is
// defined by the matchings. The cuts have previously been generated based on
// the field singularities, see
// igl::polyvector_field_cut_mesh_with_singularities. Inputs:
...
//   vectors of the input vector set field
//                    (stacked horizontally for each triangle). Vector #1 in one
//                    face does not necessarily match vector #1 in the adjacent
//                    face.
...
//   need to be cut
// Outputs:
//   sol3D_combed     #F by 3n list of the 3D coordinates of the per-face
//   vectors of the combed vector set field
//                    (stacked horizontally for each triangle). Vector #1 in one
//                    face will match vector #1 in the adjacent face.
//
template <typename DerivedV, typename DerivedF, typename DerivedTT,
          typename DerivedS, typename DerivedM, typename DerivedC>
IGL_INLINE void polyvector_field_comb_from_matchings_and_cuts(
    const Eigen::PlainObjectBase<DerivedV> &V,
    const Eigen::PlainObjectBase<DerivedF> &F,
    const Eigen::PlainObjectBase<DerivedTT> &TT, const Eigen::MatrixXi &E2F,
    const Eigen::MatrixXi &F2E, const Eigen::PlainObjectBase<DerivedS> &sol3D,
