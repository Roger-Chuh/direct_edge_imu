...
IGL_INLINE void igl::quat_to_axis_angle(const Q_type *q, Q_type *axis,
                                        Q_type &angle) {
  if (fabs(q[3]) > (1.0 + igl::EPS<Q_type>())) {
    ...
    if (q[3] >= 1.0f)
      ...
      a = PI; // and keep V
    else if (fabs(q[0] * q[0] + q[1] * q[1] + q[2] * q[2] + q[3] * q[3]) <
             igl::EPS_SQ<Q_type>()) {
      a = 0;
    } else {
      a = acos(q[3]);
  ...
  if (fabs(angle) < igl::EPS<Q_type>() &&
      fabs(axis[0] * axis[0] + axis[1] * axis[1] + axis[2] * axis[2]) <
          igl::EPS_SQ<Q_type>()) {
...
template <typename Q_type>
IGL_INLINE void igl::quat_to_axis_angle_deg(const Q_type *q, Q_type *axis,
                                            Q_type &angle) {
...
template void igl::quat_to_axis_angle_deg<float>(float const *, float *,
                                                 float &);
#endif
