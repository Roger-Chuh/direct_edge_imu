...
namespace igl {
template <typename DerivedV, typename DerivedF> class MissMatchCalculator {
public:
...
private:
  ...
  inline int MissMatchByCross(const int f0, const int f1) {
    ...
    Eigen::Matrix<typename DerivedV::Scalar, 3, 1> dir1Rot =
        igl::rotation_matrix_from_directions(n1, n0) * dir1;
    dir1Rot.normalize();
    ...
    double angle_diff =
        atan2(dir1Rot.dot(PD2.row(f0)), dir1Rot.dot(PD1.row(f0)));

    //    std::cerr << "Dani: " << dir0(0) << " " << dir0(1) << " " << dir0(2)
    //    << " " << dir1Rot(0) << " " << dir1Rot(1) << " " << dir1Rot(2) << " "
    //    << angle_diff << std::endl;
...
public:
  ...
  inline void
  calculateMissmatch(Eigen::PlainObjectBase<DerivedF> &Handle_MMatch) {
...
IGL_INLINE void
igl::cross_field_missmatch(const Eigen::PlainObjectBase<DerivedV> &V,
                           const Eigen::PlainObjectBase<DerivedF> &F,
                           const Eigen::PlainObjectBase<DerivedV> &PD1,
                           const Eigen::PlainObjectBase<DerivedV> &PD2,
                           const bool isCombed,
...
template void igl::cross_field_missmatch<Eigen::Matrix<double, -1, 3, 0, -1, 3>,
                                         Eigen::Matrix<int, -1, 3, 0, -1, 3>>(
    Eigen::PlainObjectBase<Eigen::Matrix<double, -1, 3, 0, -1, 3>> const &,
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, 3, 0, -1, 3>> const &,
    Eigen::PlainObjectBase<Eigen::Matrix<double, -1, 3, 0, -1, 3>> const &,
    Eigen::PlainObjectBase<Eigen::Matrix<double, -1, 3, 0, -1, 3>> const &,
    bool, Eigen::PlainObjectBase<Eigen::Matrix<int, -1, 3, 0, -1, 3>> &);
template void
igl::cross_field_missmatch<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
