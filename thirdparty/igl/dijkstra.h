...
//
// Output:
//   min_distance     #V by 1 list of the minimum distances from source to all
//   vertices previous         #V by 1 list of the previous visited vertices
//   (for each vertex) - used for backtracking
//
template <typename IndexType, typename DerivedD, typename DerivedP>
IGL_INLINE int
dijkstra_compute_paths(const IndexType &source,
...
//
// Inputs:
//   vertex           vertex to which we want the shortest path (from same
//   source as above) previous         #V by 1 list of the previous visited
//   vertices (for each vertex) - result of Dijkstra's algorithm
//
// Output:
//   path             #P by 1 list of vertex indices in the shortest path from
//   source to vertex
//
template <typename IndexType, typename DerivedP>
IGL_INLINE void
dijkstra_get_shortest_path_to(const IndexType &vertex,
