...
template <typename DerivedV, typename DerivedF, typename VFType,
          typename VVType, typename DerivedTT, typename DerivedC,
          typename DerivedS>
IGL_INLINE void igl::polyvector_field_cut_mesh_with_singularities(
  ...
  igl::cut_mesh_from_singularities(
      V, F, Eigen::MatrixXd::Zero(F.rows(), 3).eval(), cuts);
  ...
  for (int i = 0; i < singularities.rows(); ++i) {
    ...
    int vertex_found = igl::dijkstra_compute_paths(
        singularities[i], vertices_in_cut, VV, min_distance, previous);
    if (vertex_found == -1)
    ...
    for (int ii = 0; ii < path.size() - 1; ++ii) {
      ...
      it = std::set_intersection(vf0.begin(), vf0.end(), vf1.begin(), vf1.end(),
                                 common_face_v.begin());
...
IGL_INLINE void igl::polyvector_field_cut_mesh_with_singularities(
  ...
  igl::polyvector_field_cut_mesh_with_singularities(V, F, VF, VV, TT, TTi,
                                                    singularities, cuts);
}

#ifdef IGL_STATIC_LIBRARY
// Explicit template specialization
template void igl::polyvector_field_cut_mesh_with_singularities<
    Eigen::Matrix<double, -1, -1, 0, -1, -1>,
    Eigen::Matrix<int, -1, -1, 0, -1, -1>, int, int,
