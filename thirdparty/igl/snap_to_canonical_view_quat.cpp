...
IGL_INLINE bool igl::snap_to_canonical_view_quat(const Q_type *q,
  ...
  Q_type qn[4];
  bool valid_len = igl::normalize_quat(q, qn);
  ...
  for (double sign = -1; sign <= 1; sign += 2) {
    for (int i = 0; i < NUM_CANONICAL_VIEW_QUAT; i++) {
      ...
      for (int j = 0; j < 4; j++) {
        ...
        distance += (qn[j] - sign * igl::CANONICAL_VIEW_QUAT<Q_type>(i, j)) *
...
IGL_INLINE bool
igl::snap_to_canonical_view_quat(const Eigen::Quaternion<Scalarq> &q,
                                 const double threshold,
                                 Eigen::Quaternion<Scalars> &s) {
  return snap_to_canonical_view_quat<Scalars>(q.coeffs().data(), threshold,
                                              s.coeffs().data());
}
...
template bool igl::snap_to_canonical_view_quat<double>(const double *, double,
                                                       double *);
// generated by autoexplicit.sh
template bool igl::snap_to_canonical_view_quat<float>(const float *, float,
                                                      float *);
template bool igl::snap_to_canonical_view_quat<float, float>(
    Eigen::Quaternion<float, 0> const &, double, Eigen::Quaternion<float, 0> &);
template bool igl::snap_to_canonical_view_quat<double, double>(
    Eigen::Quaternion<double, 0> const &, double,
