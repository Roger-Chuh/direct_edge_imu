...
namespace igl {
template <typename DerivedV, typename DerivedF> class PlanarizerShapeUp {
protected:
  ...
  // vector consisting of the projected face vertices (might be different for
  // the same vertex belonging to different faces)
  Eigen::Matrix<typename DerivedV::Scalar, Eigen::Dynamic, 1> Vv, weightsSqrt,
      P;
  ...
  inline void
  assembleSelector(int fi, Eigen::SparseMatrix<typename DerivedV::Scalar> &S);
...
public:
  ...
  inline PlanarizerShapeUp(const Eigen::PlainObjectBase<DerivedV> &V_,
                           ...
                           const int maxIter_, const double &threshold_);
...
inline igl::PlanarizerShapeUp<DerivedV, DerivedF>::PlanarizerShapeUp(
    const Eigen::PlainObjectBase<DerivedV> &V_,
    const Eigen::PlainObjectBase<DerivedF> &F_, const int maxIter_,
    const double &threshold_)
    : numV(V_.rows()), numF(F_.rows()), Vin(V_), Fin(F_),
      weightsSqrt(
          Eigen::Matrix<typename DerivedV::Scalar, Eigen::Dynamic, 1>::Ones(
              numF, 1)),
      maxIter(maxIter_), threshold(threshold_) {
...
inline void igl::PlanarizerShapeUp<DerivedV, DerivedF>::assembleQ() {
  ...
  for (int fi = 0; fi < numF; fi++) {
    ...
    Eigen::SparseMatrix<typename DerivedV::Scalar> Qi =
        weightsSqrt(fi) * Ni * Sfi;
    ...
    for (int k = 0; k < Qi.outerSize(); ++k)
      for (typename Eigen::SparseMatrix<
               typename DerivedV::Scalar>::InnerIterator it(Qi, k);
           it; ++it) {
        typename DerivedV::Scalar val = it.value();
        int row = it.row();
        int col = it.col();
        tripletList.push_back(Eigen::Triplet<typename DerivedV::Scalar>(
            row + 3 * ni * fi, col, val));
      }
  }
...
inline void igl::PlanarizerShapeUp<DerivedV, DerivedF>::assembleNi() {
  ...
  for (int ii = 0; ii < ni; ii++) {
    ...
    tripletList.push_back(
        Eigen::Triplet<typename DerivedV::Scalar>(3 * ii + 0, 3 * ii + 0, 1.));
    tripletList.push_back(
        Eigen::Triplet<typename DerivedV::Scalar>(3 * ii + 1, 3 * ii + 1, 1.));
    tripletList.push_back(
        Eigen::Triplet<typename DerivedV::Scalar>(3 * ii + 2, 3 * ii + 2, 1.));
  }
  Ni.resize(3 * ni, 3 * ni);
...
template <typename DerivedV, typename DerivedF>
inline void igl::PlanarizerShapeUp<DerivedV, DerivedF>::assembleSelector(
    int fi, Eigen::SparseMatrix<typename DerivedV::Scalar> &S) {

  std::vector<Eigen::Triplet<typename DerivedV::Scalar>> tripletList;
  for (int fvi = 0; fvi < ni; fvi++) {
    int vi = Fin(fi, fvi);
    tripletList.push_back(
        Eigen::Triplet<typename DerivedV::Scalar>(3 * fvi + 0, 3 * vi + 0, 1.));
    tripletList.push_back(
        Eigen::Triplet<typename DerivedV::Scalar>(3 * fvi + 1, 3 * vi + 1, 1.));
    tripletList.push_back(
        Eigen::Triplet<typename DerivedV::Scalar>(3 * fvi + 2, 3 * vi + 2, 1.));
  }
...
inline void igl::PlanarizerShapeUp<DerivedV, DerivedF>::assembleP() {
  ...
  for (int fi = 0; fi < numF; fi++) {
    ...
    Eigen::Matrix<typename DerivedV::Scalar, Eigen::Dynamic, Eigen::Dynamic> CC(
        3, ni);
    for (int i = 0; i < ni; ++i)
    ...
    Eigen::Matrix<typename DerivedV::Scalar, 3, 1> lambda =
        es.eigenvalues().real();
...
template <typename DerivedV, typename DerivedF>
inline void igl::PlanarizerShapeUp<DerivedV, DerivedF>::planarize(
  ...
  for (int iter = 0; iter < maxIter; ++iter) {
    ...
    // std::cerr<<"iter #"<<iter<<": max non-planarity:
    // "<<nonPlanarity<<std::endl;
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE void
igl::planarize_quad_mesh(const Eigen::PlainObjectBase<DerivedV> &Vin,
                         const Eigen::PlainObjectBase<DerivedF> &Fin,
                         const int maxIter, const double &threshold,
                         Eigen::PlainObjectBase<DerivedV> &Vout) {
  PlanarizerShapeUp<DerivedV, DerivedF> planarizer(Vin, Fin, maxIter,
                                                   threshold);
...
template void igl::planarize_quad_mesh<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
