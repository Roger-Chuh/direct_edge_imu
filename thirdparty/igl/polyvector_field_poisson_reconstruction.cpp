...
IGL_INLINE double igl::polyvector_field_poisson_reconstruction(
  ...
  Eigen::SparseMatrix<typename DerivedV::Scalar> Q =
      gradMatrix.transpose() * M * gradMatrix;
  ...
  Eigen::VectorXd vec;
  vec.setZero(3 * numF, 1);
  for (int i = 0; i < half_degree; ++i) {
    vec << sol3D_combed.col(i * 3 + 0), sol3D_combed.col(i * 3 + 1),
        sol3D_combed.col(i * 3 + 2);
    ...
    igl::slice_into(yu, Iu, index, scalars);
    scalars(Ik[0], i) = xk[0];
  }
  ...
  for (int i = 0; i < half_degree; ++i) {
    Eigen::VectorXd diff = (sol3D_recon.block(0, i * 3, numF, 3) -
                            sol3D_combed.block(0, i * 3, numF, 3))
                               .rowwise()
                               .norm();
    diff = diff.array() /
           sol3D_combed.block(0, i * 3, numF, 3).rowwise().norm().array();
...
template double igl::polyvector_field_poisson_reconstruction<
