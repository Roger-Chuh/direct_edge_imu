...
IGL_INLINE void igl::quad_planarity(const Eigen::PlainObjectBase<DerivedV> &V,
  ...
  for (int i = 0; i < nf; ++i) {
    ...
    Eigen::Matrix<typename DerivedV::Scalar, 1, 3> diagCross =
        (v3 - v1).cross(v4 - v2);
    typename Eigen::PlainObjectBase<DerivedV>::Scalar denom =
...
template void igl::quad_planarity<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
