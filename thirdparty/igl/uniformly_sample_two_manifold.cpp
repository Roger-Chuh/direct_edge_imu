...
IGL_INLINE void igl::uniformly_sample_two_manifold(const Eigen::MatrixXd &W,
  ...
  const auto &bary_dist = [](const Eigen::MatrixXd &W, const Eigen::MatrixXi &F,
                             const int face_A, const Eigen::Vector3d &bary_A,
                             const int face_B,
                             const Eigen::Vector3d &bary_B) -> double {
    return ((bary_A(0) * W.row(F(face_A, 0)) + bary_A(1) * W.row(F(face_A, 1)) +
             bary_A(2) * W.row(F(face_A, 2))) -
            (bary_B(0) * W.row(F(face_B, 0)) + bary_B(1) * W.row(F(face_B, 1)) +
             bary_B(2) * W.row(F(face_B, 2))))
        .norm();
  };
  ...
  if (F.cols() == 4) {
    ...
    // Faces from tets
    MatrixXi TF =
        cat(1, cat(1, cat(2, T0, cat(2, T1, T2)), cat(2, T0, cat(2, T2, T3))),
            cat(1, cat(2, T0, cat(2, T3, T1)), cat(2, T1, cat(2, T3, T2))));
  ...
  vector<vector<int>> sample_faces;
  sample_faces.resize(k);
  // List of list of barycentric coordinates, for each sample gives b-coords in
  // face its on
  vector<vector<Eigen::Vector3d>> sample_barys;
  sample_barys.resize(k);
  // List of current maxmins amongst samples
  vector<int> cur_maxmin;
  cur_maxmin.resize(k);
  ...
  while (radius > min_radius) {
    ...
    for (int i = 0; i < k; i++) {
      ...
      int index_in_face = (bary(0) > bary(1) ? (bary(0) > bary(2) ? 0 : 2)
                                             : (bary(1) > bary(2) ? 1 : 2));
      ...
      VectorXd seed = bary(0) * W.row(F(face_i, 0)) +
      ...
      for (int s = 0; s < (int)sample_faces[i].size(); s++) {
        ...
        for (int j = 0; j < k; j++) {
          ...
          if (i == j) {
            ...
            d = bary_dist(W, F, sample_face, sample_bary, seed_j_face,
                          seed_j_bary);
          }
          D[i](s, j) = d;
        }
        ...
        for (int j = 0; j < W.cols(); j++) {
          ...
          double d = ((sample_bary(0) * W.row(F(sample_face, 0)) +
  ...
  for (int i = 0; i < k; i++) {
    int face_i = sample_faces[i][cur_maxmin[i]];
    Eigen::Vector3d bary(sample_barys[i][cur_maxmin[i]]);
    WS.row(i) = bary(0) * W.row(F(face_i, 0)) + bary(1) * W.row(F(face_i, 1)) +
                bary(2) * W.row(F(face_i, 2));
  }
  verbose("Lap: %g\n", get_seconds() - start);
...
IGL_INLINE void
igl::uniformly_sample_two_manifold_at_vertices(const Eigen::MatrixXd &OW,
                                               const int k, const double push,
                                               Eigen::VectorXi &S) {
  using namespace Eigen;
  using namespace std;
