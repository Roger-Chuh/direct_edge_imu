...
// defined if no early exit is supported, i.e., always take a fixed number of
// iterations
...
IGL_INLINE bool
igl::arap_dof_precomputation(const Eigen::MatrixXd &V, const Eigen::MatrixXi &F,
  ...
  cout << endl
       << "];" << endl
       << "Lapl=sparse(LaplIJV(:,1),LaplIJV(:,2),LaplIJV(:,3)," << Lapl.rows()
       << "," << Lapl.cols() << ");" << endl;
#endif
  ...
  cout << endl
       << "];" << endl
       << "G_sum=sparse(G_sumIJV(:,1),G_sumIJV(:,2),G_sumIJV(:,3),"
  ...
  cout << endl
       << "];" << endl
       << "CSM=sparse(CSMIJV(:,1),CSMIJV(:,2),CSMIJV(:,3)," << CSM.rows() << ","
       << CSM.cols() << ");" << endl;
#endif
  ...
  for (int i = 0; i < data.dim; i++) {
    ...
    for (int j = 0; j < data.dim; j++) {
      ...
      // printf("CSM_M(): slice\n");
      slice(CSM, colon<int>(j * k, (j + 1) * k - 1),
            colon<int>(j * n, (j + 1) * n - 1), CSMj);
      assert(CSMj.rows() == k);
      assert(CSMj.cols() == n);
      ...
      if (is_sparse(CSMjM_i)) {
        ...
        //        printf("CSM_M[%d].block(%d*%d=%d,0,%d,%d): %d
        ...
        //        printf("CSM_MjM_ifull: %d
        //        %d\n",i,CSMjM_ifull.rows(),CSMjM_ifull.cols());
        data.CSM_M[i].block(j * k, 0, CSMjM_i.rows(), CSMjM_i.cols()) =
            CSMjM_ifull;
      } else {
  ...
  massmatrix(
      V, F,
      (F.cols() > 3 ? MASSMATRIX_TYPE_BARYCENTRIC : MASSMATRIX_TYPE_VORONOI),
      Mass);
...
static typename MatrixXS::Scalar
condense_CSM(const std::vector<MatrixXS> &CSM_M_SSCALAR, int numBones, int dim,
             MatrixXS &CSM) {
  ...
  for (int r = 0; r < numRows; r++) {
    for (int coord = 0; coord < dim + 1; coord++) {
      for (int b = 0; b < numBones; b++) {
        ...
        for (int v = 0; v < 3; v++) {
          for (int w = 0; w < 3; w++) {
            blok(v, w) = CSM_M_SSCALAR[v](r, coord * (numBones * dim) + b +
                                                 w * numBones);
          }
        }
...
// splits x_0, ... , x_dim coordinates in column vector 'L' into a
// numBones*(dimp1) x dim matrix 'Lsep'; assumes 'Lsep' has already been
// preallocated
//
// is this the same as uncolumnize? no.
template <typename MatL, typename MatLsep>
static void splitColumns(const MatL &L, int numBones, int dim, int dimp1,
                         MatLsep &Lsep) {
  assert(L.cols() == 1);
  ...
  for (int b = 0; b < numBones; b++) {
    for (int coord = 0; coord < dimp1; coord++) {
      for (int c = 0; c < dim; c++) {
        Lsep(coord * numBones + b, c) =
            L(coord * numBones * dim + c * numBones + b, 0);
      }
    }
  }
}

// the inverse of splitColumns, i.e., takes numBones*(dimp1) x dim matrix 'Lsep'
// and merges the dimensions into columns vector 'L' (which is assumed to be
// already allocated):
//
// is this the same as columnize? no.
template <typename MatrixXS>
static void mergeColumns(const MatrixXS &Lsep, int numBones, int dim, int dimp1,
                         MatrixXS &L) {
  assert(L.cols() == 1);
  assert(L.rows() == dim * (dimp1)*numBones);
...
template <typename MatrixXS>
static typename MatrixXS::Scalar condense_Solve1(MatrixXS &Solve1, int numBones,
                                                 int numGroups, int dim,
                                                 MatrixXS &CSolve1) {
      ...
      for (int colCoord = 0; colCoord < dim; colCoord++) {
        for (int g = 0; g < numGroups; g++) {
          ...
          for (int r = 0; r < 3; r++) {
            for (int c = 0; c < 3; c++) {
              blok(r, c) =
                  Solve1(rowCoord * numBones * dim + r * numBones + b,
                         colCoord * numGroups * dim + c * numGroups + g);
            }
          }
          ...
          CSolve1(rowCoord * numBones + b, colCoord * numGroups + g) =
              blok(0, 0);
        }
      }
    }
...
IGL_INLINE bool igl::arap_dof_recomputation(
  ...
  // to be there :)
  data.M_FullSolve << (-0.5 * M_Solve.block(0, 0, fsRows, fsRows) * data.M_KG)
                          .template cast<SSCALAR>(),
  ...
  MatrixXS SolveBlock1 = data.M_FullSolve.block(0, 0, data.M_FullSolve.rows(),
                                                data.dim * data.dim * k);
  SSCALAR maxErr2 =
      condense_Solve1(SolveBlock1, data.m, k, data.dim, data.CSolveBlock1);
...
template <typename LbsMatrixType, typename SSCALAR>
IGL_INLINE bool
igl::arap_dof_update(const ArapDOFData<LbsMatrixType, SSCALAR> &data,
                     const Eigen::Matrix<double, Eigen::Dynamic, 1> &B_eq,
                     const Eigen::MatrixXd &L0, const int max_iters,
                     const double
  ...
  // timing variables
  double sec_start, sec_covGather, sec_fitRotations,
      // sec_rhs,
      sec_prepMult, sec_solve, sec_end;
  ...
  MatrixXS L_prev;
#endif
  // We will be iterating on L_SSCALAR, only at the end we convert back to
  // double
  ...
  const MatrixXS L_part2 =
      data.M_FullSolve.block(0, Rcol.rows(), data.M_FullSolve.rows(),
                             B_eq_SSCALAR.rows()) *
      B_eq_SSCALAR;
  const MatrixXS L_part3 =
      data.M_FullSolve.block(0, Rcol.rows() + B_eq_SSCALAR.rows(),
                             data.M_FullSolve.rows(), B_eq_fix_SSCALAR.rows()) *
      B_eq_fix_SSCALAR;
...
#endif
  {
    ...
    if (data.with_dynamics) {
      ...
      MatrixXS temp = -1.0 * ((-1.0 / (data.h * data.h)) * data.L0.array() +
      ...
      MatrixXd temp2 =
          data.Mass_tilde * temp_d + temp_g + data.fext.template cast<double>();
    ...
    assert(L_SSCALAR.rows() == L_part1.rows() &&
           L_SSCALAR.rows() == L_part2and3.rows());
  ...
  printf("ARAP preparation = %f, "
         "all %i iterations = %f [ms]\n",
         (timer_prepFinished - timer_start) * 1000.0, max_iters,
...
template bool
igl::arap_dof_update<Eigen::Matrix<double, -1, -1, 0, -1, -1>, double>(
    ArapDOFData<Eigen::Matrix<double, -1, -1, 0, -1, -1>, double> const &,
    Eigen::Matrix<double, -1, 1, 0, -1, 1> const &,
    Eigen::Matrix<double, -1, -1, 0, -1, -1> const &, int, double,
    Eigen::Matrix<double, -1, -1, 0, -1, -1> &);
template bool
igl::arap_dof_recomputation<Eigen::Matrix<double, -1, -1, 0, -1, -1>, double>(
    Eigen::Matrix<int, -1, 1, 0, -1, 1> const &,
    Eigen::SparseMatrix<double, 0, int> const &,
    ArapDOFData<Eigen::Matrix<double, -1, -1, 0, -1, -1>, double> &);
template bool
igl::arap_dof_precomputation<Eigen::Matrix<double, -1, -1, 0, -1, -1>, double>(
    Eigen::Matrix<double, -1, -1, 0, -1, -1> const &,
    Eigen::Matrix<int, -1, -1, 0, -1, -1> const &,
    Eigen::Matrix<double, -1, -1, 0, -1, -1> const &,
    Eigen::Matrix<int, -1, 1, 0, -1, 1> const &,
    ArapDOFData<Eigen::Matrix<double, -1, -1, 0, -1, -1>, double> &);
template bool
igl::arap_dof_update<Eigen::Matrix<double, -1, -1, 0, -1, -1>, float>(
    igl::ArapDOFData<Eigen::Matrix<double, -1, -1, 0, -1, -1>, float> const &,
    Eigen::Matrix<double, -1, 1, 0, -1, 1> const &,
    Eigen::Matrix<double, -1, -1, 0, -1, -1> const &, int, double,
    Eigen::Matrix<double, -1, -1, 0, -1, -1> &);
template bool
igl::arap_dof_recomputation<Eigen::Matrix<double, -1, -1, 0, -1, -1>, float>(
    Eigen::Matrix<int, -1, 1, 0, -1, 1> const &,
    Eigen::SparseMatrix<double, 0, int> const &,
    igl::ArapDOFData<Eigen::Matrix<double, -1, -1, 0, -1, -1>, float> &);
template bool
igl::arap_dof_precomputation<Eigen::Matrix<double, -1, -1, 0, -1, -1>, float>(
    Eigen::Matrix<double, -1, -1, 0, -1, -1> const &,
    Eigen::Matrix<int, -1, -1, 0, -1, -1> const &,
    Eigen::Matrix<double, -1, -1, 0, -1, -1> const &,
    Eigen::Matrix<int, -1, 1, 0, -1, 1> const &,
