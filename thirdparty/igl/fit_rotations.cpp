...
IGL_INLINE void igl::fit_rotations(const Eigen::PlainObjectBase<DerivedS> &S,
  ...
  // MatrixXd si(dim,dim);
  Eigen::Matrix<typename DerivedS::Scalar, 3, 3>
      si; // = Eigen::Matrix3d::Identity();
...
template <typename DerivedS, typename DerivedD>
IGL_INLINE void
igl::fit_rotations_planar(const Eigen::PlainObjectBase<DerivedS> &S,
                          Eigen::PlainObjectBase<DerivedD> &R) {
  using namespace std;
  const int dim = S.cols();
  const int nr = S.rows() / dim;
...
#ifdef __SSE__
IGL_INLINE void igl::fit_rotations_SSE(const Eigen::MatrixXf &S,
                                       Eigen::MatrixXf &R) {
  const int cStep = 4;
  ...
  for (; r < nr; r += cStep) {
    ...
    // Not sure why polar_dec computes transpose...
    for (int k = 0; k < numMats; k++) {
      R.block(0, (r + k) * dim, dim, dim) =
          ri.block(3 * k, 0, dim, dim).transpose();
    }
  }
}

IGL_INLINE void igl::fit_rotations_SSE(const Eigen::MatrixXd &S,
                                       Eigen::MatrixXd &R) {
...
#ifdef __AVX__
IGL_INLINE void igl::fit_rotations_AVX(const Eigen::MatrixXf &S,
                                       Eigen::MatrixXf &R) {
  const int cStep = 8;
...
template void igl::fit_rotations<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
                                 Eigen::Matrix<double, -1, -1, 0, -1, -1>>(
    Eigen::PlainObjectBase<Eigen::Matrix<double, -1, -1, 0, -1, -1>> const &,
    bool, Eigen::PlainObjectBase<Eigen::Matrix<double, -1, -1, 0, -1, -1>> &);
template void
igl::fit_rotations_planar<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
                          Eigen::Matrix<double, -1, -1, 0, -1, -1>>(
    Eigen::PlainObjectBase<Eigen::Matrix<double, -1, -1, 0, -1, -1>> const &,
    Eigen::PlainObjectBase<Eigen::Matrix<double, -1, -1, 0, -1, -1>> &);
template void
igl::fit_rotations_planar<Eigen::Matrix<float, -1, -1, 0, -1, -1>,
                          Eigen::Matrix<float, -1, -1, 0, -1, -1>>(
    Eigen::PlainObjectBase<Eigen::Matrix<float, -1, -1, 0, -1, -1>> const &,
    Eigen::PlainObjectBase<Eigen::Matrix<float, -1, -1, 0, -1, -1>> &);
template void igl::fit_rotations<Eigen::Matrix<float, -1, -1, 0, -1, -1>,
                                 Eigen::Matrix<float, -1, -1, 0, -1, -1>>(
    Eigen::PlainObjectBase<Eigen::Matrix<float, -1, -1, 0, -1, -1>> const &,
    bool, Eigen::PlainObjectBase<Eigen::Matrix<float, -1, -1, 0, -1, -1>> &);
#endif
