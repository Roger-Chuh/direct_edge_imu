...
namespace igl {
// HalfEdgeIterator - Fake halfedge for fast and easy navigation on triangle
// meshes with vertex_triangle_adjacency and triangle_triangle adjacency
template <typename DerivedF> class HalfEdgeIterator {
public:
  // Init the HalfEdgeIterator by specifying Face,Edge Index and Orientation
  IGL_INLINE HalfEdgeIterator(const Eigen::PlainObjectBase<DerivedF> &_F,
  ...
  // Change Vertex
  IGL_INLINE void flipV() { reverse = !reverse; }

  IGL_INLINE bool isBorder() { return (FF)(fi, ei) == -1; }
   ...
   * v, producing the sequence a, b, c, d, a, b, c, ...
   */
  ...
  // Get face index
  IGL_INLINE int Fi() { return fi; }

  // Get edge index
  IGL_INLINE int Ei() { return ei; }

  IGL_INLINE bool operator==(HalfEdgeIterator &p2) {
    return ((fi == p2.fi) && (ei == p2.ei) && (reverse == p2.reverse) &&
            (F == p2.F) && (FF == p2.FF) && (FFi == p2.FFi));
  }
