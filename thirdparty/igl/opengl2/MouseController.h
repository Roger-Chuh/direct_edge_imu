...
public:
  ...
  static inline void propogate_to_descendants_if(const VectorXb &S,
  ...
  static inline void color_if(const VectorXb &S,
...
private:
  ...
  typedef std::vector<Eigen::Quaterniond,
      ...
      RotationList;
  RotationList m_rotations, m_rotations_at_selection,
...
public:
  ...
  inline void set_selection_from_last_drag(const Eigen::MatrixXd &C,
                                           const Eigen::MatrixXi &BE,
                                           const Eigen::VectorXi &P,
                                           const Eigen::VectorXi &RP);
  // Set from explicit selection
  inline void set_selection(const Eigen::VectorXi &S, const Eigen::MatrixXd &C,
                            const Eigen::MatrixXi &BE, const Eigen::VectorXi &P,
                            const Eigen::VectorXi &RP);
...
inline void igl::opengl2::MouseController::propogate_to_descendants_if(
    const VectorXb &S, const Eigen::VectorXi &P, VectorXb &T) {
  using namespace std;
  const int n = S.rows();
  assert(P.rows() == n);
  // dynamic programming
...
inline void igl::opengl2::MouseController::color_if(
    const VectorXb &S, const Eigen::Vector4f &selected_color,
    const Eigen::Vector4f &unselected_color, Eigen::MatrixXf &C) {
  C.resize(S.rows(), 4);
...
inline igl::opengl2::MouseController::MouseController()
    : m_is_selecting(false), m_selection(), m_down_x(-1), m_down_y(-1),
      m_drag_x(-1), m_drag_y(-1), m_width(-1), m_height(-1), m_widget(),
      m_widget_rot_at_selection(), m_rotations(), m_rotations_at_selection(),
      m_root_enabled(true) {}
  ...
  if (m_is_selecting) {
    ...
    for (int e = 0; e < m_selection.size(); e++) {
      if (m_selection(e)) {
        ...
        m_rotations[e] =
            m_rotations_at_selection[e] * frame.conjugate() *
            (m_widget.rot * m_widget_rot_at_selection.conjugate()) * frame;
      }
    }
    return true;
  }
...
inline void igl::opengl2::MouseController::draw() const {
  ...
  if (m_is_selecting) {
    ...
    draw_rectangular_marquee(m_down_x, m_height - m_down_y, m_drag_x,
                             m_height - m_drag_y);
...
inline void igl::opengl2::MouseController::set_selection_from_last_drag(
    ...
    const Eigen::VectorXi &P, const Eigen::VectorXi &RP) {
  using namespace Eigen;
  using namespace std;
  using namespace igl;
  ...
  for (int e = 0; e < BE.rows(); e++) {
    ...
    m_selection(e) = line_segment_in_rectangle(
        projs.head(2), projd.head(2), Vector2d(m_down_x, m_height - m_down_y),
...
inline void igl::opengl2::MouseController::set_selection(
    const Eigen::VectorXi &S, const Eigen::MatrixXd &C,
    const Eigen::MatrixXi &BE, const Eigen::VectorXi &P,
    const Eigen::VectorXi &RP) {
  using namespace igl;
  using namespace Eigen;
...
inline bool
igl::opengl2::MouseController::set_rotations(const RotationList &vQ) {
