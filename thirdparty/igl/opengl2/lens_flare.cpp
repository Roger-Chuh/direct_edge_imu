...
IGL_INLINE void
igl::opengl2::lens_flare_load_textures(std::vector<GLuint> &shine_id,
                                       std::vector<GLuint> &flare_id) {

  const auto setup_texture = [](const uint8_t *texture, const int width,
                                const int height, GLuint texobj,
  ...
  for (int i = 0; i < (int)shine_id.size(); i++) {
    setup_texture(SHINE_TEXTURES[i], SHINE_TEXTURE_WIDTHS[i],
                  SHINE_TEXTURE_HEIGHTS[i], shine_id[i], GL_LINEAR, GL_LINEAR);
  }
  flare_id.resize(6);
  glGenTextures(6, &flare_id[0]);
  for (int i = 0; i < (int)flare_id.size(); i++) {
    setup_texture(FLARE_TEXTURES[i], FLARE_TEXTURE_WIDTHS[i],
                  FLARE_TEXTURE_HEIGHTS[i], flare_id[i], GL_LINEAR, GL_LINEAR);
  }
}

IGL_INLINE void
igl::opengl2::lens_flare_create(const float *A, const float *B, const float *C,
...
IGL_INLINE void igl::opengl2::lens_flare_draw(
    ...
    const std::vector<GLuint> &shine_ids, const std::vector<GLuint> &flare_ids,
    const Eigen::Vector3f &light, const float near_clip, int &shine_tic) {
  ...
  Vector3f center =
      unproject(Vector3f(0.5 * vp[2], 0.5 * vp[3], plight[2] - 1e-3));
