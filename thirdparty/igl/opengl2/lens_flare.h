...
struct Flare {
  ...
  float color[3];
  Flare() : type(-1), scale(0), loc(0) {}
  Flare(int type, float location, float scale, const float color[3],
        float colorScale)
      : type(type), scale(scale), loc(location) {
...
//
// Inputs:
//   start_id   starting texture id location (should have at least id:id+16
//   free)
// Outputs:
//   shine  list of texture ids for shines
//   flare  list of texture ids for flares
IGL_INLINE void lens_flare_load_textures(std::vector<GLuint> &shine_ids,
...
IGL_INLINE void lens_flare_create(const float *A, const float *B,
                                  const float *C, std::vector<Flare> &flares);
...
// Outputs:
//   shine_tic  current "tic" in shine textures
IGL_INLINE void lens_flare_draw(const std::vector<Flare> &flares,
                                const std::vector<GLuint> &shine_ids,
                                const std::vector<GLuint> &flare_ids,
                                const Eigen::Vector3f &light,
                                const float near_clip, int &shine_tic);
} // namespace opengl2
}; // namespace igl
