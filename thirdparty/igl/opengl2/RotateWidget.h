...
public:
  ...
  inline bool intersect(const int x, const int y, Eigen::Vector3d &hit) const;
...
inline void igl::opengl2::RotateWidget::glColor4fv(const bool a,
...
inline Eigen::Quaterniond igl::opengl2::RotateWidget::axis_q(const int a) {
  ...
  const Eigen::Quaterniond axes[3] = {
      Eigen::Quaterniond(
          Eigen::AngleAxisd(igl::PI * 0.5, Eigen::Vector3d(0, 1, 0))),
      Eigen::Quaterniond(
          Eigen::AngleAxisd(igl::PI * 0.5, Eigen::Vector3d(1, 0, 0))),
      Eigen::Quaterniond::Identity()};
  return axes[a];
}

inline Eigen::Vector3d igl::opengl2::RotateWidget::view_direction(const int x,
                                                                  const int y) {
...
igl::opengl2::RotateWidget::view_direction(const Eigen::Vector3d &pos) {
  using namespace Eigen;
...
inline igl::opengl2::RotateWidget::RotateWidget()
    : pos(0, 0, 0), rot(Eigen::Quaterniond::Identity()), down_rot(rot),
      down_xy(-1, -1), drag_xy(-1, -1), outer_radius_on_screen(91.),
      outer_over_inner(1.13684210526), m_is_enabled(true),
      down_type(DOWN_TYPE_NONE), selected_type(DOWN_TYPE_NONE) {}

inline Eigen::Vector3d
igl::opengl2::RotateWidget::unproject_onto(const int x, const int y) const {
  using namespace Eigen;
  ...
  return (uxy - pos) / unprojected_inner_radius() * outer_over_inner *
         outer_over_inner;
}

inline bool igl::opengl2::RotateWidget::intersect(const int x, const int y,
  ...
  double t0, t1;
  if (!ray_sphere_intersect(uxy, view, pos, unprojected_inner_radius(), t0,
                            t1)) {
    return false;
  }
  hit = uxy + t0 * view;
...
inline bool igl::opengl2::RotateWidget::down(const int x, const int y) {
  ...
  if (fabs(r - outer_radius_on_screen) < thresh) {
    ...
    auto on_meridian = [&](const Vector3d &hit, const Quaterniond &rot,
                           const Quaterniond &m, Vector3d &pl_hit) -> bool {
      ...
      pl_hit += pos;
      return (project(pl_hit).head(2) - project(hit).head(2)).norm() <
             2 * thresh;
    };
    ...
    for (int a = 0; a < 3; a++) {
      ...
      if (on_meridian(hit, rot, Quaterniond(axis_q(a)), pl_hit)) {
        ...
        {
          Vector3d dir3 =
              axis_q(a).conjugate() * down_rot.conjugate() * (hit - pos);
...
inline bool igl::opengl2::RotateWidget::drag(const int x, const int y) {
  ...
  case DOWN_TYPE_OUTLINE: {
    ...
    const double dtheta =
        atan2(A(0) * B(1) - A(1) * B(0), A(0) * B(0) + A(1) * B(1));
  ...
  case DOWN_TYPE_TRACKBALL: {
    ...
    const int h = vp(3);
    Quaterniond dq;
    trackball(r, r, 1, Quaterniond::Identity(),
              double(down_xy(0) - ppos(0)) + r / 2.,
              double((h - down_xy(1)) - (h - ppos(1))) + r / 2.,
              double(x - ppos(0)) + r / 2.,
              double((h - y) - (h - ppos(1))) + r / 2., dq);
...
inline void igl::opengl2::RotateWidget::draw_guide() const {
  ...
  glPushAttrib(GL_DEPTH_BUFFER_BIT | GL_ENABLE_BIT | GL_POLYGON_BIT |
               GL_POINT_BIT | GL_TRANSFORM_BIT | GL_STENCIL_BUFFER_BIT |
               GL_LIGHTING_BIT);
  ...
  {
    const Vector3d nudown(udown.normalized()), nudrag(udrag.normalized());
    glPushMatrix();
