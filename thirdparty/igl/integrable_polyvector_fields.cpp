...
IGL_INLINE igl::integrable_polyvector_fields_parameters::
    integrable_polyvector_fields_parameters()
    : numIter(5), wBarrier(0.1), sBarrier(0.9), wCurl(10), wQuotCurl(10),
      wSmooth(1.), wCloseUnconstrained(1e-3), wCloseConstrained(100),
      redFactor_wsmooth(.8), gamma(0.1), tikh_gamma(1e-8) {}
...
private:
  ...
  IGL_INLINE void
  rj_barrier_face(const Eigen::RowVectorXd &vec2D_a, const double &s,
                  Eigen::VectorXd &residuals, bool do_jac = false,
                  Eigen::MatrixXd &J = *(Eigen::MatrixXd *)NULL);
  IGL_INLINE void rj_polycurl_edge(
      const Eigen::RowVectorXd &vec2D_a, const Eigen::RowVector2d &ea,
      const Eigen::RowVectorXd &vec2D_b, const Eigen::RowVector2d &eb,
      Eigen::VectorXd &residuals, bool do_jac = false,
      Eigen::MatrixXd &Jac = *(Eigen::MatrixXd *)NULL);
  IGL_INLINE void rj_quotcurl_edge_polyversion(
      const Eigen::RowVectorXd &vec2D_a, const Eigen::RowVector2d &ea,
      const Eigen::RowVectorXd &vec2D_b, const Eigen::RowVector2d &eb,
      Eigen::VectorXd &residuals, bool do_jac = false,
      Eigen::MatrixXd &Jac = *(Eigen::MatrixXd *)NULL);
  IGL_INLINE void rj_smoothness_edge(
      const Eigen::RowVectorXd &vec2D_a, const Eigen::RowVectorXd &vec2D_b,
      const double &k, const int nA, const int nB, Eigen::VectorXd &residuals,
      bool do_jac = false, Eigen::MatrixXd &Jac = *(Eigen::MatrixXd *)NULL);

public:
  IGL_INLINE IntegrableFieldSolver(
      IntegrableFieldSolverData<DerivedV, DerivedF, DerivedFF, DerivedC>
          &cffsoldata);
  ...
  solveGaussNewton(integrable_polyvector_fields_parameters &params,
                   const Eigen::VectorXd &x_initial, Eigen::VectorXd &x);

  // Compute residuals and Jacobian for Gauss Newton
  IGL_INLINE double RJ(const Eigen::VectorXd &x, const Eigen::VectorXd &x0,
  ...
  IGL_INLINE void RJ_Barrier(const Eigen::MatrixXd &sol2D, const double &s,
                             const double &wBarrierSqrt,
                             const int startRowInJacobian, bool doJacs = false,
                             const int startIndexInVectors = 0);
  IGL_INLINE void RJ_Closeness(const Eigen::MatrixXd &sol2D,
                               ...
                               bool doJacs = false,
                               const int startIndexInVectors = 0);
  IGL_INLINE void RJ_Curl(const Eigen::MatrixXd &sol2D, const double &wCASqrt,
                          const double &wCBSqrt, const int startRowInJacobian,
                          bool doJacs = false,
                          const int startIndexInVectors = 0);
...
template <typename DerivedV, typename DerivedF, typename DerivedFF,
          typename DerivedC>
IGL_INLINE
igl::IntegrableFieldSolverData<DerivedV, DerivedF, DerivedFF,
                               DerivedC>::IntegrableFieldSolverData() {}

template <typename DerivedV, typename DerivedF, typename DerivedFF,
          typename DerivedC>
IGL_INLINE void
igl::IntegrableFieldSolverData<DerivedV, DerivedF, DerivedFF, DerivedC>::
...
IGL_INLINE void
igl::IntegrableFieldSolverData<DerivedV, DerivedF, DerivedFF,
                               DerivedC>::makeFieldCCW(Eigen::MatrixXd &sol3D) {
  // sort ccw
  Eigen::RowVectorXd t;
  Eigen::RowVectorXd all(1, 2 * 2 * 3);
  Eigen::VectorXi order;
  for (int fi = 0; fi < numF; ++fi) {
    // take all 4 vectors (including opposites) and pick two that are in ccw
    // order
    all << sol3D.row(fi), -sol3D.row(fi);
    Eigen::VectorXi inv_order_unused;
    igl::sort_vectors_ccw(all, FN.row(fi).eval(), order, true, t, false,
                          inv_order_unused);
    // if we are in a constrained face, we need to make sure that the first
    // vector is always the same vector as in the constraints
    if (is_constrained_face[fi]) {
      const Eigen::RowVector3d &constraint =
          constrained_vec3.block(indInConstrained[fi], 0, 1, 3);
      ;
      int best_i = -1;
...
IGL_INLINE void
igl::IntegrableFieldSolverData<DerivedV, DerivedF, DerivedFF, DerivedC>::
    add_Jacobian_to_svector(const int &toplace, const Eigen::MatrixXd &tJac,
...
IGL_INLINE void
igl::IntegrableFieldSolverData<DerivedV, DerivedF, DerivedFF, DerivedC>::
    add_jac_indices_face(const int numInnerRows, const int numInnerCols,
                         const int startRowInJacobian,
                         const int startIndexInVectors, Eigen::VectorXi &Rows,
  ...
  for (int fi = 0; fi < numF; ++fi) {
    ...
    face_Jacobian_indices(startRow, startIndex, fi, 2, numInnerRows,
                          numInnerCols, Rows, Columns);
  }
}
...
IGL_INLINE void
igl::IntegrableFieldSolverData<DerivedV, DerivedF, DerivedFF, DerivedC>::
    face_Jacobian_indices(const int &startRow, const int &toplace,
                          const int &fi, const int &half_degree,
                          const int &numInnerRows, const int &numInnerCols,
                          Eigen::VectorXi &rows, Eigen::VectorXi &columns) {
  int ind = toplace;
  for (int j = 0; j < numInnerRows; ++j) {
    for (int k = 0; k < numInnerCols; ++k, ++ind) {
...
IGL_INLINE void
igl::IntegrableFieldSolverData<DerivedV, DerivedF, DerivedFF, DerivedC>::
    add_jac_indices_edge(const int numInnerRows, const int numInnerCols,
                         const int startRowInJacobian,
                         const int startIndexInVectors, Eigen::VectorXi &Rows,
                         Eigen::VectorXi &Columns) {
  for (int ii = 0; ii < numInteriorEdges; ++ii) {
    ...
    edge_Jacobian_indices(startRow, startIndex, a, b, 2, numInnerRows,
                          numInnerCols, Rows, Columns);
  }
}
...
IGL_INLINE void
igl::IntegrableFieldSolverData<DerivedV, DerivedF, DerivedFF, DerivedC>::
    edge_Jacobian_indices(const int &startRow, const int &toplace, const int &a,
                          const int &b, const int &half_degree,
                          const int &numInnerRows, const int &numInnerCols,
                          Eigen::VectorXi &rows, Eigen::VectorXi &columns) {
  int ind = toplace;
  for (int j = 0; j < numInnerRows; ++j) {
    for (int k = 0; k < numInnerCols; ++k, ++ind) {
      ...
      int iv = (k % (2 * half_degree)) /
               2;      // which vector (0..half_degree-1) am i at
      int ixy = k % 2; // which part (real/imag) am i at
      rows(ind) = startRow + j;
...
IGL_INLINE void
igl::IntegrableFieldSolverData<DerivedV, DerivedF, DerivedFF,
                               DerivedC>::computeJacobianPattern() {
  ...
  num_residuals = num_residuals_smooth + num_residuals_close +
                  num_residuals_polycurl + num_residuals_quotcurl +
                  num_residuals_barrier;

  residuals.setZero(num_residuals, 1);

  // per edge
  numJacElements_smooth =
      numInteriorEdges * numInnerJacCols_edge * numInnerJacRows_smooth;
  numJacElements_polycurl =
      numInteriorEdges * numInnerJacCols_edge * numInnerJacRows_polycurl;
  numJacElements_quotcurl =
      numInteriorEdges * numInnerJacCols_edge * numInnerJacRows_quotcurl;

  // per face
  numJacElements_barrier =
      numF * numInnerJacCols_face * numInnerJacRows_barrier;
  numJacElements_close = numF * numInnerJacCols_face * numInnerJacRows_close;

  numJacElements = (numJacElements_smooth + numJacElements_polycurl +
                    numJacElements_quotcurl) +
                   (numJacElements_barrier + numJacElements_close);
  // allocate
  ...
  // smoothness
  add_jac_indices_edge(numInnerJacRows_smooth, numInnerJacCols_edge,
                       startRowInJacobian, startIndexInVectors, II_Jac, JJ_Jac);
  startRowInJacobian += num_residuals_smooth;
  startIndexInVectors += numJacElements_smooth;

  // closeness
  add_jac_indices_face(numInnerJacRows_close, numInnerJacCols_face,
                       startRowInJacobian, startIndexInVectors, II_Jac, JJ_Jac);
  startRowInJacobian += num_residuals_close;
  startIndexInVectors += numJacElements_close;

  // barrier
  add_jac_indices_face(numInnerJacRows_barrier, numInnerJacCols_face,
                       startRowInJacobian, startIndexInVectors, II_Jac, JJ_Jac);
  startRowInJacobian += num_residuals_barrier;
  startIndexInVectors += numJacElements_barrier;

  // polycurl
  add_jac_indices_edge(numInnerJacRows_polycurl, numInnerJacCols_edge,
                       startRowInJacobian, startIndexInVectors, II_Jac, JJ_Jac);
  startRowInJacobian += num_residuals_polycurl;
  startIndexInVectors += numJacElements_polycurl;

  // quotcurl
  add_jac_indices_edge(numInnerJacRows_quotcurl, numInnerJacCols_edge,
                       startRowInJacobian, startIndexInVectors, II_Jac, JJ_Jac);
...
IGL_INLINE void
igl::IntegrableFieldSolverData<DerivedV, DerivedF, DerivedFF,
                               DerivedC>::computeHessianPattern() {
  ...
  for (int ii = 0; ii < II_Jac.rows(); ++ii) {
    ...
    for (int jj = starti; jj < II_Jac.rows(); ++jj) {
      ...
      Hess_triplets.push_back(Eigen::Triplet<double>(JJ_Jac(ii), JJ_Jac(jj),
                                                     SS_Jac(ii) * SS_Jac(jj)));
    }
  }
...
IGL_INLINE void
igl::IntegrableFieldSolverData<DerivedV, DerivedF, DerivedFF,
                               DerivedC>::computeNewHessValues() {
  for (int i = 0; i < Hess_triplets.size(); ++i)
    Hess_triplets[i] = Eigen::Triplet<double>(
        Hess_triplets[i].row(), Hess_triplets[i].col(),
...
template <typename DerivedV, typename DerivedF, typename DerivedFF,
          typename DerivedC>
IGL_INLINE igl::IntegrableFieldSolver<DerivedV, DerivedF, DerivedFF, DerivedC>::
    IntegrableFieldSolver(
        IntegrableFieldSolverData<DerivedV, DerivedF, DerivedFF, DerivedC>
            &cffsoldata)
    : data(cffsoldata){};
...
template <typename DerivedV, typename DerivedF, typename DerivedFF,
          typename DerivedC>
IGL_INLINE bool
igl::IntegrableFieldSolver<DerivedV, DerivedF, DerivedFF, DerivedC>::solve(
...
template <typename DerivedV, typename DerivedF, typename DerivedFF,
          typename DerivedC>
IGL_INLINE void
igl::IntegrableFieldSolver<DerivedV, DerivedF, DerivedFF, DerivedC>::
    solveGaussNewton(integrable_polyvector_fields_parameters &params,
                     const Eigen::VectorXd &x_initial, Eigen::VectorXd &x) {
  ...
  for (int innerIter = 0; innerIter < params.numIter; ++innerIter) {
    ...
    if ((data.residuals.array() == std::numeric_limits<double>::infinity())
            .any()) {
      std::cerr << "IntegrableFieldSolver -- residuals: got infinity somewhere"
                << std::endl;
      exit(1);
    };
...
template <typename DerivedV, typename DerivedF, typename DerivedFF,
          typename DerivedC>
IGL_INLINE double
igl::IntegrableFieldSolver<DerivedV, DerivedF, DerivedFF, DerivedC>::RJ(
    const Eigen::VectorXd &x, const Eigen::VectorXd &x0,
    const integrable_polyvector_fields_parameters &params, bool doJacs) {
  ...
  // smoothness
  RJ_Smoothness(sol2D, sqrt(params.wSmooth), startRowInJacobian, doJacs,
                startIndexInVectors);
  startRowInJacobian += data.num_residuals_smooth;
  startIndexInVectors += data.numJacElements_smooth;

  // closeness
  RJ_Closeness(sol2D, sol02D, sqrt(params.wCloseUnconstrained),
               sqrt(params.wCloseConstrained), startRowInJacobian, doJacs,
               startIndexInVectors);
  startRowInJacobian += data.num_residuals_close;
  startIndexInVectors += data.numJacElements_close;

  // barrier
  RJ_Barrier(sol2D, params.sBarrier, sqrt(params.wBarrier), startRowInJacobian,
             doJacs, startIndexInVectors);
  startRowInJacobian += data.num_residuals_barrier;
  startIndexInVectors += data.numJacElements_barrier;

  // polycurl
  RJ_Curl(sol2D, params.wCurl, powl(params.wCurl, 2), startRowInJacobian,
          doJacs, startIndexInVectors);
  startRowInJacobian += data.num_residuals_polycurl;
  startIndexInVectors += data.numJacElements_polycurl;

  // quotcurl
  RJ_QuotCurl(sol2D, sqrt(params.wQuotCurl), startRowInJacobian, doJacs,
              startIndexInVectors);
...
igl::IntegrableFieldSolver<DerivedV, DerivedF, DerivedFF, DerivedC>::
    rj_smoothness_edge(const Eigen::RowVectorXd &vec2D_a,
                       const Eigen::RowVectorXd &vec2D_b, const double &k,
                       const int nA, const int nB, Eigen::VectorXd &residuals,
                       bool do_jac, Eigen::MatrixXd &Jac) {
  ...
  residuals << cA * (t10 + t9) - sA * (t13)-t12 - t11,
  ...
  if (do_jac) {
    ...
    Jac.resize(4, 8);
    Jac << 2 * xua * cA - 2 * yua * sA, -2 * yua * cA - 2 * xua * sA,
        2 * xva * cA - 2 * yva * sA, -2 * yva * cA - 2 * xva * sA, -2 * xub,
        2 * yub, -2 * xvb, 2 * yvb, 2 * yua * cA + 2 * xua * sA,
        2 * xua * cA - 2 * yua * sA, 2 * yva * cA + 2 * xva * sA,
        2 * xva * cA - 2 * yva * sA, -2 * yub, -2 * xub, -2 * yvb, -2 * xvb,
        cB * (t21)-sB * (t20), -cB * (t20)-sB * (t21), cB * (t23)-sB * (t22),
        -cB * (t22)-sB * (t23), 4 * xvb * t4 - 2 * xub * t11,
        2 * yub * t11 + 4 * t3 * yvb, 4 * xub * t4 - 2 * xvb * t12,
        2 * yvb * t12 + 4 * t3 * yub, cB * (t20) + sB * (t21),
        cB * (t21)-sB * (t20), cB * (t22) + sB * (t23), cB * (t23)-sB * (t22),
        -2 * yub * t11 - 4 * t3 * yvb, 4 * xvb * t4 - 2 * xub * t11,
        -2 * yvb * t12 - 4 * t3 * yub, 4 * xub * t4 - 2 * xvb * t12;
  }
}
...
IGL_INLINE void
igl::IntegrableFieldSolver<DerivedV, DerivedF, DerivedFF, DerivedC>::
    RJ_Smoothness(const Eigen::MatrixXd &sol2D, const double &wSmoothSqrt,
                  const int startRowInJacobian, bool doJacs,
  ...
  for (int ii = 0; ii < data.numInteriorEdges; ++ii) {
    ...
    Eigen::MatrixXd tJac;
    Eigen::VectorXd tRes;
    rj_smoothness_edge(sol2D.row(a), sol2D.row(b), data.K[k],
    ...
    data.residuals.segment(startRow, data.numInnerJacRows_smooth) =
        wSmoothSqrt * tRes;

    if (doJacs) {
      int startIndex = startIndexInVectors + data.numInnerJacRows_smooth *
                                                 data.numInnerJacCols_edge * ii;
...
igl::IntegrableFieldSolver<DerivedV, DerivedF, DerivedFF, DerivedC>::
    rj_barrier_face(const Eigen::RowVectorXd &vec2D_a, const double &s,
                    Eigen::VectorXd &residuals, bool do_jac,
                    Eigen::MatrixXd &Jac) {
  ...
  if (t05 >= s)
    ...
    residuals << 1 / ((3 * t00 - 3 * t01) / s - (3 * t05_2) / s_2 +
                      t05_3 / s_3) -
                     1;
    double t03 = (s - t05) * (s - t05);
    double t06 = (3 * s_2 - 3 * s * t00 + 3 * s * t01 + xua_2 * yva_2 -
                  2 * xua * t01 * yva + xva_2 * yua_2);
    double t04 = t06 * t06;
    if (do_jac)
      Jac << -(3 * s_3 * yva * t03) / (t05_2 * t04),
...
template <typename DerivedV, typename DerivedF, typename DerivedFF,
          typename DerivedC>
IGL_INLINE void
igl::IntegrableFieldSolver<DerivedV, DerivedF, DerivedFF, DerivedC>::RJ_Barrier(
    const Eigen::MatrixXd &sol2D, const double &s, const double &wBarrierSqrt,
    const int startRowInJacobian, bool doJacs, const int startIndexInVectors) {
  if (wBarrierSqrt == 0)
    return;
  ...
  for (int fi = 0; fi < data.numF; ++fi) {
    Eigen::MatrixXd tJac;
    Eigen::VectorXd tRes;
    rj_barrier_face(sol2D.row(fi), s, tRes, doJacs, tJac);

    int startRow = startRowInJacobian + data.numInnerJacRows_barrier * fi;
    data.residuals.segment(startRow, data.numInnerJacRows_barrier) =
        wBarrierSqrt * tRes;

    if (doJacs) {
      int startIndex = startIndexInVectors + data.numInnerJacRows_barrier *
                                                 data.numInnerJacCols_face * fi;
      data.add_Jacobian_to_svector(startIndex, wBarrierSqrt * tJac,
                                   data.SS_Jac);
    }
  }
}
...
IGL_INLINE void
igl::IntegrableFieldSolver<DerivedV, DerivedF, DerivedFF, DerivedC>::
    RJ_Closeness(const Eigen::MatrixXd &sol2D, const Eigen::MatrixXd &sol02D,
                 const double &wCloseUnconstrainedSqrt,
                 const double &wCloseConstrainedSqrt,
                 const int startRowInJacobian, bool doJacs,
                 const int startIndexInVectors) {
  ...
  for (int fi = 0; fi < data.numF; ++fi) {
    ...
    if (!data.is_constrained_face[fi])
      ...
      if (data.is_constrained_face[fi] == 1)
        ...
        weights << wCloseConstrainedSqrt, wCloseConstrainedSqrt,
            wCloseUnconstrainedSqrt, wCloseUnconstrainedSqrt;
      else
    ...
    tRes.resize(4, 1);
    tRes << (sol2D.row(fi) - sol02D.row(fi)).transpose();
    int startRow = startRowInJacobian + data.numInnerJacRows_close * fi;
    data.residuals.segment(startRow, data.numInnerJacRows_close) =
        weights.array() * tRes.array();

    if (doJacs) {
      int startIndex = startIndexInVectors + data.numInnerJacRows_close *
                                                 data.numInnerJacCols_face * fi;
      data.add_Jacobian_to_svector(startIndex, weights.asDiagonal() * tJac,
                                   data.SS_Jac);
    }
  }
}
...
igl::IntegrableFieldSolver<DerivedV, DerivedF, DerivedFF, DerivedC>::
    rj_polycurl_edge(const Eigen::RowVectorXd &vec2D_a,
                     ...
                     const Eigen::RowVector2d &eb, Eigen::VectorXd &residuals,
                     bool do_jac, Eigen::MatrixXd &Jac) {
  const Eigen::RowVector2d &ua = vec2D_a.segment(0, 2);
  const Eigen::RowVector2d &va = vec2D_a.segment(2, 2);
  const Eigen::RowVector2d &ub = vec2D_b.segment(0, 2);
  ...
  residuals << dua_2 - dub_2 + dva_2 - dvb_2, dua_2 * dva_2 - dub_2 * dvb_2;

  if (do_jac) {

    Jac.resize(2, 8);
    Jac << 2 * xea * dua, 2 * yea * dua, 2 * xea * dva, 2 * yea * dva,
        -2 * xeb * dub, -2 * yeb * dub, -2 * xeb * dvb, -2 * yeb * dvb,
        2 * xea * dua * dva_2, 2 * yea * dua * dva_2, 2 * xea * dua_2 * dva,
        2 * yea * dua_2 * dva, -2 * xeb * dub * dvb_2, -2 * yeb * dub * dvb_2,
        -2 * xeb * dub_2 * dvb, -2 * yeb * dub_2 * dvb;
  }
}
...
template <typename DerivedV, typename DerivedF, typename DerivedFF,
          typename DerivedC>
IGL_INLINE void
igl::IntegrableFieldSolver<DerivedV, DerivedF, DerivedFF, DerivedC>::RJ_Curl(
    const Eigen::MatrixXd &sol2D, const double &wCASqrt, const double &wCBSqrt,
    const int startRowInJacobian, bool doJacs, const int startIndexInVectors) {
  ...
  for (int ii = 0; ii < data.numInteriorEdges; ++ii) {
    ...
    Eigen::RowVector2d ea;
    ea << xea, yea;
    double xeb = data.B1.row(b).dot(ce);
    double yeb = data.B2.row(b).dot(ce);
    Eigen::RowVector2d eb;
    eb << xeb, yeb;

    Eigen::MatrixXd tJac;
    Eigen::VectorXd tRes;
    rj_polycurl_edge(sol2D.row(a), ea, sol2D.row(b), eb, tRes, doJacs, tJac);
    ...
    if (doJacs) {
      ...
      int startIndex = startIndexInVectors + data.numInnerJacRows_polycurl *
                                                 data.numInnerJacCols_edge * ii;
...
igl::IntegrableFieldSolver<DerivedV, DerivedF, DerivedFF, DerivedC>::
  ...
  if (do_jac) {
    Jac.resize(1, 8);
    Jac << xea * dva * t00 - 2 * xea * dua * dub * dvb,
        yea * dva * t00 - 2 * yea * dua * dub * dvb,
        xea * dua * t00 + 2 * xea * dub * dva * dvb,
        yea * dua * t00 + 2 * yea * dub * dva * dvb,
        2 * xeb * dua * dub * dva - xeb * dvb * t01,
        2 * yeb * dua * dub * dva - yeb * dvb * t01,
        -xeb * dub * t01 - 2 * xeb * dua * dva * dvb,
        -yeb * dub * t01 - 2 * yeb * dua * dva * dvb;
  }
}
...
IGL_INLINE void
igl::IntegrableFieldSolver<DerivedV, DerivedF, DerivedFF, DerivedC>::
    RJ_QuotCurl(const Eigen::MatrixXd &sol2D, const double &wQuotCurlSqrt,
                const int startRowInJacobian, bool doJacs,
                const int startIndexInVectors) {
  for (int ii = 0; ii < data.numInteriorEdges; ++ii) {
    // the two faces of the flap
    ...
    Eigen::MatrixXd tJac;
    Eigen::VectorXd tRes;
    rj_quotcurl_edge_polyversion(sol2D.row(a), ea, sol2D.row(b), eb, tRes,
                                 doJacs, tJac);

    int startRow = startRowInJacobian + data.numInnerJacRows_quotcurl * ii;
    data.residuals.segment(startRow, data.numInnerJacRows_quotcurl) =
        wQuotCurlSqrt * tRes;

    if (doJacs) {
      int startIndex = startIndexInVectors + data.numInnerJacRows_quotcurl *
                                                 data.numInnerJacCols_edge * ii;
      data.add_Jacobian_to_svector(startIndex, wQuotCurlSqrt * tJac,
                                   data.SS_Jac);
    }
  }
}
...
IGL_INLINE void igl::integrable_polyvector_fields_precompute(
    ...
    const Eigen::PlainObjectBase<DerivedF> &F, const Eigen::VectorXi &b,
    const Eigen::PlainObjectBase<DerivedC> &bc,
    const Eigen::VectorXi &constraint_level,
    const Eigen::PlainObjectBase<DerivedFF> &original_field,
    igl::IntegrableFieldSolverData<DerivedV, DerivedF, DerivedFF, DerivedC>
...
template <typename DerivedV, typename DerivedF, typename DerivedFF,
          typename DerivedC>
IGL_INLINE void igl::integrable_polyvector_fields_solve(
    igl::IntegrableFieldSolverData<DerivedV, DerivedF, DerivedFF, DerivedC>
        &cffsoldata,
    integrable_polyvector_fields_parameters &params,
    Eigen::PlainObjectBase<DerivedFF> &current_field,
    bool current_field_is_not_ccw) {
  igl::IntegrableFieldSolver<DerivedV, DerivedF, DerivedFF, DerivedC> cffs(
      cffsoldata);
...
template igl::IntegrableFieldSolverData<
    Eigen::Matrix<double, -1, -1, 0, -1, -1>,
    Eigen::Matrix<int, -1, -1, 0, -1, -1>,
    Eigen::Matrix<double, -1, -1, 0, -1, -1>,
    Eigen::Matrix<double, -1, -1, 0, -1, -1>>::IntegrableFieldSolverData();
template void igl::integrable_polyvector_fields_solve<
    Eigen::Matrix<double, -1, -1, 0, -1, -1>,
    Eigen::Matrix<int, -1, -1, 0, -1, -1>,
    Eigen::Matrix<double, -1, -1, 0, -1, -1>,
    Eigen::Matrix<double, -1, -1, 0, -1, -1>>(
    igl::IntegrableFieldSolverData<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
                                   Eigen::Matrix<int, -1, -1, 0, -1, -1>,
                                   Eigen::Matrix<double, -1, -1, 0, -1, -1>,
                                   Eigen::Matrix<double, -1, -1, 0, -1, -1>> &,
    igl::integrable_polyvector_fields_parameters &,
    Eigen::PlainObjectBase<Eigen::Matrix<double, -1, -1, 0, -1, -1>> &, bool);
