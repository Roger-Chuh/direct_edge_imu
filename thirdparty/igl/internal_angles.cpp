...
IGL_INLINE void igl::internal_angles(const Eigen::PlainObjectBase<DerivedV> &V,
  ...
  if (F.cols() == 3) {
    // Edge lengths
    Matrix<typename DerivedV::Scalar, DerivedF::RowsAtCompileTime,
    ...
    auto corner = [](const Eigen::PlainObjectBase<DerivedV> &x,
    ...
    for (unsigned i = 0; i < F.rows(); ++i) {
      for (unsigned j = 0; j < F.cols(); ++j) {
        K(i, j) =
            corner(V.row(F(i, int(j - 1 + F.cols()) % F.cols())),
                   V.row(F(i, j)), V.row(F(i, (j + 1 + F.cols()) % F.cols())));
      }
    }
  }
}
...
IGL_INLINE void igl::internal_angles(const Eigen::PlainObjectBase<DerivedL> &L,
                                     Eigen::PlainObjectBase<DerivedK> &K) {
...
template void igl::internal_angles<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
...
template void
igl::internal_angles<Eigen::Matrix<double, -1, 3, 1, -1, 3>,
    ...
    Eigen::PlainObjectBase<Eigen::Matrix<unsigned int, -1, -1, 1, -1, -1>> const
        &,
    Eigen::PlainObjectBase<Eigen::Matrix<double, -1, -1, 0, -1, -1>> &);
template void igl::internal_angles<Eigen::Matrix<double, -1, 3, 0, -1, 3>,
