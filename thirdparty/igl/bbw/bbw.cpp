...
igl::bbw::BBWData::BBWData()
    : partition_unity(false), W0(),
#ifndef IGL_NO_MOSEK
      mosek_data(),
#endif
      active_set_params(), qp_solver(QP_SOLVER_IGL_ACTIVE_SET), verbosity(0) {
...
template <typename DerivedV, typename DerivedEle, typename Derivedb,
          typename Derivedbc, typename DerivedW>
IGL_INLINE bool igl::bbw::bbw(const Eigen::PlainObjectBase<DerivedV> &V,
...
#pragma omp parallel for
      for (int i = 0; i < m; i++) {
        ...
        if (data.verbosity >= 1) {
#pragma omp critical
          cout << "BBW: Computing weight for handle " << i + 1 << " out of "
               << m << "." << endl;
        }
        ...
        // use initial guess
        Wi = W.col(i);
        SolverStatus ret = active_set(Q, c, b, bci, Aeq, Beq, Aieq, Bieq, lx,
                                      ux, eff_params, Wi);
        switch (ret) {
    ...
    case QP_SOLVER_MOSEK: {
#ifdef IGL_NO_MOSEK
      assert(false &&
             "Use another QPSolver. Recompile without IGL_NO_MOSEK defined.");
      cerr << "Use another QPSolver. Recompile without IGL_NO_MOSEK defined."
           << endl;
      return false;
#else
      // Loop over handles
      for (int i = 0; i < m; i++) {
        ...
        slice_into(bci, b, lx);
        bool r =
            mosek_quadprog(Q, c, 0, A, lc, uc, lx, ux, data.mosek_data, Wi);
        if (!r) {
          return false;
        }
        W.col(i) = Wi;
...
template bool igl::bbw::bbw<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
