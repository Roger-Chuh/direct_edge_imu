...
static IGL_INLINE void push_result(
    const Eigen::PlainObjectBase<DerivedF> &F, const int f,
    const CGAL::Object &result,
    std::map<typename DerivedF::Index,
             std::pair<typename DerivedF::Index, std::vector<CGAL::Object>>>
        &offending,
    std::map<std::pair<typename DerivedF::Index, typename DerivedF::Index>,
...
template <typename Kernel, typename DerivedVA, typename DerivedFA,
          typename DerivedVB, typename DerivedFB, typename DerivedIF,
          typename DerivedVVA, typename DerivedFFA, typename DerivedJA,
          typename DerivedIMA, typename DerivedVVB, typename DerivedFFB,
          typename DerivedJB, typename DerivedIMB>
static IGL_INLINE bool
intersect_other_helper(const Eigen::PlainObjectBase<DerivedVA> &VA,
  ...
  typedef CGAL::Box_intersection_d::Box_with_handle_d<double, 3,
                                                      TrianglesIterator>
      Box;
  typedef std::map<Index, std::pair<Index, std::vector<CGAL::Object>>>
      OffendingMap;
  ...
  try {
    CGAL::box_intersection_d(A_boxes.begin(), A_boxes.end(), B_boxes.begin(),
                             B_boxes.end(), cb);
  } catch (int e) {
  ...
  {
    int i = 0;
    for (list<int>::const_iterator ifit = lIF.begin(); ifit != lIF.end();) {
      IF(i, 0) = (*ifit);
      ifit++;
      IF(i, 1) = (*ifit);
      ifit++;
  ...
  if (!params.detect_only) {
    remesh_intersections(VA, FA, TA, offendingA, edge2facesA, VVA, FFA, JA,
                         IMA);
    remesh_intersections(VB, FB, TB, offendingB, edge2facesB, VVB, FFB, JB,
                         IMB);
  }
...
template <typename DerivedVA, typename DerivedFA, typename DerivedVB,
          typename DerivedFB, typename DerivedIF, typename DerivedVVA,
          typename DerivedFFA, typename DerivedJA, typename DerivedIMA,
          typename DerivedVVB, typename DerivedFFB, typename DerivedJB,
          typename DerivedIMB>
IGL_INLINE bool
igl::cgal::intersect_other(const Eigen::PlainObjectBase<DerivedVA> &VA,
                           const Eigen::PlainObjectBase<DerivedFA> &FA,
                           const Eigen::PlainObjectBase<DerivedVB> &VB,
                           const Eigen::PlainObjectBase<DerivedFB> &FB,
                           const RemeshSelfIntersectionsParam &params,
                           ...
                           Eigen::PlainObjectBase<DerivedIMB> &IMB) {
  if (params.detect_only) {
    return intersect_other_helper<CGAL::Epick>(VA, FA, VB, FB, params, IF, VVA,
                                               FFA, JA, IMA, VVB, FFB, JB, IMB);
  } else {
    return intersect_other_helper<CGAL::Epeck>(VA, FA, VB, FB, params, IF, VVA,
                                               FFA, JA, IMA, VVB, FFB, JB, IMB);
  }
}

IGL_INLINE bool
igl::cgal::intersect_other(const Eigen::MatrixXd &VA, const Eigen::MatrixXi &FA,
                           const Eigen::MatrixXd &VB, const Eigen::MatrixXi &FB,
                           const bool first_only, Eigen::MatrixXi &IF) {
  Eigen::MatrixXd VVA, VVB;
  Eigen::MatrixXi FFA, FFB;
  Eigen::VectorXi JA, JB, IMA, IMB;
  return intersect_other(VA, FA, VB, FB, {true, first_only}, IF, VVA, FFA, JA,
                         IMA, VVB, FFB, JB, IMB);
}
