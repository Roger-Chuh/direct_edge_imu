...
template <typename DerivedV, typename DerivedF, typename DerivedG,
          typename DerivedJ, typename Derivedflip>
IGL_INLINE void
igl::cgal::outer_hull(const Eigen::PlainObjectBase<DerivedV> &V,
  ...
  typedef Matrix<typename DerivedV::Scalar, Dynamic,
                 DerivedV::ColsAtCompileTime>
      MatrixXV;
  // typedef Matrix<typename
  // DerivedF::Scalar,Dynamic,DerivedF::ColsAtCompileTime> MatrixXF;
  typedef Matrix<typename DerivedG::Scalar, Dynamic,
                 DerivedG::ColsAtCompileTime>
      MatrixXG;
  typedef Matrix<typename DerivedJ::Scalar, Dynamic,
                 DerivedJ::ColsAtCompileTime>
      MatrixXJ;
  ...
  for (Index id = 0; id < (Index)ncc; id++) {
    ...
    while (!Q.empty()) {
      ...
      for (size_t i = 0; i < val; i++) {
        ...
        std::cout << i << ": "
                  << " (e: " << uE2E[EMAP(e)][i] << ", f: "
                  << uE2E[EMAP(e)][i] % m * (uE2C[EMAP(e)][i] ? 1 : -1) << ")"
                  << std::endl;
      }
#endif
      ...
      for (size_t step = 1; step < val + 2; step++) {
        const int nfei_new =
            (diIM(e) + 2 * val + e_cons * step * (flip(f) ? -1 : 1)) % val;
        const int nf = uE2E[EMAP(e)][nfei_new] % m;
        {
          ...
          if (!FH[nf]) {
            nfei = nfei_new;
            //} else {
            //    std::cout << "skipping face " << nfei_new << " because it is
            //    seen before"
            //        << std::endl;
          }
          break;
          ...
          //    std::cout << di[EMAP(e)][diIM(nfei_new)].transpose() <<
          //    std::endl; std::cout << "skipping face " << nfei_new << " with
          //    identical dihedral angle"
          //        << std::endl;
        }
        //#ifdef IGL_OUTER_HULL_DEBUG
        //        cout<<"Skipping co-planar facet: "<<(f+1)<<" -->
        //        "<<(nf+1)<<endl;
        //#endif
      }
  ...
  const auto &has_overlapping_bbox =
      [](const Eigen::PlainObjectBase<DerivedV> &V, const MatrixXV &BC,
         const MatrixXG &A, const MatrixXJ &AJ, const MatrixXG &B) -> bool {
    const auto &bounding_box = [](const Eigen::PlainObjectBase<DerivedV> &V,
                                  const MatrixXG &F) -> DerivedV {
      DerivedV BB(2, 3);
      BB << 1e26, 1e26, 1e26, -1e26, -1e26, -1e26;
  ...
  for (size_t id = 0; id < ncc; id++) {
    ...
    for (size_t i = 0; i < num_unresolved_components; i++) {
      ...
      query_points(i, 0) =
          (V(f(0, 0), 0) + V(f(0, 1), 0) + V(f(0, 2), 0)) / 3.0;
      query_points(i, 1) =
          (V(f(0, 0), 1) + V(f(0, 1), 1) + V(f(0, 2), 1)) / 3.0;
      query_points(i, 2) =
          (V(f(0, 0), 2) + V(f(0, 1), 2) + V(f(0, 2), 2)) / 3.0;
    }
...
template void igl::cgal::outer_hull<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
