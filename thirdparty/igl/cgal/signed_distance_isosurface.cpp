...
IGL_INLINE bool igl::cgal::signed_distance_isosurface(
    const Eigen::MatrixXd &IV, const Eigen::MatrixXi &IF, const double level,
    const double angle_bound, const double radius_bound,
    const double distance_bound, const SignedDistanceType sign_type,
  ...
  case SIGNED_DISTANCE_TYPE_PSEUDONORMAL:
    ...
    per_edge_normals(IV, IF, PER_EDGE_NORMALS_WEIGHTING_TYPE_UNIFORM, FN, EN, E,
                     EMAP);
    break;
  }
  ...
  default:
    assert(false && "Unknown SignedDistanceType");
  case SIGNED_DISTANCE_TYPE_UNSIGNED:
    fun = [&tree, &IV, &IF, &level](const Point_3 &q) -> FT {
      int i;
      RowVector3d c;
      const double sd =
          tree.squared_distance(IV, IF, RowVector3d(q.x(), q.y(), q.z()), i, c);
      return sd - level;
    };
  case SIGNED_DISTANCE_TYPE_DEFAULT:
  case SIGNED_DISTANCE_TYPE_WINDING_NUMBER:
    fun = [&tree, &IV, &IF, &hier, &level](const Point_3 &q) -> FT {
    ...
    break;
  case SIGNED_DISTANCE_TYPE_PSEUDONORMAL:
    fun = [&tree, &IV, &IF, &FN, &VN, &EN, &EMAP,
           &level](const Point_3 &q) -> FT {
      const double sd = igl::signed_distance_pseudonormal(
  ...
  CGAL::Surface_mesh_default_criteria_3<Tr> criteria(angle_bound, radius_bound,
                                                     distance_bound);
  // meshing surface
