...
IGL_INLINE size_t
igl::cgal::peel_outer_hull_layers(const Eigen::PlainObjectBase<DerivedV> &V,
  ...
  typedef Matrix<typename DerivedF::Scalar, Dynamic,
                 DerivedF::ColsAtCompileTime>
      MatrixXF;
  typedef Matrix<Index, Dynamic, 1> MatrixXI;
  typedef Matrix<typename Derivedflip::Scalar, Dynamic,
                 Derivedflip::ColsAtCompileTime>
      MatrixXflip;
  ...
  while (Fr.size() > 0) {
...
#ifdef IGL_PEEL_OUTER_HULL_LAYERS_DEBUG
    {
      ...
      std::transform(
          V.data(), V.data() + V.rows() * V.cols(), vertices.data(),
          [](typename DerivedV::Scalar val) { return CGAL::to_double(val); });
...
template size_t igl::cgal::peel_outer_hull_layers<
