...
ElementType determine_element_type(const Eigen::PlainObjectBase<DerivedV> &V,
...
template <typename DerivedV, typename DerivedF, typename DerivedI>
void extract_adj_faces(const Eigen::PlainObjectBase<DerivedV> &V,
                       const Eigen::PlainObjectBase<DerivedF> &F,
                       const Eigen::PlainObjectBase<DerivedI> &I,
  ...
  for (size_t i = 0; i < num_faces; i++) {
    ...
    if ((f[0] == s && f[1] == d) || (f[1] == s && f[2] == d) ||
        (f[2] == s && f[0] == d)) {
      adj_faces.push_back((I(i, 0) + 1) * -1);
      continue;
    }
    if ((f[0] == d && f[1] == s) || (f[1] == d && f[2] == s) ||
        (f[2] == d && f[0] == s)) {
...
template <typename DerivedV, typename DerivedF, typename DerivedI>
void extract_adj_vertices(const Eigen::PlainObjectBase<DerivedV> &V,
                          const Eigen::PlainObjectBase<DerivedF> &F,
                          const Eigen::PlainObjectBase<DerivedI> &I,
  ...
  std::copy(unique_adj_vertices.begin(), unique_adj_vertices.end(),
            adj_vertices.begin());
}

template <typename DerivedV, typename DerivedF, typename DerivedI>
bool determine_point_edge_orientation(const Eigen::PlainObjectBase<DerivedV> &V,
                                      const Eigen::PlainObjectBase<DerivedF> &F,
                                      const Eigen::PlainObjectBase<DerivedI> &I,
  ...
  if (adj_faces[order[0]] > 0 && adj_faces[order[num_adj_faces - 1] < 0]) {
    return true;
...
bool determine_point_vertex_orientation(
    ...
    const Eigen::PlainObjectBase<DerivedF> &F,
    const Eigen::PlainObjectBase<DerivedI> &I, const Point_3 &query, size_t s) {
...
template <typename DerivedV, typename DerivedF, typename DerivedI>
bool determine_point_face_orientation(const Eigen::PlainObjectBase<DerivedV> &V,
                                      const Eigen::PlainObjectBase<DerivedF> &F,
                                      const Eigen::PlainObjectBase<DerivedI> &I,
  ...
  if (result == CGAL::COPLANAR) {
    throw "Cannot determine inside/outside because query point lies exactly on "
          "the input surface.";
  }
...
IGL_INLINE void
igl::cgal::points_inside_component(const Eigen::PlainObjectBase<DerivedV> &V,
                                   const Eigen::PlainObjectBase<DerivedF> &F,
                                   const Eigen::PlainObjectBase<DerivedI> &I,
  ...
  for (size_t i = 0; i < num_faces; i++) {
    ...
    triangles.emplace_back(Point_3(V(f[0], 0), V(f[0], 1), V(f[0], 2)),
  ...
  for (size_t i = 0; i < num_queries; i++) {
    ...
    size_t element_index;
    switch (
        determine_element_type(V, F, I, fid, closest_point, element_index)) {
    case VERTEX: {
      const size_t s = F(I(fid, 0), element_index);
      inside(i, 0) = determine_point_vertex_orientation(V, F, I, query, s);
    } break;
    case EDGE: {
      const size_t s = F(I(fid, 0), (element_index + 1) % 3);
      const size_t d = F(I(fid, 0), (element_index + 2) % 3);
      inside(i, 0) = determine_point_edge_orientation(V, F, I, query, s, d);
    } break;
    case FACE:
...
template void
igl::cgal::points_inside_component<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
...
template void igl::cgal::points_inside_component<
    ...
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, 1, 0, -1, 1>> &);
template void
igl::cgal::points_inside_component<Eigen::Matrix<double, -1, 3, 0, -1, 3>,
                                   Eigen::Matrix<int, -1, 3, 0, -1, 3>,
