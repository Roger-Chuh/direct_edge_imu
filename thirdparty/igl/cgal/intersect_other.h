...
#define assert(isOK)                                                           \
  ((isOK) ? (void)0                                                            \
          : (void)mexErrMsgTxt(C_STR(__FILE__ << ":" << __LINE__               \
                                              << ": failed assertion `"        \
                                              << #isOK << "'" << std::endl)))
#endif
...
template <typename DerivedVA, typename DerivedFA, typename DerivedVB,
          ...
          typename DerivedIMB>
IGL_INLINE bool intersect_other(const Eigen::PlainObjectBase<DerivedVA> &VA,
...
IGL_INLINE bool intersect_other(const Eigen::MatrixXd &VA,
