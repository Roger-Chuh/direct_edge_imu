...
void igl::cgal::order_facets_around_edge(
    ...
    const Eigen::PlainObjectBase<DerivedF> &F, size_t s, size_t d,
  ...
  auto get_face_index = [&](int adj_f) -> size_t { return abs(adj_f) - 1; };

  auto get_opposite_vertex = [&](size_t fid) -> size_t {
    typedef typename DerivedF::Scalar Index;
    if (F(fid, 0) != (Index)s && F(fid, 0) != (Index)d)
      return F(fid, 0);
    if (F(fid, 1) != (Index)s && F(fid, 1) != (Index)d)
      return F(fid, 1);
    if (F(fid, 2) != (Index)s && F(fid, 2) != (Index)d)
      return F(fid, 2);
    assert(false);
    return -1;
  };
  ...
  if (adj_faces.size() == 0) {
    ...
    case CGAL::COPLANAR: {
      ...
      case CGAL::NEGATIVE:
        ...
        break;
      case CGAL::COLLINEAR:
        std::cerr << "Degenerated triangle detected." << std::endl;
        assert(false);
        break;
      default:
        assert(false);
  ...
  for (size_t i = 0; i < num_adj_faces; i++) {
    ...
    case CGAL::ON_ORIENTED_BOUNDARY: {
      auto inplane_orientation = CGAL::coplanar_orientation(p_s, p_d, p_o, p_a);
  ...
  for (size_t i = 0; i < num_adj_faces; i++) {
    ...
    const Point_3 &p_b = opposite_vertices[order((i + 1) % num_adj_faces, 0)];
...
template <typename DerivedV, typename DerivedF, typename DerivedI>
IGL_INLINE void igl::cgal::order_facets_around_edge(
    const Eigen::PlainObjectBase<DerivedV> &V,
    const Eigen::PlainObjectBase<DerivedF> &F, size_t s, size_t d,
    const std::vector<int> &adj_faces,
  ...
  igl::cgal::order_facets_around_edge(vertices, faces, N + 1, N + 2,
...
template void igl::cgal::order_facets_around_edge<
...
template void
igl::cgal::order_facets_around_edge<Eigen::Matrix<double, -1, 3, 0, -1, 3>,
    ...
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, 1, 0, -1, 1>> &);
template void
igl::cgal::order_facets_around_edge<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
                                    Eigen::Matrix<int, -1, -1, 0, -1, -1>,
                                    Eigen::Matrix<int, -1, 1, 0, -1, 1>>(
