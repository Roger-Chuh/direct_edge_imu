...
IGL_INLINE void igl::cgal::polyhedron_to_mesh(const Polyhedron &poly,
                                              Eigen::MatrixXd &V,
                                              Eigen::MatrixXi &F) {
  using namespace std;
  ...
  {
    size_t v = 0;
    for (typename Polyhedron::Vertex_const_iterator p = poly.vertices_begin();
         p != poly.vertices_end(); p++) {
      V(v, 0) = p->point().x();
      V(v, 1) = p->point().y();
      V(v, 2) = p->point().z();
      vertex_to_index[p] = v;
...
template void igl::cgal::polyhedron_to_mesh<CGAL::Polyhedron_3<
    CGAL::Simple_cartesian<double>, CGAL::Polyhedron_items_with_id_3,
    CGAL::HalfedgeDS_default, std::allocator<int>>>(
    CGAL::Polyhedron_3<CGAL::Simple_cartesian<double>,
                       CGAL::Polyhedron_items_with_id_3,
                       CGAL::HalfedgeDS_default, std::allocator<int>> const &,
