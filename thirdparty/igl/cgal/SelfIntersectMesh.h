...
template <typename Kernel, typename DerivedV, typename DerivedF,
          typename DerivedVV, typename DerivedFF, typename DerivedIF,
          typename DerivedJ, typename DerivedIM>
class SelfIntersectMesh {
  typedef SelfIntersectMesh<Kernel, DerivedV, DerivedF, DerivedVV, DerivedFF,
                            DerivedIF, DerivedJ, DerivedIM>
      Self;
...
public:
  ...
  inline SelfIntersectMesh(const Eigen::PlainObjectBase<DerivedV> &V,
                           const Eigen::PlainObjectBase<DerivedF> &F,
...
private:
  ...
  //
  inline bool intersect(const Triangle_3 &A, const Triangle_3 &B,
                        const Index fa, const Index fb);
  ...
  //
  inline bool single_shared_vertex(const Triangle_3 &A, const Triangle_3 &B,
                                   const Index fa, const Index fb,
                                   const Index va, const Index vb);
  ...
  inline bool
  double_shared_vertex(const Triangle_3 &A, const Triangle_3 &B, const Index fa,
                       const Index fb,
...
public:
  ...
  static inline void box_intersect_static(SelfIntersectMesh *SIM, const Box &a,
                                          const Box &b);
};
} // namespace cgal
} // namespace igl
...
// static void box_intersect(SelfIntersectMesh * SIM,const Box & A, const Box &
// B)
//{
...
inline void igl::cgal::SelfIntersectMesh<
    Kernel, DerivedV, DerivedF, DerivedVV, DerivedFF, DerivedIF, DerivedJ,
    DerivedIM>::box_intersect_static(Self *SIM, const typename Self::Box &a,
...
inline igl::cgal::SelfIntersectMesh<
    Kernel, DerivedV, DerivedF, DerivedVV, DerivedFF, DerivedIF, DerivedJ,
    DerivedIM>::SelfIntersectMesh(const Eigen::PlainObjectBase<DerivedV> &V,
                                  const Eigen::PlainObjectBase<DerivedF> &F,
                                  const RemeshSelfIntersectionsParam &params,
                                  Eigen::PlainObjectBase<DerivedVV> &VV,
                                  Eigen::PlainObjectBase<DerivedFF> &FF,
                                  ...
                                  Eigen::PlainObjectBase<DerivedJ> &J,
                                  Eigen::PlainObjectBase<DerivedIM> &IM)
    : V(V), F(F), count(0), T(), lIF(), offending(), edge2faces(),
      params(params) {
  using namespace std;
  ...
  for (TrianglesIterator tit = T.begin(); tit != T.end(); ++tit) {
    boxes.push_back(Box(tit->bbox(), tit));
  }
  // Leapfrog callback
  std::function<void(const Box &a, const Box &b)> cb =
      std::bind(&box_intersect_static, this,
                // Explicitly use std namespace to avoid confusion with boost
                // (who puts _1 etc. in global namespace)
  ...
  {
    Index i = 0;
    for (typename IndexList::const_iterator ifit = lIF.begin();
         ifit != lIF.end();) {
      IF(i, 0) = (*ifit);
      ifit++;
      IF(i, 1) = (*ifit);
...
template <typename Kernel, typename DerivedV, typename DerivedF,
          ...
          typename DerivedJ, typename DerivedIM>
inline void
igl::cgal::SelfIntersectMesh<Kernel, DerivedV, DerivedF, DerivedVV, DerivedFF,
                             DerivedIF, DerivedJ,
  ...
  if (offending.count(f) == 0) {
    ...
    for (Index e = 0; e < 3; e++) {
      ...
      Index i = F(f, (e + 1) % 3) < F(f, (e + 2) % 3) ? F(f, (e + 1) % 3)
                                                      : F(f, (e + 2) % 3);
      Index j = F(f, (e + 1) % 3) < F(f, (e + 2) % 3) ? F(f, (e + 2) % 3)
                                                      : F(f, (e + 1) % 3);
...
inline void igl::cgal::SelfIntersectMesh<
    Kernel, DerivedV, DerivedF, DerivedVV, DerivedFF, DerivedIF, DerivedJ,
    DerivedIM>::count_intersection(const Index fa, const Index fb) {
  mark_offensive(fa);
  mark_offensive(fb);
  this->count++;
...
inline bool igl::cgal::SelfIntersectMesh<
    Kernel, DerivedV, DerivedF, DerivedVV, DerivedFF, DerivedIF, DerivedJ,
    DerivedIM>::intersect(const Triangle_3 &A, const Triangle_3 &B,
                          const Index fa, const Index fb) {
...
inline bool igl::cgal::SelfIntersectMesh<
    Kernel, DerivedV, DerivedF, DerivedVV, DerivedFF, DerivedIF, DerivedJ,
    DerivedIM>::single_shared_vertex(const Triangle_3 &A, const Triangle_3 &B,
                                     const Index fa, const Index fb,
                                     const Index va, const Index vb) {
  ////using namespace std;
  // CGAL::Object result = CGAL::intersection(A,B);
...
inline bool igl::cgal::SelfIntersectMesh<
  ...
  if (CGAL::do_intersect(sa, B)) {
    ...
    if (const Point_3 *p = CGAL::object_cast<Point_3>(&result)) {
      ...
      // cerr<<REDRUM("Coplanar at: "<<fa<<" & "<<fb<<" (single
      // shared).")<<endl;
      // Must be coplanar
      // WRONG:
...
igl::cgal::SelfIntersectMesh<Kernel, DerivedV, DerivedF, DerivedVV, DerivedFF,
                             DerivedIF, DerivedJ, DerivedIM>::
    double_shared_vertex(const Triangle_3 &A, const Triangle_3 &B,
                         const Index fa, const Index fb,
                         const std::vector<std::pair<Index, Index>> shared) {
  using namespace std;

  // must be co-planar
  if (A.supporting_plane() != B.supporting_plane() &&
  ...
  const auto &opposite_point_inside = [](const Triangle_3 &A, const Index a0,
                                         const Index a1,
  ...
  const auto &opposite_edges_intersect = [](const Triangle_3 &A, const Index va,
                                            const Triangle_3 &B,
                                            const Index vb) -> bool {
    Segment_3 sa(A.vertex((va + 1) % 3), A.vertex((va + 2) % 3));
  ...
  if (!opposite_point_inside(A, shared[0].first, shared[1].first, B) &&
  ...
  try {
    ...
    if (!result.empty()) {
      if (CGAL::object_cast<Segment_3>(&result)) {
        // not coplanar
        assert(false && "Co-planar non-degenerate triangles should intersect "
                        "over triangle");
        return false;
        ...
        offending[fa].second.push_back(result);
        offending[fb].second.push_back(result);
        // cerr<<REDRUM("Coplanar at: "<<fa<<" & "<<fb<<" (double
        // shared).")<<endl;
        return true;
      }
    } else {
...
inline void igl::cgal::SelfIntersectMesh<
    Kernel, DerivedV, DerivedF, DerivedVV, DerivedFF, DerivedIF, DerivedJ,
    DerivedIM>::box_intersect(const Box &a, const Box &b) {
  using namespace std;
  ...
  if (comb_shared_vertices == 3) {
    ...
    /// preprocessing
    // cerr<<REDRUM("Facets "<<fa<<" and "<<fb<<" are combinatorial
    // duplicates")<<endl;
    goto done;
  }
  if (total_shared_vertices == 3) {
    ...
    // cerr<<REDRUM("Facets "<<fa<<" and "<<fb<<" are geometrical
    // duplicates")<<endl;
    goto done;
  }
