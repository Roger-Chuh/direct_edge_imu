...
IGL_INLINE int igl::launch_medit(const Eigen::PlainObjectBase<DerivedV> &V,
  ...
  const string default_medit_file_contents = "BackgroundColor 1 1 1\n"
                                             "LineColor 0 0 0\n"
                                             "WindowSize 1024 800\n"
                                             "RenderMode shading + lines\n";
  FILE *fp = fopen(TEMP_MEDIT_FILE, "w");
  if (fp == NULL) {
    cerr << "^" << __FUNCTION__ << ": Could not write to " << TEMP_MEDIT_FILE
         << endl;
    return -1;
  }
...
template int igl::launch_medit<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
