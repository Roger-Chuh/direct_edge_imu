...
#define SERIALIZE_TYPE(Type, Params)                                           \
  ...
  template <>                                                                  \
  inline void serialize(const Type &obj, std::vector<char> &buffer) {          \
    _serialization(true, const_cast<Type &>(obj), buffer);                     \
  }                                                                            \
  template <>                                                                  \
  inline void deserialize(Type &obj, const std::vector<char> &buffer) {        \
...
#define SERIALIZE_MEMBER(Object)                                               \
  igl::serializer(s, obj.Object, std::string(#Object), buffer);
#define SERIALIZE_MEMBER_NAME(Object, Name)                                    \
  igl::serializer(s, obj.Object, std::string(Name), buffer);
...
template <typename T>
inline bool serialize(const T &obj, const std::string &objectName,
...
template <typename T>
inline bool deserialize(T &obj, const std::string &objectName,
...
template <typename T>
inline bool serializer(bool serialize, T &obj, const std::string &objectName,
                       const std::string &filename, bool overwrite = false);
template <typename T>
inline bool serializer(bool serialize, T &obj, const std::string &objectName,
                       std::vector<char> &buffer);

// User defined types have to either overload the function
// igl::serialization::serialize() and igl::serialization::deserialize() for
// their type (non-intrusive serialization):
//
...
private:
  template <typename T> struct SerializationObject : public SerializableBase {
    bool Binary;
    std::string Name;
...
template <typename T> struct IndexedPointer : public IndexedPointerBase {
  const T *Object;
};
...
template <typename T1, typename T2> struct is_stl_container<std::pair<T1, T2>> {
...
template <typename T> struct is_stl_container<std::set<T>> {
  static const bool value = true;
};
template <typename T1, typename T2> struct is_stl_container<std::map<T1, T2>> {
  static const bool value = true;
};

template <typename T> struct is_eigen_type { static const bool value = false; };
...
template <typename T> struct is_smart_ptr { static const bool value = false; };
...
template <typename T> struct is_serializable {
  static const bool value =
      std::is_fundamental<T>::value || std::is_same<std::string, T>::value ||
      std::is_enum<T>::value || std::is_base_of<SerializableBase, T>::value ||
      is_stl_container<T>::value || is_eigen_type<T>::value ||
      std::is_pointer<T>::value || serialization::is_smart_ptr<T>::value;
};

// non serializable types
template <typename T>
inline typename std::enable_if<!is_serializable<T>::value, size_t>::type
getByteSize(const T &obj);
template <typename T>
inline typename std::enable_if<!is_serializable<T>::value>::type
serialize(const T &obj, std::vector<char> &buffer,
...
// fundamental types
template <typename T>
inline typename std::enable_if<std::is_fundamental<T>::value, size_t>::type
getByteSize(const T &obj);
template <typename T>
...
inline void serialize(const std::string &obj, std::vector<char> &buffer,
                      std::vector<char>::iterator &iter);
...
// SerializableBase
template <typename T>
inline typename std::enable_if<std::is_base_of<SerializableBase, T>::value,
                               size_t>::type
getByteSize(const T &obj);
template <typename T>
inline
    typename std::enable_if<std::is_base_of<SerializableBase, T>::value>::type
    serialize(const T &obj, std::vector<char> &buffer,
              std::vector<char>::iterator &iter);
template <typename T>
inline
...
template <typename T1, typename T2>
inline void serialize(const std::pair<T1, T2> &obj, std::vector<char> &buffer,
                      std::vector<char>::iterator &iter);
template <typename T1, typename T2>
...
template <typename T1, typename T2>
inline void serialize(const std::vector<T1, T2> &obj, std::vector<char> &buffer,
                      std::vector<char>::iterator &iter);
template <typename T1, typename T2>
inline void deserialize(std::vector<T1, T2> &obj,
                        std::vector<char>::const_iterator &iter);
template <typename T2>
inline void deserialize(std::vector<bool, T2> &obj,
                        std::vector<char>::const_iterator &iter);

// std::set
template <typename T> inline size_t getByteSize(const std::set<T> &obj);
template <typename T>
inline void serialize(const std::set<T> &obj, std::vector<char> &buffer,
                      std::vector<char>::iterator &iter);
template <typename T>
inline void deserialize(std::set<T> &obj,
                        std::vector<char>::const_iterator &iter);
...
template <typename T1, typename T2>
inline void serialize(const std::map<T1, T2> &obj, std::vector<char> &buffer,
                      std::vector<char>::iterator &iter);
template <typename T1, typename T2>
inline void deserialize(std::map<T1, T2> &obj,
                        std::vector<char>::const_iterator &iter);
...
template <typename T, int P, typename I>
inline void serialize(const Eigen::SparseMatrix<T, P, I> &obj,
                      std::vector<char> &buffer,
                      std::vector<char>::iterator &iter);
template <typename T, int P, typename I>
inline void deserialize(Eigen::SparseMatrix<T, P, I> &obj,
                        std::vector<char>::const_iterator &iter);

template <typename T, int P>
inline size_t getByteSize(const Eigen::Quaternion<T, P> &obj);
template <typename T, int P>
inline void serialize(const Eigen::Quaternion<T, P> &obj,
                      std::vector<char> &buffer,
                      std::vector<char>::iterator &iter);
template <typename T, int P>
inline void deserialize(Eigen::Quaternion<T, P> &obj,
                        std::vector<char>::const_iterator &iter);

// raw pointers
template <typename T>
inline typename std::enable_if<std::is_pointer<T>::value, size_t>::type
getByteSize(const T &obj);
template <typename T>
...
template <typename T>
inline
    typename std::enable_if<serialization::is_smart_ptr<T>::value, size_t>::type
    getByteSize(const T &obj);
template <typename T>
inline typename std::enable_if<serialization::is_smart_ptr<T>::value>::type
serialize(const T &obj, std::vector<char> &buffer,
          std::vector<char>::iterator &iter);
template <template <typename> class T0, typename T1>
inline typename std::enable_if<serialization::is_smart_ptr<T0<T1>>::value>::type
deserialize(T0<T1> &obj, std::vector<char>::const_iterator &iter);

// std::weak_ptr
template <typename T> inline size_t getByteSize(const std::weak_ptr<T> &obj);
template <typename T>
inline void serialize(const std::weak_ptr<T> &obj, std::vector<char> &buffer,
                      std::vector<char>::iterator &iter);
template <typename T>
inline void deserialize(std::weak_ptr<T> &obj,
                        std::vector<char>::const_iterator &iter);
...
// helper functions
template <typename T> inline void updateMemoryMap(T &obj, size_t size);
} // namespace serialization
} // namespace igl
...
inline bool serialize(const T &obj, const std::string &objectName,
  ...
  if (file.is_open()) {
    ...
    std::cerr << "serialization: file " << filename << " not found!"
              << std::endl;
  }
...
inline bool serialize(const T &obj, const std::string &objectName,
  ...
  size_t newHeaderSize = serialization::getByteSize(objectName) +
                         serialization::getByteSize(objectType) +
                         sizeof(size_t);
...
template <typename T>
inline bool serializer(bool s, T &obj, const std::string &objectName,
                       const std::string &filename, bool overwrite) {
  return s ? serialize(obj, objectName, filename, overwrite)
           : deserialize(obj, objectName, filename);
}

template <typename T>
inline bool serializer(bool s, T &obj, const std::string &objectName,
                       std::vector<char> &buffer) {
  return s ? serialize(obj, objectName, buffer)
           : deserialize(obj, objectName, buffer);
}

inline bool Serializable::PreSerialization() const { return true; }

inline void Serializable::PostSerialization() const {}

inline bool Serializable::PreDeserialization() { return true; }
...
inline Serializable::Serializable() { initialized = false; }
...
inline size_t getByteSize(const std::vector<T1, T2> &obj) {
  return std::accumulate(
      obj.begin(), obj.end(), sizeof(size_t),
      [](const size_t &acc, const T1 &cur) { return acc + getByteSize(cur); });
}
...
template <typename T> inline size_t getByteSize(const std::set<T> &obj) {
  return std::accumulate(
      obj.begin(), obj.end(), getByteSize(obj.size()),
      [](const size_t &acc, const T &cur) { return acc + getByteSize(cur); });
}
...
inline size_t getByteSize(const std::map<T1, T2> &obj) {
  return std::accumulate(obj.begin(), obj.end(), sizeof(size_t),
                         [](const size_t &acc, const std::pair<T1, T2> &cur) {
                           return acc + getByteSize(cur);
                         });
}
...
inline size_t getByteSize(const Eigen::Matrix<T, R, C, P, MR, MC> &obj) {
  ...
  return 2 * sizeof(typename Eigen::Matrix<T, R, C, P, MR, MC>::Index) +
...
template <typename T, int P, typename I>
inline size_t getByteSize(const Eigen::SparseMatrix<T, P, I> &obj) {
  // space for numbers of rows,cols,nonZeros and tripplets with data
  // (rowIdx,colIdx,value)
...
inline void serialize(const Eigen::SparseMatrix<T, P, I> &obj,
  ...
  for (int k = 0; k < obj.outerSize(); ++k) {
    for (typename Eigen::SparseMatrix<T, P, I>::InnerIterator it(obj, k); it;
         ++it) {
...
deserialize(T &obj, std::vector<char>::const_iterator &iter) {
  ...
  if (isNullPtr) {
    if (obj) {
      std::cout << "serialization: possible memory leak in serialization for '"
                << typeid(obj).name() << "'" << std::endl;
      obj = nullptr;
    }
  } else {
    if (obj) {
      std::cout << "serialization: possible memory corruption in "
                   "deserialization for '"
                << typeid(obj).name() << "'" << std::endl;
    } else {
...
inline void serialize(const T &obj, std::vector<char> &buffer) {
  std::cerr
      << typeid(obj).name()
      << " is not serializable: derive from igl::Serializable or spezialize "
         "the template function igl::serialization::serialize(const T& "
         "obj,std::vector<char>& buffer)"
      << std::endl;
}
...
inline void deserialize(T &obj, const std::vector<char> &buffer) {
  std::cerr
      << typeid(obj).name()
      << " is not deserializable: derive from igl::Serializable or spezialize "
         "the template function igl::serialization::deserialize(T& obj, const "
         "std::vector<char>& buffer)"
      << std::endl;
}
...
template <typename T>
inline void
updateMemoryMap(T &obj, size_t size,
  ...
  auto p = std::pair<std::uintptr_t, IndexedPointerBase *>(startAddress,
                                                           startBasePtr);

  auto el = memoryMap.insert(p);
  auto iter = ++el.first; // next elememt
  if (el.second && (iter == memoryMap.end() ||
                    iter->second->Type != IndexedPointerBase::END)) {
    // not yet serialized
    ...
    auto p =
        std::pair<std::uintptr_t, IndexedPointerBase *>(endAddress, endBasePtr);
