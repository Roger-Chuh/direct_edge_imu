...
template <typename DerivedV, typename DerivedF> class AngleBoundFFSolverData {
public:
...
private:
  ...
  IGL_INLINE void computeCoefficientLaplacian(
      int n, Eigen::SparseMatrix<std::complex<typename DerivedV::Scalar>> &D);
...
private:
  ...
  //    IGL_INLINE void computeAngles(Eigen::Matrix<typename DerivedV::Scalar,
  //    Eigen::Dynamic, 1> &angles);
...
IGL_INLINE
igl::AngleBoundFFSolverData<DerivedV, DerivedF>::AngleBoundFFSolverData(
    const Eigen::PlainObjectBase<DerivedV> &_V,
    const Eigen::PlainObjectBase<DerivedF> &_F)
    : V(_V), numV(_V.rows()), F(_F), numF(_F.rows()) {
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE void
igl::AngleBoundFFSolverData<DerivedV, DerivedF>::computeLaplacians() {
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE void
igl::AngleBoundFFSolverData<DerivedV, DerivedF>::precomputeInteriorEdges() {
  // Flag border edges
  numInteriorEdges = 0;
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE void
igl::AngleBoundFFSolverData<DerivedV, DerivedF>::computeCoefficientLaplacian(
    int n, Eigen::SparseMatrix<std::complex<typename DerivedV::Scalar>> &D) {
  std::vector<Eigen::Triplet<std::complex<typename DerivedV::Scalar>>>
      tripletList;
    ...
    if (!isBorderEdge[eid]) {
      ...
      tripletList.push_back(
          Eigen::Triplet<std::complex<typename DerivedV::Scalar>>(
              fid0, fid0, std::complex<typename DerivedV::Scalar>(1.)));
      tripletList.push_back(
          Eigen::Triplet<std::complex<typename DerivedV::Scalar>>(
              fid1, fid1, std::complex<typename DerivedV::Scalar>(1.)));
      tripletList.push_back(
          Eigen::Triplet<std::complex<typename DerivedV::Scalar>>(
              fid0, fid1, -1. * std::polar(1., -1. * n * K[eid])));
      tripletList.push_back(
          Eigen::Triplet<std::complex<typename DerivedV::Scalar>>(
              fid1, fid0, -1. * std::polar(1., 1. * n * K[eid])));
    }
  }
  D.resize(numF, numF);
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE void igl::AngleBoundFFSolverData<DerivedV, DerivedF>::computek() {
  K.setZero(numE);
  // For every non-border edge
  for (unsigned eid = 0; eid < numE; ++eid) {
    if (!isBorderEdge[eid]) {
      ...
      Eigen::Matrix<typename DerivedV::Scalar, 1, 3> common_edge =
          V.row(F(fid0, (fid0_vc + 1) % 3)) - V.row(F(fid0, fid0_vc));
      common_edge.normalize();

      // Map the two triangles in a new space where the common edge is the x
      // axis and the N0 the z axis
      Eigen::Matrix<typename DerivedV::Scalar, 3, 3> P;
      Eigen::Matrix<typename DerivedV::Scalar, 1, 3> o =
          V.row(F(fid0, fid0_vc));
      Eigen::Matrix<typename DerivedV::Scalar, 1, 3> tmp =
          -N0.cross(common_edge);
      ...
      // k_ij is the angle between the triangle on the left and the one on the
      // right
      Eigen::Matrix<typename DerivedV::Scalar, 1, 3> ref0 =
          V0.row(1) - V0.row(0);
      Eigen::Matrix<typename DerivedV::Scalar, 1, 3> ref1 =
          V1.row(1) - V1.row(0);
      ...
      Eigen::Matrix<typename DerivedV::Scalar, 1, 2> tmp1 =
          R2 * (ref0.head(2)).transpose();
...
template <typename DerivedV, typename DerivedF, typename DerivedO>
IGL_INLINE
igl::AngleBoundFFSolver<DerivedV, DerivedF, DerivedO>::AngleBoundFFSolver(
    const AngleBoundFFSolverData<DerivedV, DerivedF> &_data,
    const typename DerivedV::Scalar &_thetaMin, int _maxIter,
    const typename DerivedV::Scalar &_lambdaInit,
    const typename DerivedV::Scalar &_lambdaMultFactor,
    const bool _doHardConstraints)
    : data(_data), lambdaInit(_lambdaInit), maxIter(_maxIter),
      lambdaMultFactor(_lambdaMultFactor),
      doHardConstraints(_doHardConstraints), thetaMin(_thetaMin) {
...
template <typename DerivedV, typename DerivedF, typename DerivedO>
IGL_INLINE void
igl::AngleBoundFFSolver<DerivedV, DerivedF, DerivedO>::rotateAroundBisector(
    const std::complex<typename DerivedV::Scalar> &uin,
    const std::complex<typename DerivedV::Scalar> &vin,
...
template <typename DerivedV, typename DerivedF, typename DerivedO>
IGL_INLINE void
igl::AngleBoundFFSolver<DerivedV, DerivedF, DerivedO>::localStep() {
  for (int j = 0; j < data.numF; ++j) {
    ...
    if (current_angle < thetaMin * M_PI / 180) {
      ...
      typename DerivedV::Scalar diff =
          thetaMin * M_PI / 180 - current_angle + 1e-6;
      rotateAroundBisector(u, v, diff, u1, v1);

      //      if (computeAngle(u1, v1)<thetaMin*M_PI/180)
      //      {
      //        std::cerr<<"u = ["<<real(u)<<","<<imag(u)<< "]; v=
      //        ["<<real(v)<<","<<imag(v)<<"];"<<std::endl; std::cerr<<"u1 =
      //        ["<<real(u1)<<","<<imag(u1)<< "]; v1=
      //        ["<<real(v1)<<","<<imag(v1)<<"];"<<std::endl;
      //        std::cerr<<"current_angle = "<<current_angle<<std::endl;
      //        std::cerr<<"aout = "<<computeAngle(u1, v1)<< "; theta=
      //        "<<thetaMin*M_PI/180<<";"<<std::endl;
      //      }
...
// computeAngles(Eigen::Matrix<typename DerivedV::Scalar, Eigen::Dynamic, 1>
// &angles)
//{
...
template <typename DerivedV, typename DerivedF, typename DerivedO>
IGL_INLINE typename DerivedV::Scalar
igl::AngleBoundFFSolver<DerivedV, DerivedF, DerivedO>::computeAngle(
    const std::complex<typename DerivedV::Scalar> &u,
    const std::complex<typename DerivedV::Scalar> &v) {
  typename DerivedV::Scalar angle =
      std::min(fabs(arg(u * conj(v))), fabs(arg(u * conj(-v))));
...
template <typename DerivedV, typename DerivedF, typename DerivedO>
IGL_INLINE int
igl::AngleBoundFFSolver<DerivedV, DerivedF, DerivedO>::getNumOutOfBounds() {
...
template <typename DerivedV, typename DerivedF, typename DerivedO>
IGL_INLINE void igl::AngleBoundFFSolver<DerivedV, DerivedF,
...
template <typename DerivedV, typename DerivedF, typename DerivedO>
IGL_INLINE void
igl::AngleBoundFFSolver<DerivedV, DerivedF, DerivedO>::globalStep(
...
template <typename DerivedV, typename DerivedF, typename DerivedO>
IGL_INLINE void
igl::AngleBoundFFSolver<DerivedV, DerivedF, DerivedO>::minQuadWithKnownMini(
    const Eigen::SparseMatrix<std::complex<typename DerivedV::Scalar>> &Q,
    const Eigen::SparseMatrix<std::complex<typename DerivedV::Scalar>> &f,
    const Eigen::VectorXi isConstrained,
    const Eigen::Matrix<std::complex<typename DerivedV::Scalar>, Eigen::Dynamic,
...
template <typename DerivedV, typename DerivedF, typename DerivedO>
IGL_INLINE bool igl::AngleBoundFFSolver<DerivedV, DerivedF, DerivedO>::solve(
    const Eigen::VectorXi &isConstrained,
    const Eigen::PlainObjectBase<DerivedO> &initialSolution,
    Eigen::PlainObjectBase<DerivedO> &output,
...
template <typename DerivedV, typename DerivedF, typename DerivedO>
IGL_INLINE bool igl::angle_bound_frame_fields(
    const Eigen::PlainObjectBase<DerivedV> &V,
    const Eigen::PlainObjectBase<DerivedF> &F,
  ...
  igl::AngleBoundFFSolver<DerivedV, DerivedF, DerivedO> cs(
      csdata, thetaMin, maxIter, lambdaInit, lambdaMultFactor,
      doHardConstraints);
...
template <typename DerivedV, typename DerivedF, typename DerivedO>
IGL_INLINE bool igl::angle_bound_frame_fields(
    const igl::AngleBoundFFSolverData<DerivedV, DerivedF> &csdata,
    const typename DerivedV::Scalar &thetaMin,
    const Eigen::VectorXi &isConstrained,
    const Eigen::PlainObjectBase<DerivedO> &initialSolution,
    Eigen::PlainObjectBase<DerivedO> &output, int maxIter,
    const typename DerivedV::Scalar &lambdaInit,
    const typename DerivedV::Scalar &lambdaMultFactor,
    const bool doHardConstraints, typename DerivedV::Scalar *lambdaOut) {
  igl::AngleBoundFFSolver<DerivedV, DerivedF, DerivedO> cs(
      csdata, thetaMin, maxIter, lambdaInit, lambdaMultFactor,
      doHardConstraints);
