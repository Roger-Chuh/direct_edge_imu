...
IGL_INLINE void igl::doublearea(const Eigen::PlainObjectBase<DerivedV> &V,
  ...
  const auto &proj_doublearea = [&V, &F](const int x, const int y,
                                         const int f) -> double {
...
template <typename DerivedA, typename DerivedB, typename DerivedC>
IGL_INLINE typename DerivedA::Scalar
igl::doublearea_single(const Eigen::PlainObjectBase<DerivedA> &A,
                       const Eigen::PlainObjectBase<DerivedB> &B,
                       const Eigen::PlainObjectBase<DerivedC> &C) {
  auto r = A - C;
  auto s = B - C;
...
#pragma omp parallel for if (m > IGL_OMP_MIN_VALUE)
  for (Index i = 0; i < m; i++) {
    ...
    const typename Derivedl::Scalar arg =
        (l(i, 0) + (l(i, 1) + l(i, 2))) * (l(i, 2) - (l(i, 0) - l(i, 1))) *
        (l(i, 2) + (l(i, 0) - l(i, 1))) * (l(i, 0) + (l(i, 1) - l(i, 2)));
...
template <typename DerivedV, typename DerivedF, typename DeriveddblA>
IGL_INLINE void
igl::doublearea_quad(const Eigen::PlainObjectBase<DerivedV> &V,
                     const Eigen::PlainObjectBase<DerivedF> &F,
                     Eigen::PlainObjectBase<DeriveddblA> &dblA) {
...
template void igl::doublearea<Eigen::Matrix<double, -1, 3, 0, -1, 3>,
    ...
    Eigen::PlainObjectBase<Eigen::Matrix<double, -1, 1, 0, -1, 1>> &);
template Eigen::Matrix<double, 2, 1, 0, 2, 1>::Scalar
igl::doublearea_single<Eigen::Matrix<double, 2, 1, 0, 2, 1>,
