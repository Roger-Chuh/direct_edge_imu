...
template <typename DerivedV, typename DerivedF, typename DerivedFF,
          typename DerivedC>
class IntegrableFieldSolverData;
...
//   bc                #B by 6 list of the representative vectors at the
//   constrained faces constraint_level  #B by 1 list of "constraint level" flag
//   (level=2 indicates that both directions are constrained,
//                     level = 1 indicates a partially constrained face, i.e.
//                     only the first vector will be constrained)
//   original_field    #F by 6 list of the representative vectors of the frame
//   field to be used as a starting point
//                     (up to permutation and sign, stacked horizontally for
//                     each face)
// Returns:
//   data              an IntegrableFieldSolverData object that holds all
//   intermediate
//                     data needed by the solve routine, with correctly
//                     initialized values.
//
template <typename DerivedV, typename DerivedF, typename DerivedFF,
          typename DerivedC>
IGL_INLINE void integrable_polyvector_fields_precompute(
...
// Inputs:
//   data                         an IntegrableFieldSolverData object that holds
//   all intermediate
//                                data needed by the solve routine, with their
//                                values at the current time instance.
//   params                       solver parameters (see below)
//   current_field                #F by 6 list of the representative vectors of
//   the current frame field to be used as a starting point
//   current_field_is_not_ccw     boolean, determines whether the representative
//   vectors in the current field are in
//                                non- ccw order - if true, they will be
//                                corrected before being passed to the solver.
//                                The field returned by this routine is always
//                                in ccw order, so this flag typically only
//                                needs to be set to true during the first call
//                                to solve(). If unsure, set to true.
// Returns:
//   current_field                updated estimate for the integrable field
//
template <typename DerivedV, typename DerivedF, typename DerivedFF,
          typename DerivedC>
IGL_INLINE void integrable_polyvector_fields_solve(
...
struct igl::integrable_polyvector_fields_parameters {
  ...
  double wBarrier;
  // the s-parameter of the barrier term (see Schüller et al. 2013, Locally
  // Injective Mappings)
  double sBarrier;
  ...
  double wSmooth;
  // weight for the closeness to the original field, for the unconstrained faces
  // (regularization term)
  ...
  double gamma;
  // tikhonov regularization term (typically not needed, default value should
  // suffice)
  double tikh_gamma;
...
public:
  ...
  Eigen::VectorXi
      is_constrained_face; // 0 for unconstrained, 1 for constrained once
                           // (partial) and 2 for fully constrained
  ...
  IGL_INLINE void face_Jacobian_indices(const int &startRow, const int &toplace,
  ...
  IGL_INLINE void initializeOriginalVariable(
      const Eigen::PlainObjectBase<DerivedFF> &original_field);
  IGL_INLINE void
...
public:
  ...
  IGL_INLINE IntegrableFieldSolverData(
      ...
      const Eigen::PlainObjectBase<DerivedF> &_F, const Eigen::VectorXi &b,
      const Eigen::VectorXi &constraint_level,
      const Eigen::PlainObjectBase<DerivedFF> &original_field);
};
