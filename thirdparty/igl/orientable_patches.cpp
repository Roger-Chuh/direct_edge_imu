...
IGL_INLINE void
igl::orientable_patches(const Eigen::PlainObjectBase<DerivedF> &F,
  ...
  for (int j = 0; j < (int)uE2FT.outerSize(); j++) {
    ...
    // Iterate over inside
    if (degree > 2) {
      for (typename SparseMatrix<AScalar>::InnerIterator it(uE2FT, j); it;
           ++it) {
...
template void igl::orientable_patches<Eigen::Matrix<int, -1, -1, 0, -1, -1>,
