...
template <typename MT, typename DerivedV, typename DerivedF, typename DerivedE,
          typename DerivedEMAP>
void igl::crouzeix_raviart_massmatrix(
    const Eigen::PlainObjectBase<DerivedV> &V,
    const Eigen::PlainObjectBase<DerivedF> &F, Eigen::SparseMatrix<MT> &M,
...
void igl::crouzeix_raviart_massmatrix(
  ...
  for (int f = 0; f < m; f++) {
    for (int c = 0; c < 3; c++) {
      MIJV[f + m * c] =
          Triplet<MT>(EMAP(f + m * c), EMAP(f + m * c), TA(f) * 0.5);
    }
  }
...
template void igl::crouzeix_raviart_massmatrix<
...
template void
igl::crouzeix_raviart_massmatrix<float, Eigen::Matrix<float, -1, 3, 1, -1, 3>,
