...
template <typename DerivedV, typename DerivedF, typename DerivedI,
          typename IndexType, typename DerivedA>
  ...
  for (size_t i = 0; i < num_selected_faces; i++) {
    ...
    for (size_t j = 0; j < 3; j++) {
      ...
      if (outer_vid == INVALID || vx > outer_val) {
        ...
        bool replace = (vy > outer_y) || ((vy == outer_y) && (vz > outer_z));
        if (replace) {
          outer_val = vx;
          outer_vid = v;
          candidate_faces = {f};
...
IGL_INLINE void igl::outer_edge(const Eigen::PlainObjectBase<DerivedV> &V,
                                const Eigen::PlainObjectBase<DerivedF> &F,
                                const Eigen::PlainObjectBase<DerivedI> &I,
                                IndexType &v1, IndexType &v2,
                                Eigen::PlainObjectBase<DerivedA> &A) {
  // Algorithm:
  //    Find an outer vertex first.
  //    Find the incident edge with largest abs slope when projected onto XY
  //    plane. If there is a tie, check the signed slope and use the positive
  //    one. If there is still a tie, break it using the projected slope onto ZX
  //    plane. If there is still a tie, again check the signed slope and use the
  //    positive one. If there is still a tie, then there are multiple
  //    overlapping edges, which violates the precondition.
    ...
    if (!infinite_slope_detected && outer_v[0] != opp_v[0]) {
      ...
      if (outer_opp_vid == INVALID) {
        ...
        if (u_slope_YX > u_outer_slope_YX) {
          ...
          const Scalar u_outer_slope_ZX = unsigned_value(outer_slope_ZX);
...
template void igl::outer_facet<Eigen::Matrix<double, -1, 3, 0, -1, 3>,
                               Eigen::Matrix<int, -1, 3, 0, -1, 3>,
                               Eigen::Matrix<double, -1, 3, 0, -1, 3>,
                               Eigen::Matrix<long, -1, 1, 0, -1, 1>, int>(
    Eigen::PlainObjectBase<Eigen::Matrix<double, -1, 3, 0, -1, 3>> const &,
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, 3, 0, -1, 3>> const &,
    Eigen::PlainObjectBase<Eigen::Matrix<double, -1, 3, 0, -1, 3>> const &,
    Eigen::PlainObjectBase<Eigen::Matrix<long, -1, 1, 0, -1, 1>> const &, int &,
    bool &);
template void
igl::outer_facet<Eigen::Matrix<double, -1, -1, 1, -1, -1>,
                 Eigen::Matrix<int, -1, -1, 1, -1, -1>,
                 Eigen::Matrix<double, -1, -1, 1, -1, -1>,
                 Eigen::Matrix<int, -1, -1, 1, -1, -1>, unsigned long>(
    Eigen::PlainObjectBase<Eigen::Matrix<double, -1, -1, 1, -1, -1>> const &,
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, -1, 1, -1, -1>> const &,
    Eigen::PlainObjectBase<Eigen::Matrix<double, -1, -1, 1, -1, -1>> const &,
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, -1, 1, -1, -1>> const &,
    unsigned long &, bool &);
template void igl::outer_facet<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
