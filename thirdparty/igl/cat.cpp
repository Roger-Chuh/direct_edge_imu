...
IGL_INLINE void igl::cat(const int dim, const Eigen::SparseMatrix<Scalar> &A,
...
IGL_INLINE void igl::cat(const int dim, const Eigen::MatrixBase<Derived> &A,
...
IGL_INLINE void igl::cat(const std::vector<std::vector<Mat>> &A, Mat &C) {
  ...
  C.resize(0, 0);
  for (typename vector<vector<Mat>>::const_iterator rit = A.begin();
       rit != A.end(); rit++) {
    // Concatenate each row horizontally
    // Start with empty matrix
    Mat row(0, 0);
    for (typename vector<vector<Mat>>::iterator cit = A.begin(); rit != A.end();
         rit++) {
      row = cat(2, row, *cit);
    }
...
igl::cat<Eigen::Matrix<double, -1, -1, 0, -1, -1>>(
    int, Eigen::Matrix<double, -1, -1, 0, -1, -1> const &,
    Eigen::Matrix<double, -1, -1, 0, -1, -1> const &);
// generated by autoexplicit.sh
template Eigen::SparseMatrix<double, 0, int>
igl::cat<Eigen::SparseMatrix<double, 0, int>>(
    int, Eigen::SparseMatrix<double, 0, int> const &,
    Eigen::SparseMatrix<double, 0, int> const &);
// generated by autoexplicit.sh
template Eigen::Matrix<int, -1, -1, 0, -1, -1>
igl::cat<Eigen::Matrix<int, -1, -1, 0, -1, -1>>(
    int, Eigen::Matrix<int, -1, -1, 0, -1, -1> const &,
    Eigen::Matrix<int, -1, -1, 0, -1, -1> const &);
template void igl::cat<Eigen::Matrix<double, -1, 1, 0, -1, 1>,
