...
template <typename DerivedV, typename DerivedT, typename Derivedplane,
          typename DerivedU, typename DerivedG, typename DerivedJ,
          typename BCType>
IGL_INLINE void igl::slice_tets(
    const Eigen::PlainObjectBase<DerivedV> &V,
    const Eigen::PlainObjectBase<DerivedT> &T,
    const Eigen::PlainObjectBase<Derivedplane> &plane,
    Eigen::PlainObjectBase<DerivedU> &U, Eigen::PlainObjectBase<DerivedG> &G,
    Eigen::PlainObjectBase<DerivedJ> &J, Eigen::SparseMatrix<BCType> &BC) {
  ...
  VectorXS IV =
      (V.col(0) * plane(0) + V.col(1) * plane(1) + V.col(2) * plane(2))
          .array() +
      plane(3);
  MatrixX4S IT(m, 4);
  ...
  const auto &extract_rows = [](const PlainObjectBase<DerivedT> &T,
                                const MatrixX4S &IT, const VectorXb &I,
                                MatrixX4I &TI, MatrixX4S &ITI, VectorXI &JI) {
  ...
  const auto &apply_sort = [](const MatrixX4I &T, const MatrixX4I &sJ,
                              MatrixX4I &sT) {
    sT.resize(T.rows(), 4);
    for (size_t t = 0; t < (size_t)T.rows(); t++) {
      for (size_t c = 0; c < 4; c++) {
  ...
  const auto &one_below = [&V, &apply_sort](const MatrixX4I &T,
                                            const MatrixX4S &IT, MatrixX3I &G,
    ...
    MatrixX4I sT;
    apply_sort(T, sJ, sT);
    MatrixX3S lambda = sIT.rightCols(3).array() /
    ...
    for (size_t c = 0; c < 3; c++) {
      for (size_t t = 0; t < (size_t)m; t++) {
        ...
        IJV.push_back(
            Triplet<BCType>(c * m + t, sT(t, c + 1), 1 - lambda(t, c)));
      }
    }
  ...
  const auto &two_below = [&V, &apply_sort](const MatrixX4I &T,
                                            const MatrixX4S &IT, MatrixX3I &G,
                                            SparseMatrix<BCType> &BC) {
    // Number of tets
    const size_t m = T.rows();
    ...
    MatrixX4I sT;
    apply_sort(T, sJ, sT);
    MatrixX2S lambda = sIT.rightCols(2).array() /
                       (sIT.rightCols(2).colwise() - sIT.col(0)).array();
    MatrixX2S gamma = sIT.rightCols(2).array() /
    ...
    for (size_t c = 0; c < 2; c++) {
      for (size_t t = 0; t < (size_t)m; t++) {
        IJV.push_back(
            Triplet<BCType>(0 * 2 * m + c * m + t, sT(t, 0), lambda(t, c)));
        IJV.push_back(Triplet<BCType>(0 * 2 * m + c * m + t, sT(t, c + 2),
                                      1 - lambda(t, c)));
        IJV.push_back(
            Triplet<BCType>(1 * 2 * m + c * m + t, sT(t, 1), gamma(t, c)));
        IJV.push_back(Triplet<BCType>(1 * 2 * m + c * m + t, sT(t, c + 2),
                                      1 - gamma(t, c)));
      }
    }
  ...
  G << G13, (G31.array() + BC13.rows()),
      (G22.array() + BC13.rows() + BC31.rows());
  MatrixX3S N;
...
template void igl::slice_tets<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
    ...
    Eigen::PlainObjectBase<Eigen::Matrix<double, 4, 1, 0, 4, 1>> const &,
