...
IGL_INLINE void igl::polyvector_field_comb_from_matchings_and_cuts(
  ...
  Eigen::MatrixXi used;
  used.setConstant(F.rows(), half_degree, -1);
  ...
  while (!d.empty()) {
    ...
    for (int k = 0; k < 3; k++) {
      ...
      // check its orientation to determine whether match_ab or match_ba should
      // be used
      if ((E2F(current_edge, 0) == f0) && (E2F(current_edge, 1) == f1)) {
        // look at match_ab
        for (int i = 0; i < half_degree; ++i) {
          int ii = used(f0, i);
          used(f1, i) = (match_ab(current_edge, ii % half_degree) +
                         (ii >= half_degree) * half_degree) %
                        full_degree;
        }
      } else {
        assert((E2F(current_edge, 1) == f0) && (E2F(current_edge, 0) == f1));
        // look at match_ba
        for (int i = 0; i < half_degree; ++i) {
          int ii = used(f0, i);
          used(f1, i) = (match_ba(current_edge, ii % half_degree) +
                         (ii >= half_degree) * half_degree) %
                        full_degree;
        }
      }
      ...
      for (int i = 0; i < half_degree; ++i) {
        ...
        sol3D_combed.block(f1, i * 3, 1, 3) =
            sign * sol3D.block(f1, (used(f1, i) % half_degree) * 3, 1, 3);
      }
      d.push_back(f1);
    }
  }
...
IGL_INLINE void igl::polyvector_field_comb_from_matchings_and_cuts(
  ...
  igl::polyvector_field_comb_from_matchings_and_cuts(
      V, F, TT, E2F, F2E, sol3D, match_ab, match_ba, cuts, sol3D_combed);
}

#ifdef IGL_STATIC_LIBRARY
// Explicit template specialization
template void igl::polyvector_field_comb_from_matchings_and_cuts<
