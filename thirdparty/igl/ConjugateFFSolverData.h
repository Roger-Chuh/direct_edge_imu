...
template <typename DerivedV, typename DerivedF> class ConjugateFFSolverData {
public:
...
public:
  ...
  IGL_INLINE void evaluateConjugacy(
      const Eigen::Matrix<typename DerivedV::Scalar, Eigen::Dynamic, 2> &pvU,
...
IGL_INLINE
igl::ConjugateFFSolverData<DerivedV, DerivedF>::ConjugateFFSolverData(
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE void
igl::ConjugateFFSolverData<DerivedV,
                           DerivedF>::computeCurvatureAndPrincipals() {
  ...
  for (int i = 0; i < numF; ++i) {
    if (kmin[i] != kmin[i] || kmax[i] != kmax[i] ||
        (dmin3.row(i).array() != dmin3.row(i).array()).any() ||
        (dmax3.row(i).array() != dmax3.row(i).array()).any()) {
      kmin[i] = 0;
      kmax[i] = 0;
      dmin3.row(i) = B1.row(i);
      dmax3.row(i) = B2.row(i);
    } else {
      dmax3.row(i) = (dmax3.row(i) - (dmax3.row(i).dot(FN.row(i))) * FN.row(i))
                         .normalized();
      dmin3.row(i) = dmin3.row(i) - (dmin3.row(i).dot(FN.row(i))) * FN.row(i);
      dmin3.row(i) =
          (dmin3.row(i) - (dmin3.row(i).dot(dmax3.row(i))) * dmax3.row(i))
              .normalized();
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE void
igl::ConjugateFFSolverData<DerivedV, DerivedF>::precomputeConjugacyStuff() {
  H.resize(numF);
  UH.resize(numF);
  s.resize(numF);
  ...
  for (int i = 0; i < numF; ++i) {
    ...
    typename DerivedV::Scalar e1x = dmin(i, 0), e1y = dmin(i, 1),
                              e2x = dmax(i, 0), e2y = dmax(i, 1), k1 = kmin[i],
                              k2 = kmax[i];

    H[i] << 0, 0, k1 * e1x * e1x, k1 * e1x * e1y, 0, 0, k1 * e1x * e1y,
        k1 * e1y * e1y, k2 * e2x * e2x, k2 * e2x * e2y, 0, 0, k2 * e2x * e2y,
        k2 * e2y * e2y, 0, 0;
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE void
igl::ConjugateFFSolverData<DerivedV, DerivedF>::computeLaplacians() {
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE void
igl::ConjugateFFSolverData<DerivedV, DerivedF>::precomputeInteriorEdges() {
  // Flag border edges
  numInteriorEdges = 0;
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE void
igl::ConjugateFFSolverData<DerivedV, DerivedF>::computeCoefficientLaplacian(
    int n, Eigen::SparseMatrix<std::complex<typename DerivedV::Scalar>> &D) {
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE void igl::ConjugateFFSolverData<DerivedV, DerivedF>::computek() {
  K.setZero(numE);
  // For every non-border edge
  for (unsigned eid = 0; eid < numE; ++eid) {
    if (!isBorderEdge[eid]) {
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE void
igl::ConjugateFFSolverData<DerivedV, DerivedF>::evaluateConjugacy(
    const Eigen::Matrix<typename DerivedV::Scalar, Eigen::Dynamic, 2> &pvU,
    const Eigen::Matrix<typename DerivedV::Scalar, Eigen::Dynamic, 2> &pvV,
    Eigen::Matrix<typename DerivedV::Scalar, Eigen::Dynamic, 1> &conjValues)
    const {
  ...
  for (int j = 0; j < numF; ++j) {
    ...
    x << pvU.row(j).transpose(), pvV.row(j).transpose();
