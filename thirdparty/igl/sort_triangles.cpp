...
template <typename DerivedV, typename DerivedF, typename DerivedMV,
          typename DerivedP, typename DerivedFF, typename DerivedI>
IGL_INLINE void igl::sort_triangles(const Eigen::PlainObjectBase<DerivedV> &V,
  ...
  // Barycenter, centroid
  Eigen::Matrix<typename DerivedV::Scalar, DerivedF::RowsAtCompileTime, 1> D,
      sD;
  Eigen::Matrix<typename DerivedV::Scalar, DerivedF::RowsAtCompileTime, 4> BC,
      PBC;
  barycenter(V, F, BC);
  ...
  //// Closest corner
  // Eigen::Matrix<typename DerivedV::Scalar, DerivedF::RowsAtCompileTime,1>
  // D,sD; D.setConstant(F.rows(),1,-1e26); for(int c = 0;c<3;c++)
  //{
  //  Eigen::Matrix<typename DerivedV::Scalar, DerivedF::RowsAtCompileTime,4> C;
  //  Eigen::Matrix<typename DerivedV::Scalar, DerivedF::RowsAtCompileTime,1>
  //  DC; C.resize(F.rows(),4); for(int f = 0;f<F.rows();f++)
  //  {
  //    C(f,0) = V(F(f,c),0);
  //    C(f,1) = V(F(f,c),1);
  //    C(f,2) = V(F(f,c),2);
  ...
  // D,sD,ssD; D.resize(F.rows(),3); for(int c = 0;c<3;c++)
  //{
  //  Eigen::Matrix<typename DerivedV::Scalar, DerivedF::RowsAtCompileTime,4> C;
  //  C.resize(F.rows(),4);
...
//    static bool seg_seg_intersect(const Vec3 &  a1, const Vec3 &  a2, const
//    Vec3 &  b1, const Vec3 &  b2)
//    {
//      Vec3 b = a2-a1;
//      Vec3 d = b2-b1;
...
//      intersection points if (bDotDPerp == 0)
//        return false;
//
//      Vec3 c = b1-a1;
...
template void igl::sort_triangles<
    ...
    Eigen::Matrix<int, -1, -1, 0, -1, -1>, Eigen::Matrix<double, 4, 4, 0, 4, 4>,
    Eigen::Matrix<double, 4, 4, 0, 4, 4>, Eigen::Matrix<int, -1, -1, 0, -1, -1>,
    Eigen::Matrix<int, -1, 1, 0, -1, 1>>(
    Eigen::PlainObjectBase<Eigen::Matrix<double, -1, 4, 0, -1, 4>> const &,
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, -1, 0, -1, -1>> const &,
    Eigen::PlainObjectBase<Eigen::Matrix<double, 4, 4, 0, 4, 4>> const &,
    Eigen::PlainObjectBase<Eigen::Matrix<double, 4, 4, 0, 4, 4>> const &,
