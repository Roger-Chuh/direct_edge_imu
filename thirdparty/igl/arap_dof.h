...
template <typename LbsMatrixType, typename SSCALAR> struct ArapDOFData;
...
IGL_INLINE bool
arap_dof_precomputation(const Eigen::MatrixXd &V, const Eigen::MatrixXi &F,
...
template <typename LbsMatrixType, typename SSCALAR>
IGL_INLINE bool
arap_dof_recomputation(const Eigen::Matrix<int, Eigen::Dynamic, 1> &fixed_dim,
...
template <typename LbsMatrixType, typename SSCALAR>
IGL_INLINE bool
arap_dof_update(const ArapDOFData<LbsMatrixType, SSCALAR> &data,
                const Eigen::Matrix<double, Eigen::Dynamic, 1> &B_eq,
                const Eigen::MatrixXd &L0, const int max_iters,
                const double tol, Eigen::MatrixXd &L);
...
template <typename LbsMatrixType, typename SSCALAR> struct ArapDOFData {
  ...
  // Full solve matrix that contains also conversion from rotations to the right
  // hand side, i.e., solves Poisson transformations just from rotations and
  ...
  // Default values
  ArapDOFData()
      : energy(igl::ARAP_ENERGY_TYPE_SPOKES), with_dynamics(false), h(1),
        grav_dir(0, -1, 0), grav_mag(0) {}
};
} // namespace igl
