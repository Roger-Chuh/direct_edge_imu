...
IGL_INLINE void igl::arap_rhs(const Eigen::MatrixXd &V,
                              const Eigen::MatrixXi &F, const int dim,
  ...
  case ARAP_ENERGY_TYPE_ELEMENTS:
    ...
    break;
  default:
    fprintf(stderr, "arap_rhs.h: Error: Unsupported arap energy %d\n", energy);
    return;
  }
  ...
  if (Vdim == 2) {
    ...
    if (dim == 3) {
      ...
      K = cat(
          2, cat(2, cat(2, repdiag(KX, dim), ZZ), cat(2, repdiag(KY, dim), ZZ)),
          cat(2, repdiag(KZ, dim), ZZ));
    } else {
      assert(false);
      fprintf(stderr, "arap_rhs.h: Error: Unsupported dimension %d\n", dim);
    }
  } else {
    assert(false);
    fprintf(stderr, "arap_rhs.h: Error: Unsupported dimension %d\n", Vdim);
