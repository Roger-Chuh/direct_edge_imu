...
namespace igl {
template <typename DerivedV, typename DerivedF> class PolyVectorFieldFinder {
private:
  ...
  IGL_INLINE void setFieldFromGeneralCoefficients(
      const std::vector<Eigen::Matrix<std::complex<typename DerivedV::Scalar>,
                                      Eigen::Dynamic, 1>> &coeffs,
      std::vector<Eigen::Matrix<typename DerivedV::Scalar, Eigen::Dynamic, 2>>
          &pv);
  IGL_INLINE void computeCoefficientLaplacian(
      int n, Eigen::SparseMatrix<std::complex<typename DerivedV::Scalar>> &D);
  IGL_INLINE void getGeneralCoeffConstraints(
      const Eigen::VectorXi &isConstrained,
      const Eigen::Matrix<typename DerivedV::Scalar, Eigen::Dynamic,
                          Eigen::Dynamic> &cfW,
      int k,
...
IGL_INLINE
igl::PolyVectorFieldFinder<DerivedV, DerivedF>::PolyVectorFieldFinder(
    const Eigen::PlainObjectBase<DerivedV> &_V,
    const Eigen::PlainObjectBase<DerivedF> &_F, const int &_n)
    : V(_V), F(_F), numF(_F.rows()), n(_n) {
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE void
igl::PolyVectorFieldFinder<DerivedV, DerivedF>::precomputeInteriorEdges() {
  // Flag border edges
  numInteriorEdges = 0;
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE void
igl::PolyVectorFieldFinder<DerivedV, DerivedF>::minQuadWithKnownMini(
    const Eigen::SparseMatrix<std::complex<typename DerivedV::Scalar>> &Q,
    const Eigen::SparseMatrix<std::complex<typename DerivedV::Scalar>> &f,
    const Eigen::VectorXi isConstrained,
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE bool igl::PolyVectorFieldFinder<DerivedV, DerivedF>::solve(
    const Eigen::VectorXi &isConstrained,
    const Eigen::Matrix<typename DerivedV::Scalar, Eigen::Dynamic,
                        Eigen::Dynamic> &cfW,
    Eigen::Matrix<typename DerivedV::Scalar, Eigen::Dynamic, Eigen::Dynamic>
  ...
  std::vector<
      Eigen::Matrix<std::complex<typename DerivedV::Scalar>, Eigen::Dynamic, 1>>
      coeffs(n, Eigen::Matrix<std::complex<typename DerivedV::Scalar>,
                              Eigen::Dynamic, 1>::Zero(numF, 1));
  ...
  for (int i = 0; i < n; ++i) {
    ...
    Eigen::Matrix<std::complex<typename DerivedV::Scalar>, Eigen::Dynamic, 1>
        Ck;
    getGeneralCoeffConstraints(isConstrained, cfW, i, Ck);
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE void
igl::PolyVectorFieldFinder<DerivedV, DerivedF>::setFieldFromGeneralCoefficients(
    const std::vector<Eigen::Matrix<std::complex<typename DerivedV::Scalar>,
                                    Eigen::Dynamic, 1>> &coeffs,
    std::vector<Eigen::Matrix<typename DerivedV::Scalar, Eigen::Dynamic, 2>>
        &pv) {
  pv.assign(
      n, Eigen::Matrix<typename DerivedV::Scalar, Eigen::Dynamic, 2>::Zero(numF,
                                                                           2));
  for (int i = 0; i < numF; ++i) {
    ...
    Eigen::Matrix<std::complex<typename DerivedV::Scalar>, Eigen::Dynamic, 1>
        roots;
    igl::polyRoots<std::complex<typename DerivedV::Scalar>,
                   typename DerivedV::Scalar>(polyCoeff, roots);

    Eigen::VectorXi done;
    done.setZero(2 * n, 1);

    Eigen::Matrix<std::complex<typename DerivedV::Scalar>, Eigen::Dynamic, 1> u(
        n, 1);
    int ind = 0;
    for (int k = 0; k < 2 * n; ++k) {
      if (done[k])
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE void
igl::PolyVectorFieldFinder<DerivedV, DerivedF>::computeCoefficientLaplacian(
    int n, Eigen::SparseMatrix<std::complex<typename DerivedV::Scalar>> &D) {
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE void
igl::PolyVectorFieldFinder<DerivedV, DerivedF>::getGeneralCoeffConstraints(
    const Eigen::VectorXi &isConstrained,
    const Eigen::Matrix<typename DerivedV::Scalar, Eigen::Dynamic,
                        Eigen::Dynamic> &cfW,
    int k,
    ...
    if (isConstrained[fi]) {
      ...
      for (int j = 0; j < allCombs.rows(); ++j) {
        ...
        for (int i = 0; i < allCombs.cols(); ++i) {
          ...
          const Eigen::Matrix<typename DerivedV::Scalar, 1, 3> &w =
              cfW.block(fi, 3 * index, 1, 3);
