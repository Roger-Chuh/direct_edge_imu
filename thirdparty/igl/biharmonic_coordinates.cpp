...
template <typename DerivedV, typename DerivedT, typename SType,
          typename DerivedW>
IGL_INLINE bool
igl::biharmonic_coordinates(const Eigen::PlainObjectBase<DerivedV> &V,
...
igl::biharmonic_coordinates(const Eigen::PlainObjectBase<DerivedV> &V,
                            const Eigen::PlainObjectBase<DerivedT> &T,
                            const std::vector<std::vector<SType>> &S,
                            const int k, Eigen::PlainObjectBase<DerivedW> &W) {
  ...
  const size_t mp = count_if(
      S.begin(), S.end(), [](const vector<int> &h) { return h.size() == 1; });
  ...
  // minimize    ½ W' A W'
  // subject to  W(b,:) = J
  return min_quad_with_fixed(A, VectorXd::Zero(A.rows()).eval(), b, J,
                             SparseMatrix<double>(), VectorXd(), true, W);
}

#ifdef IGL_STATIC_LIBRARY
// Explicit template specialization
template bool
igl::biharmonic_coordinates<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
