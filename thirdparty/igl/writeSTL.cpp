...
IGL_INLINE bool igl::writeSTL(const std::string &filename,
  ...
  if (ascii) {
    ...
    if (stl_file == NULL) {
      cerr << "IOError: " << filename << " could not be opened for writing."
           << endl;
      return false;
    }
    ...
    for (int f = 0; f < F.rows(); f++) {
      ...
      for (int c = 0; c < F.cols(); c++) {
        fprintf(stl_file, "vertex %e %e %e\n", (float)V(F(f, c), 0),
                (float)V(F(f, c), 1), (float)V(F(f, c), 2));
      }
...
template bool igl::writeSTL<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
