...
public:
  ...
  // Return an Affine transformation (rigid actually) that takes a world 3d
  // coordinate and transforms it into the relative camera coordinates.
  ...
  inline void look_at(const Eigen::Vector3d &eye, const Eigen::Vector3d &at,
...
inline igl::Camera::Camera()
    : m_angle(45.0), m_aspect(1), m_near(1e-2), m_far(100), m_at_dist(1),
      m_orthographic(false), m_rotation_conj(1, 0, 0, 0),
      m_translation(0, 0, 1) {}
...
inline Eigen::Matrix4d igl::Camera::projection() const {
  ...
  if (m_orthographic) {
    ...
    P << z_fix * 2. / (right - left), 0, 0, -tx, 0, z_fix * 2. / (top - bottom),
        0, -ty, 0, 0, -z_fix * 2. / (far - near), -tz, 0, 0, 0, 1;
  } else {
    ...
    P << xScale, 0, 0, 0, 0, yScale, 0, 0, 0, 0, -(far + near) / (far - near),
        -1, 0, 0, -2. * near * far / (far - near), 0;
    P = P.transpose().eval();
  }
  return P;
}
...
inline Eigen::Vector3d igl::Camera::unit_plane() const {
  ...
  const double w = 2. *
                   sqrt(-d * d / (a * a * pow(tan(0.5 * theta), 2.) - 1.)) * a *
                   tan(0.5 * theta);
  const double h = w / a;
...
inline void igl::Camera::dolly_zoom(const double da) {
  ...
  if (!m_orthographic) {
    ...
    // change in distance
    const double s = (2. * tan(old_angle / 2. / 180. * M_PI)) /
...
inline void igl::Camera::orbit(const Eigen::Quaterniond &q) {
  ...
  m_translation =
      m_rotation_conj *
      (old_at + m_rotation_conj.conjugate() * Vector3d(0, 0, 1) * m_at_dist);
  assert((old_at - at()).squaredNorm() < DOUBLE_EPS);
}

inline void igl::Camera::look_at(const Eigen::Vector3d &eye,
  ...
  // cout<<"eye()-at():
  // "<<(this->eye()-this->at()).normalized().transpose()<<endl;
