...
IGL_INLINE bool igl::writePLY(const std::string &filename,
  ...
  PlyFile *ply =
      ply_write(fp, 2, elem_names, (ascii ? PLY_ASCII : PLY_BINARY_LE));
  if (ply == NULL) {
    return false;
  }
  ...
  ply_describe_element(ply, "vertex", V.rows(), plist.size(), &plist[0]);
...
template bool igl::writePLY<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
    ...
    bool);
template bool igl::writePLY<Eigen::Matrix<double, -1, 3, 0, -1, 3>,
                            Eigen::Matrix<int, -1, 3, 0, -1, 3>>(
    std::basic_string<char, std::char_traits<char>, std::allocator<char>> const
        &,
    Eigen::PlainObjectBase<Eigen::Matrix<double, -1, 3, 0, -1, 3>> const &,
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, 3, 0, -1, 3>> const &, bool);
#endif
