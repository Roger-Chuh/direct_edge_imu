...
IGL_INLINE void igl::signed_distance(const Eigen::MatrixXd &P,
                                     const Eigen::MatrixXd &V,
                                     const Eigen::MatrixXi &F,
                                     const SignedDistanceType sign_type,
                                     Eigen::VectorXd &S, Eigen::VectorXi &I,
                                     Eigen::MatrixXd &C, Eigen::MatrixXd &N) {
  using namespace Eigen;
  using namespace std;
  const int dim = V.cols();
  assert((V.cols() == 3 || V.cols() == 2) &&
         "V should have 3d or 2d positions");
  assert((P.cols() == 3 || V.cols() == 2) &&
         "P should have 3d or 2d positions");
  ...
  case SIGNED_DISTANCE_TYPE_PSEUDONORMAL:
    ...
    case 3:
      ...
      per_edge_normals(V, F, PER_EDGE_NORMALS_WEIGHTING_TYPE_UNIFORM, FN, EN, E,
                       EMAP);
      break;
    case 2:
      FN.resize(F.rows(), 2);
  ...
  for (int p = 0; p < P.rows(); p++) {
    ...
    case SIGNED_DISTANCE_TYPE_UNSIGNED:
      s = 1.;
      sqrd = dim == 3 ? tree3.squared_distance(V, F, q3, i, c3)
                      : tree2.squared_distance(V, F, q2, i, c2);
      break;
    case SIGNED_DISTANCE_TYPE_DEFAULT:
    case SIGNED_DISTANCE_TYPE_WINDING_NUMBER:
      dim == 3
          ? signed_distance_winding_number(tree3, V, F, hier3, q3, s, sqrd, i,
                                           c3)
    ...
    case SIGNED_DISTANCE_TYPE_PSEUDONORMAL: {
      RowVector3d n3;
      RowVector2d n2;
      dim == 3 ? signed_distance_pseudonormal(tree3, V, F, FN, VN, EN, EMAP, q3,
                                              s, sqrd, i, c3, n3)
               : signed_distance_pseudonormal(tree2, V, F, FN, VN, q2, s, sqrd,
                                              i, c2, n2);
      Eigen::RowVectorXd n;
      (dim == 3 ? n = n3 : n = n3);
      N.row(p) = n;
...
IGL_INLINE double igl::signed_distance_pseudonormal(
    const AABB<Eigen::MatrixXd, 3> &tree, const Eigen::MatrixXd &V,
    const Eigen::MatrixXi &F, const Eigen::MatrixXd &FN,
    const Eigen::MatrixXd &VN, const Eigen::MatrixXd &EN,
    const Eigen::VectorXi &EMAP, const Eigen::RowVector3d &q) {
  double s, sqrd;
  Eigen::RowVector3d n, c;
  int i = -1;
  signed_distance_pseudonormal(tree, V, F, FN, VN, EN, EMAP, q, s, sqrd, i, c,
                               n);
  return s * sqrt(sqrd);
}
...
IGL_INLINE void igl::signed_distance_pseudonormal(
    ...
    const Eigen::MatrixXi &F, const AABB<Eigen::MatrixXd, 3> &tree,
    const Eigen::MatrixXd &FN, const Eigen::MatrixXd &VN,
    const Eigen::MatrixXd &EN, const Eigen::VectorXi &EMAP, Eigen::VectorXd &S,
    Eigen::VectorXi &I, Eigen::MatrixXd &C, Eigen::MatrixXd &N) {
  using namespace Eigen;
...
IGL_INLINE void igl::signed_distance_pseudonormal(
    ...
    const Eigen::MatrixXi &F, const Eigen::MatrixXd &FN,
    const Eigen::MatrixXd &VN, const Eigen::MatrixXd &EN,
    const Eigen::VectorXi &EMAP, const Eigen::RowVector3d &q, double &s,
    double &sqrd, int &f, Eigen::RowVector3d &c, Eigen::RowVector3d &n) {
  using namespace Eigen;
  using namespace std;
...
IGL_INLINE void igl::signed_distance_pseudonormal(
    const AABB<Eigen::MatrixXd, 2> &tree, const Eigen::MatrixXd &V,
    const Eigen::MatrixXi &F, const Eigen::MatrixXd &FN,
    const Eigen::MatrixXd &VN, const Eigen::RowVector2d &q, double &s,
    double &sqrd, int &f, Eigen::RowVector2d &c, Eigen::RowVector2d &n) {
  using namespace Eigen;
  using namespace std;
  sqrd = tree.squared_distance(V, F, q, f, c);
...
IGL_INLINE void igl::signed_distance_winding_number(
    ...
    const Eigen::Matrix<double, 1, 3> &q, double &s, double &sqrd, int &i,
...
IGL_INLINE void igl::signed_distance_winding_number(
    const AABB<Eigen::MatrixXd, 2> &tree, const Eigen::MatrixXd &V,
    const Eigen::MatrixXi &F, const Eigen::Matrix<double, 1, 2> &q, double &s,
    double &sqrd, int &i, Eigen::Matrix<double, 1, 2> &c) {
  using namespace Eigen;
  using namespace std;
  using namespace igl;
  sqrd = tree.squared_distance(V, F, q, i, c);
