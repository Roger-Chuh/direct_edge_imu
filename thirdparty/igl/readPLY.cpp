...
IGL_INLINE bool igl::readPLY(const std::string &filename,
  ...
  if (plist != NULL) {
    ...
    for (int j = 0; j < nprops; j++) {
      ...
      if (equal_strings("nx", prop->name) || equal_strings("ny", prop->name) ||
      ...
      if (equal_strings("s", prop->name) || equal_strings("t", prop->name)) {
...
IGL_INLINE bool igl::readPLY(const std::string &filename,
  ...
  return list_to_matrix(vV, V) && list_to_matrix(vF, F) &&
         list_to_matrix(vN, N) && list_to_matrix(vUV, UV);
}
...
template bool igl::readPLY<double, int, double, double>(
