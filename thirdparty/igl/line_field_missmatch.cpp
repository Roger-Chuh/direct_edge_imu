...
private:
  ...
  inline int MissMatchByLine(const int f0, const int f1) {
...
public:
  ...
  inline void
  calculateMissmatchLine(Eigen::PlainObjectBase<DerivedO> &Handle_MMatch) {
...
template <typename DerivedV, typename DerivedF, typename DerivedO>
IGL_INLINE void
igl::line_field_missmatch(const Eigen::PlainObjectBase<DerivedV> &V,
                          const Eigen::PlainObjectBase<DerivedF> &F,
                          const Eigen::PlainObjectBase<DerivedV> &PD1,
                          const bool isCombed,
  ...
  PD2_combed = igl::rotate_vectors(
      PD1_combed, Eigen::VectorXd::Constant(1, M_PI / 2), B1, B2);
  igl::MissMatchCalculatorLine<DerivedV, DerivedF, DerivedO> sf(
      V, F, PD1_combed, PD2_combed);
