...
public:
  ...
  inline void init(const PointMatrixType &V, const FaceMatrixType &F);
  ...
  // Side effects:
  //   The first time this is ever called the embree engine is initialized.
  inline void init(const std::vector<const PointMatrixType *> &V,
  ...
  inline bool intersectRay(const Eigen::RowVector3f &origin,
                           const Eigen::RowVector3f &direction, Hit &hit,
                           float tnear = 0,
  ...
  //   tfar       end of ray segment
  //   masks      a 32 bit mask to identify active geometries.
  //   geoId      id of geometry mask (default
  //   std::numeric_limits<float>::infinity() if no: no masking) closestHit true
  //   for gets closest hit, false for furthest hit
  // Output:
  //   hit        information about hit
  // Returns true if and only if there was a hit
  inline bool intersectBeam(const Eigen::RowVector3f &origin,
                            const Eigen::RowVector3f &direction, Hit &hit,
                            float tnear = 0,
                            float tfar = std::numeric_limits<float>::infinity(),
                            int mask = 0xFFFFFFFF, int geoId = -1,
  ...
  inline bool intersectRay(const Eigen::RowVector3f &origin,
                           ...
                           std::vector<Hit> &hits, int &num_rays,
                           float tnear = 0,
                           float tfar = std::numeric_limits<float>::infinity(),
                           int mask = 0xFFFFFFFF) const;
  ...
  //   hit  information about hit
  // Returns true if and only if there was a hit
  inline bool intersectSegment(const Eigen::RowVector3f &a,
                               const Eigen::RowVector3f &ab, Hit &hit,
                               int mask = 0xFFFFFFFF) const;
...
private:
  ...
  inline void createRay(RTCRay &ray, const Eigen::RowVector3f &origin,
                        const Eigen::RowVector3f &direction, float tnear,
                        float tfar, int mask) const;
};
} // namespace embree
} // namespace igl
...
inline void igl::embree::EmbreeIntersector::global_init() {
  if (!EmbreeIntersector_inited) {
    ...
    if (rtcGetError() != RTC_NO_ERROR)
      std::cerr << "Embree: An error occured while initialiting embree core!"
                << std::endl;
#ifdef IGL_VERBOSE
    else
...
inline igl::embree::EmbreeIntersector::EmbreeIntersector()
    : // scene(NULL),
      geomID(0), triangles(NULL), vertices(NULL), initialized(false) {}
...
inline igl::embree::EmbreeIntersector &
igl::embree::EmbreeIntersector::operator=(const EmbreeIntersector &) {
  assert(false && "Embree: Assigning an EmbreeIntersector is not allowed");
  return *this;
}

inline void igl::embree::EmbreeIntersector::init(const PointMatrixType &V,
...
inline void igl::embree::EmbreeIntersector::init(
  ...
  // create a scene
  scene =
      rtcNewScene(RTC_SCENE_ROBUST | RTC_SCENE_HIGH_QUALITY, RTC_INTERSECT1);

  for (int g = 0; g < (int)V.size(); g++) {
    // create triangle mesh geometry in that scene
    geomID = rtcNewTriangleMesh(scene, RTC_GEOMETRY_STATIC, F[g]->rows(),
                                V[g]->rows(), 1);
  ...
  if (rtcGetError() != RTC_NO_ERROR)
    std::cerr
        << "Embree: An error occured while initializing the provided geometry!"
        << endl;
#ifdef IGL_VERBOSE
  else
...
inline bool igl::embree::EmbreeIntersector::intersectRay(
    const Eigen::RowVector3f &origin, const Eigen::RowVector3f &direction,
    Hit &hit, float tnear, float tfar, int mask) const {
  RTCRay ray;
...
inline bool igl::embree::EmbreeIntersector::intersectBeam(
    const Eigen::RowVector3f &origin, const Eigen::RowVector3f &direction,
    Hit &hit, float tnear, float tfar, int mask, int geoId, bool closestHit,
  ...
  if ((intersectRay(origin, direction, hit, tnear, tfar, mask) &&
  ...
  Eigen::Matrix3f rot =
      Eigen::AngleAxis<float>(2 * 3.14159265358979 / samples, direction)
          .toRotationMatrix();

  for (int r = 0; r < (int)samples; r++) {
    if (intersectRay(origin + offset * eps, direction, hit, tnear, tfar,
                     mask) &&
...
inline bool igl::embree::EmbreeIntersector ::intersectRay(
    const Eigen::RowVector3f &origin, const Eigen::RowVector3f &direction,
    std::vector<Hit> &hits, int &num_rays, float tnear, float tfar,
    int mask) const {
  using namespace std;
  num_rays = 0;
  hits.clear();
...
inline bool
igl::embree::EmbreeIntersector ::intersectSegment(const Eigen::RowVector3f &a,
                                                  const Eigen::RowVector3f &ab,
                                                  Hit &hit, int mask) const {
  RTCRay ray;
...
inline void igl::embree::EmbreeIntersector ::createRay(
    RTCRay &ray, const Eigen::RowVector3f &origin,
    const Eigen::RowVector3f &direction, float tnear, float tfar,
    int mask) const {
  ray.org[0] = origin[0];
  ray.org[1] = origin[1];
  ray.org[2] = origin[2];
