...
template <typename DerivedV, typename DerivedF, typename DerivedSD,
          typename Derivedflag>
IGL_INLINE void
igl::embree::bone_visible(const Eigen::PlainObjectBase<DerivedV> &V,
...
#pragma omp parallel for
  ...
  for (int v = 0; v < V.rows(); v++) {
    ...
    if (sd_norm < DOUBLE_EPS) {
      ...
      project_to_line(Vv(0), Vv(1), Vv(2), s(0), s(1), s(2), d(0), d(1), d(2),
    ...
    if (ei.intersectSegment(projv.template cast<float>(),
                            dir.template cast<float>(), hit)) {
...
template void
igl::embree::bone_visible<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
