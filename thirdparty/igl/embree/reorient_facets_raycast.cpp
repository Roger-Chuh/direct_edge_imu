...
IGL_INLINE void igl::embree::reorient_facets_raycast(
    ...
    const Eigen::PlainObjectBase<DerivedF> &F, int rays_total, int rays_minimum,
    bool facet_wise, bool use_parity, bool is_verbose,
    Eigen::PlainObjectBase<DerivedI> &I, Eigen::PlainObjectBase<DerivedC> &C) {
  ...
  for (int c = 0; c < num_cc; ++c) {
    num_rays_per_component(c) = max<int>(
        static_cast<int>(rays_total * area_per_component(c) / area_total),
        rays_minimum);
  }
  ...
  for (int c = 0; c < num_cc; ++c) {
    ...
    // discrete distribution for random selection of faces with probability
    // proportional to their areas
    discrete_distribution<int> ddist(CF.size(), 0, CF.size(), [&](double i) {
      return CF_area[static_cast<int>(i)];
    }); // simple ctor of (Iter, Iter) not provided by the stupid VC11/12
    for (int i = 0; i < num_rays_per_component[c]; ++i) {
      int f = CF[ddist(
          prng)]; // select face with probability proportional to face area
      float s = rdist(
          prng); // random barycentric coordinate (reference: Generating Random
                 // Points in Triangles [Turk, Graphics Gems I 1990])
      float t = rdist(prng);
      float sqrt_t = sqrtf(t);
      float a = 1 - sqrt_t;
      float b = (1 - s) * sqrt_t;
      float c = s * sqrt_t;
      Vector3f p = a * V.row(FF(f, 0))
                           .template cast<float>()
                           .eval() // be careful with the index!!!
      ...
      if (is_verbose && ray_face.size() % (rays_total / 10) == 0)
        cout << ".";
    }
  }
  if (is_verbose)
    cout << ray_face.size() << " rays. ";

  // per component voting: first=front, second=back
  vector<pair<float, float>> C_vote_distance(
      num_cc,
      make_pair(0, 0)); // sum of distance between ray origin and intersection
  vector<pair<int, int>> C_vote_infinity(
      num_cc, make_pair(0, 0)); // number of rays reaching infinity
  vector<pair<int, int>> C_vote_parity(
      num_cc, make_pair(0, 0)); // sum of parity count for each ray
...
#pragma omp parallel for
  for (int i = 0; i < (int)ray_face.size(); ++i) {
    ...
    if (!hits_front.empty() && hits_front[0].id == f)
      hits_front.erase(hits_front.begin());
    if (!hits_back.empty() && hits_back[0].id == f)
      hits_back.erase(hits_back.begin());
...
#pragma omp atomic
  ...
  for (int f = 0; f < m; ++f) {
    int c = C(f);
    if (use_parity) {
      I(f) = C_vote_parity[c].first > C_vote_parity[c].second
                 ? 1
                 : 0; // Ideally, parity for the front/back side should be 1/0
                      // (i.e., parity sum for all rays should be smaller on the
                      // front side)

    } else {
      I(f) = (C_vote_infinity[c].first == C_vote_infinity[c].second &&
...
template <typename DerivedV, typename DerivedF, typename DerivedFF,
          typename DerivedI>
IGL_INLINE void
igl::embree::reorient_facets_raycast(const Eigen::PlainObjectBase<DerivedV> &V,
  ...
  Eigen::VectorXi C;
  reorient_facets_raycast(V, F, rays_total, rays_minimum, facet_wise,
                          use_parity, is_verbose, I, C);
...
template void
igl::embree::reorient_facets_raycast<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
