...
IGL_INLINE void serialization(bool s, igl::viewer::ViewerData &obj,
...
template <>
IGL_INLINE void serialize(const igl::viewer::ViewerData &obj,
                          std::vector<char> &buffer) {
  serialization(true, const_cast<igl::viewer::ViewerData &>(obj), buffer);
}

template <>
IGL_INLINE void deserialize(igl::viewer::ViewerData &obj,
...
IGL_INLINE igl::viewer::ViewerData::ViewerData() : dirty(DIRTY_ALL) {
  clear();
};
...
IGL_INLINE void igl::viewer::ViewerData::set_mesh(const Eigen::MatrixXd &_V,
  ...
  if (V.rows() == 0 && F.rows() == 0) {
    ...
    if (_V.rows() == V.rows() && _F.rows() == F.rows()) {
      ...
      cerr << "ERROR (set_mesh): The new mesh has a different number of "
              "vertices/faces. Please clear the mesh before plotting."
           << endl;
  }
...
IGL_INLINE void
igl::viewer::ViewerData::set_vertices(const Eigen::MatrixXd &_V) {
  V = _V;
...
IGL_INLINE void igl::viewer::ViewerData::set_normals(const Eigen::MatrixXd &N) {
  ...
  if (N.rows() == V.rows()) {
    ...
    cerr << "ERROR (set_normals): Please provide a normal per face, per corner "
            "or per vertex."
         << endl;
  dirty |= DIRTY_NORMAL;
}
...
IGL_INLINE void igl::viewer::ViewerData::set_colors(const Eigen::MatrixXd &C) {
  ...
  const auto ambient = [](const MatrixXd &C) -> MatrixXd { return 0.1 * C; };
  ...
  if (C.rows() == 1) {
    ...
    cerr << "ERROR (set_colors): Please provide a single color, or a color per "
            "face or per vertex."
         << endl;
  ;
  dirty |= DIRTY_DIFFUSE;
...
IGL_INLINE void igl::viewer::ViewerData::set_uv(const Eigen::MatrixXd &UV_V,
...
IGL_INLINE void igl::viewer::ViewerData::set_points(const Eigen::MatrixXd &P,
...
IGL_INLINE void igl::viewer::ViewerData::add_points(const Eigen::MatrixXd &P,
                                                    const Eigen::MatrixXd &C) {
  ...
  for (unsigned i = 0; i < P_temp.rows(); ++i)
    points.row(lastid + i) << P_temp.row(i),
        i < C.rows() ? C.row(i) : C.row(C.rows() - 1);

  dirty |= DIRTY_OVERLAY_POINTS;
}

IGL_INLINE void igl::viewer::ViewerData::set_edges(const Eigen::MatrixXd &P,
...
IGL_INLINE void igl::viewer::ViewerData::add_edges(const Eigen::MatrixXd &P1,
  ...
  for (unsigned i = 0; i < P1_temp.rows(); ++i)
    lines.row(lastid + i) << P1_temp.row(i), P2_temp.row(i),
        i < C.rows() ? C.row(i) : C.row(C.rows() - 1);

  dirty |= DIRTY_OVERLAY_LINES;
}

IGL_INLINE void igl::viewer::ViewerData::add_label(const Eigen::VectorXd &P,
...
IGL_INLINE void
igl::viewer::ViewerData::uniform_colors(Eigen::Vector3d ambient,
