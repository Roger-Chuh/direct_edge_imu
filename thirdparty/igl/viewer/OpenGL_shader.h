...
public:
  ...
  IGL_INLINE OpenGL_shader()
      : vertex_shader(0), fragment_shader(0), geometry_shader(0),
        program_shader(0) {}
  ...
  IGL_INLINE bool
  init_from_files(const std::string &vertex_shader_filename,
  ...
  // Return the OpenGL handle of a named shader attribute (-1 if it does not
  // exist)
  ...
  // amtrix
  IGL_INLINE GLint bindVertexAttribArray(const std::string &name,
                                         GLuint bufferID,
                                         const Eigen::MatrixXf &M,
                                         bool refresh) const;

  IGL_INLINE GLuint create_shader_helper(GLint type,
