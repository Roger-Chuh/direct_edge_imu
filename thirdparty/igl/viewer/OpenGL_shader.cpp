...
IGL_INLINE bool igl::viewer::OpenGL_shader::init_from_files(
  ...
  return init(file_to_string(vertex_shader_filename),
              file_to_string(fragment_shader_filename), fragment_data_name,
              file_to_string(geometry_shader_filename),
              geometry_shader_max_vertices);
}

IGL_INLINE bool
igl::viewer::OpenGL_shader::init(const std::string &vertex_shader_string,
  ...
  geometry_shader =
      create_shader_helper(GL_GEOMETRY_SHADER, geometry_shader_string);
  fragment_shader =
      create_shader_helper(GL_FRAGMENT_SHADER, fragment_shader_string);
  ...
  if (geometry_shader) {
    ...
    glProgramParameteri(program_shader, GL_GEOMETRY_VERTICES_OUT,
                        geometry_shader_max_vertices);
  }
...
IGL_INLINE GLint
igl::viewer::OpenGL_shader::attrib(const std::string &name) const {
  return glGetAttribLocation(program_shader, name.c_str());
}

IGL_INLINE GLint
igl::viewer::OpenGL_shader::uniform(const std::string &name) const {
  return glGetUniformLocation(program_shader, name.c_str());
}

IGL_INLINE GLint igl::viewer::OpenGL_shader::bindVertexAttribArray(
    const std::string &name, GLuint bufferID, const Eigen::MatrixXf &M,
    bool refresh) const {
...
IGL_INLINE GLuint igl::viewer::OpenGL_shader::create_shader_helper(
    GLint type, const std::string &shader_string) {
  using namespace std;
