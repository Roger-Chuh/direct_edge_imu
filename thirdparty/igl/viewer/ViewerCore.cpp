...
IGL_INLINE void serialization(bool s, igl::viewer::ViewerCore &obj,
...
template <>
IGL_INLINE void serialize(const igl::viewer::ViewerCore &obj,
                          std::vector<char> &buffer) {
  serialization(true, const_cast<igl::viewer::ViewerCore &>(obj), buffer);
}

template <>
IGL_INLINE void deserialize(igl::viewer::ViewerCore &obj,
...
IGL_INLINE void
igl::viewer::ViewerCore::align_camera_center(const Eigen::MatrixXd &V,
...
IGL_INLINE void igl::viewer::ViewerCore::get_scale_and_shift_to_fit_mesh(
    const Eigen::MatrixXd &V, const Eigen::MatrixXi &F, float &zoom,
...
IGL_INLINE void igl::viewer::ViewerCore::get_scale_and_shift_to_fit_mesh(
    const Eigen::MatrixXd &V, float &zoom, Eigen::Vector3f &shift) {
  if (V.rows() == 0)
    return;
...
IGL_INLINE void igl::viewer::ViewerCore::draw(ViewerData &data,
                                              OpenGL_state &opengl,
  ...
  if (update_matrices) {
    ...
    if (orthographic) {
      ...
      ortho(-h * width / height, h * width / height, -h, h, camera_dnear,
            camera_dfar, proj);
    } else {
  ...
  GLint light_position_worldi =
      opengl.shader_mesh.uniform("light_position_world");
  ...
  if (data.V.rows() > 0) {
    ...
    if (show_lines) {
      ...
      glUniform4f(fixed_colori, line_color[0], line_color[1], line_color[2],
                  1.0f);
    ...
    if (show_vertid) {
      ...
      for (int i = 0; i < data.V.rows(); ++i)
        textrenderer.DrawText(data.V.row(i), data.V_normals.row(i),
                              to_string(i));
  ...
  if (show_overlay) {
    ...
    if (data.labels_positions.rows() > 0) {
      ...
      for (int i = 0; i < data.labels_positions.rows(); ++i)
        textrenderer.DrawText(data.labels_positions.row(i),
...
IGL_INLINE void igl::viewer::ViewerCore::draw_buffer(
    ViewerData &data, OpenGL_state &opengl, bool update_matrices,
...
IGL_INLINE void igl::viewer::ViewerCore::init() { textrenderer.Init(); }

IGL_INLINE void igl::viewer::ViewerCore::shut() { textrenderer.Shut(); }
