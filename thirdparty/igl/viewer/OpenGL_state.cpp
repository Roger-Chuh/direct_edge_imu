...
IGL_INLINE void
igl::viewer::OpenGL_state::set_data(const igl::viewer::ViewerData &data,
                                    bool invert_normals) {
  ...
  if (!data.face_based) {
    if (!per_corner_uv) {
      ...
      if (dirty & ViewerData::DIRTY_POSITION) {
        ...
        for (unsigned i = 0; i < data.F.rows(); ++i)
          for (unsigned j = 0; j < 3; ++j)
            V_vbo.col(i * 3 + j) =
                data.V.row(data.F(i, j)).transpose().cast<float>();
      }
      ...
      if (dirty & ViewerData::DIRTY_AMBIENT) {
        ...
        for (unsigned i = 0; i < data.F.rows(); ++i)
          for (unsigned j = 0; j < 3; ++j)
            V_ambient_vbo.col(i * 3 + j) =
                data.V_material_ambient.row(data.F(i, j))
                    .transpose()
                    .cast<float>();
      }
      ...
      if (dirty & ViewerData::DIRTY_DIFFUSE) {
        ...
        for (unsigned i = 0; i < data.F.rows(); ++i)
          for (unsigned j = 0; j < 3; ++j)
            V_diffuse_vbo.col(i * 3 + j) =
                data.V_material_diffuse.row(data.F(i, j))
                    .transpose()
                    .cast<float>();
      }
      ...
      if (dirty & ViewerData::DIRTY_SPECULAR) {
        ...
        for (unsigned i = 0; i < data.F.rows(); ++i)
          for (unsigned j = 0; j < 3; ++j)
            V_specular_vbo.col(i * 3 + j) =
                data.V_material_specular.row(data.F(i, j))
                    .transpose()
                    .cast<float>();
      }
      ...
      if (dirty & ViewerData::DIRTY_UV) {
        ...
        for (unsigned i = 0; i < data.F.rows(); ++i)
          for (unsigned j = 0; j < 3; ++j)
            V_uv_vbo.col(i * 3 + j) =
                data.V_uv.row(data.F(i, j)).transpose().cast<float>();
      }
    }
  } else {
    if (dirty & ViewerData::DIRTY_POSITION) {
    ...
    if (dirty & ViewerData::DIRTY_AMBIENT) {
      ...
      for (unsigned i = 0; i < data.F.rows(); ++i)
        for (unsigned j = 0; j < 3; ++j)
          V_ambient_vbo.col(i * 3 + j) =
              data.F_material_ambient.row(i).transpose().cast<float>();
    }
    ...
    if (dirty & ViewerData::DIRTY_DIFFUSE) {
      ...
      for (unsigned i = 0; i < data.F.rows(); ++i)
        for (unsigned j = 0; j < 3; ++j)
          V_diffuse_vbo.col(i * 3 + j) =
              data.F_material_diffuse.row(i).transpose().cast<float>();
    }
    ...
    if (dirty & ViewerData::DIRTY_SPECULAR) {
      ...
      for (unsigned i = 0; i < data.F.rows(); ++i)
        for (unsigned j = 0; j < 3; ++j)
          V_specular_vbo.col(i * 3 + j) =
              data.F_material_specular.row(i).transpose().cast<float>();
    }
    ...
    if (dirty & ViewerData::DIRTY_UV) {
      ...
      for (unsigned i = 0; i < data.F.rows(); ++i)
        for (unsigned j = 0; j < 3; ++j)
          V_uv_vbo.col(i * 3 + j) =
              data.V_uv.row(per_corner_uv ? data.F_uv(i, j) : data.F(i, j))
                  .transpose()
                  .cast<float>();
    }
  }
  ...
  if (dirty & ViewerData::DIRTY_OVERLAY_LINES) {
    ...
    for (unsigned i = 0; i < data.lines.rows(); ++i) {
      lines_V_vbo.col(2 * i + 0) =
          data.lines.block<1, 3>(i, 0).transpose().cast<float>();
      lines_V_vbo.col(2 * i + 1) =
          data.lines.block<1, 3>(i, 3).transpose().cast<float>();
      lines_V_colors_vbo.col(2 * i + 0) =
          data.lines.block<1, 3>(i, 6).transpose().cast<float>();
      lines_V_colors_vbo.col(2 * i + 1) =
          data.lines.block<1, 3>(i, 6).transpose().cast<float>();
  ...
  if (dirty & ViewerData::DIRTY_OVERLAY_POINTS) {
    ...
    for (unsigned i = 0; i < data.points.rows(); ++i) {
      points_V_vbo.col(i) =
          data.points.block<1, 3>(i, 0).transpose().cast<float>();
      points_V_colors_vbo.col(i) =
          data.points.block<1, 3>(i, 3).transpose().cast<float>();
      points_F_vbo(i) = i;
    }
  }
}
...
IGL_INLINE void igl::viewer::OpenGL_state::bind_mesh() {
  glBindVertexArray(vao_mesh);
  shader_mesh.bind();
  shader_mesh.bindVertexAttribArray("position", vbo_V, V_vbo,
                                    dirty & ViewerData::DIRTY_POSITION);
  shader_mesh.bindVertexAttribArray("normal", vbo_V_normals, V_normals_vbo,
                                    dirty & ViewerData::DIRTY_NORMAL);
  shader_mesh.bindVertexAttribArray("Ka", vbo_V_ambient, V_ambient_vbo,
                                    dirty & ViewerData::DIRTY_AMBIENT);
  shader_mesh.bindVertexAttribArray("Kd", vbo_V_diffuse, V_diffuse_vbo,
                                    dirty & ViewerData::DIRTY_DIFFUSE);
  shader_mesh.bindVertexAttribArray("Ks", vbo_V_specular, V_specular_vbo,
                                    dirty & ViewerData::DIRTY_SPECULAR);
  shader_mesh.bindVertexAttribArray("texcoord", vbo_V_uv, V_uv_vbo,
                                    dirty & ViewerData::DIRTY_UV);
  ...
  if (dirty & ViewerData::DIRTY_TEXTURE) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, tex_u, tex_v, 0, GL_RGB,
...
IGL_INLINE void igl::viewer::OpenGL_state::bind_overlay_lines() {
  ...
  shader_overlay_lines.bindVertexAttribArray("position", vbo_lines_V,
                                             lines_V_vbo, is_dirty);
  shader_overlay_lines.bindVertexAttribArray("color", vbo_lines_V_colors,
...
IGL_INLINE void igl::viewer::OpenGL_state::bind_overlay_points() {
  ...
  shader_overlay_points.bindVertexAttribArray("position", vbo_points_V,
                                              points_V_vbo, is_dirty);
  shader_overlay_points.bindVertexAttribArray("color", vbo_points_V_colors,
...
IGL_INLINE void igl::viewer::OpenGL_state::init() {
  std::string mesh_vertex_shader_string =
      ...
      "  gl_Position = proj * vec4 (position_eye, 1.0);" // proj * view
                                                         // * model *
                                                         // vec4(position,
                                                         // 1.0);"
      "  Kai = Ka;"
  ...
  std::string mesh_fragment_shader_string =
      ...
      "vec3 light_position_eye = vec3 (view * vec4 (light_position_world, "
      "1.0));"
      ...
      "dot_prod_specular = float(abs(dot_prod)==dot_prod) * max "
      ...
      "vec4((1.0-lighting_factor) * Kdi,1.0);"
      "outColor = mix(vec4(1,1,1,1), texture(tex, texcoordi), texture_factor) "
      "* color;"
  ...
  init_buffers();
  shader_mesh.init(mesh_vertex_shader_string, mesh_fragment_shader_string,
                   "outColor");
