...
static void glfw_mouse_press(GLFWwindow *window, int button, int action,
                             int modifier) {
  bool tw_used =
...
static void glfw_key_callback(GLFWwindow *window, int key, int scancode,
  ...
  if (__viewer->screen->keyCallbackEvent(key, scancode, action, modifier) ==
      false) {
    if (action == GLFW_PRESS)
...
static void glfw_mouse_move(GLFWwindow *window, double x, double y) {
  if (__viewer->screen->cursorPosCallbackEvent(x, y) == false ||
      __viewer->down) {
...
IGL_INLINE void Viewer::init() {
  ...
  nanogui::Window *window =
      ngui->addWindow(Eigen::Vector2i(10, 10), "libIGL-Viewer");
  ...
  ngui->addVariable<bool>(
      "Face-based", [&](bool checked) { this->data.set_face_based(checked); },
...
IGL_INLINE bool Viewer::load_mesh_from_file(const char *mesh_file_name) {
  ...
  if (extension == "off" || extension == "OFF") {
    ...
    if (!(igl::readOBJ(mesh_file_name_string, V, UV_V, corner_normals, F, UV_F,
                       fNormIndices)))
      return false;
...
IGL_INLINE bool Viewer::save_mesh_to_file(const char *mesh_file_name) {
  ...
  if (extension == "off" || extension == "OFF") {
    ...
    return igl::writeOBJ(mesh_file_name_string, data.V, data.F, corner_normals,
                         fNormIndices, UV_V, UV_F);
  } else {
    // unrecognized file type
    printf("Error: %s is not a recognized file type.\n", extension.c_str());
    return false;
...
IGL_INLINE bool Viewer::mouse_down(MouseButton button, int modifier) {
  ...
  Eigen::Vector3f coord =
      igl::project(Eigen::Vector3f(center(0), center(1), center(2)),
                   (core.view * core.model).eval(), core.proj, core.viewport);
  ...
  if (down) {
    ...
    case MouseMode::Rotation: {
      ...
      case ViewerCore::ROTATION_TYPE_TRACKBALL:
        igl::trackball(core.viewport(2), core.viewport(3), 2.0f, down_rotation,
      ...
      case ViewerCore::ROTATION_TYPE_TWO_AXIS_VALUATOR_FIXED_UP:
        igl::two_axis_valuator_fixed_up(
            core.viewport(2), core.viewport(3), 2.0, down_rotation,
            down_mouse_x, down_mouse_y, mouse_x, mouse_y, core.trackball_angle);
        break;
      }
    ...
    case MouseMode::Translation: {
      // translation
      Eigen::Vector3f pos1 = igl::unproject(
          Eigen::Vector3f(mouse_x, core.viewport[3] - mouse_y, down_mouse_z),
          (core.view * core.model).eval(), core.proj, core.viewport);
      Eigen::Vector3f pos0 = igl::unproject(
          Eigen::Vector3f(down_mouse_x, core.viewport[3] - down_mouse_y,
                          down_mouse_z),
          (core.view * core.model).eval(), core.proj, core.viewport);

      Eigen::Vector3f diff = pos1 - pos0;
      core.model_translation =
          down_translation + Eigen::Vector3f(diff[0], diff[1], diff[2]);
...
IGL_INLINE bool Viewer::mouse_scroll(float delta_y) {
  ...
  if (delta_y != 0) {
    ...
    core.camera_zoom =
        (core.camera_zoom * mult > min_zoom ? core.camera_zoom * mult
                                            : min_zoom);
  }
  return true;
}
...
IGL_INLINE int Viewer::launch_init(bool resizable, bool fullscreen) {
  ...
  if (fullscreen) {
    ...
    window = glfwCreateWindow(mode->width, mode->height, "libigl viewer",
                              monitor, nullptr);
  } else {
