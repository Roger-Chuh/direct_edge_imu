...
public:
  ...
  IGL_INLINE void set_uv(const Eigen::MatrixXd &UV_V,
  ...
  //   C  #P|1 by 3 color(s)
  IGL_INLINE void set_points(const Eigen::MatrixXd &P,
                             const Eigen::MatrixXd &C);
  IGL_INLINE void add_points(const Eigen::MatrixXd &P,
                             const Eigen::MatrixXd &C);
  ...
  //   C  #E|1 by 3 color(s)
  IGL_INLINE void set_edges(const Eigen::MatrixXd &P, const Eigen::MatrixXi &E,
                            const Eigen::MatrixXd &C);
  IGL_INLINE void add_edges(const Eigen::MatrixXd &P1,
  ...
  // (Every row contains 9 doubles in the following format S_x, S_y, S_z, T_x,
  // T_y, T_z, C_r, C_g, C_b), with S and T the coordinates of the two vertices
  // of the line in global coordinates, and C the color in floating point rgb
  // format
  Eigen::MatrixXd lines;

  // Points plotted over the scene
  // (Every row contains 6 doubles in the following format P_x, P_y, P_z, C_r,
  // C_g, C_b), with P the position in global coordinates of the center of the
  // point, and C the color in floating point rgb format
  Eigen::MatrixXd points;

  // Text labels plotted over the scene
  // Textp contains, in the i-th row, the position in global coordinates where
  // the i-th label should be anchored Texts contains in the i-th position the
  // text of the i-th label
