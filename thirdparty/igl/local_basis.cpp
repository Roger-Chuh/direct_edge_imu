...
IGL_INLINE void igl::local_basis(const Eigen::PlainObjectBase<DerivedV> &V,
  ...
  for (unsigned i = 0; i < F.rows(); ++i) {
    ...
    Eigen::Matrix<typename DerivedV::Scalar, 1, 3> t =
        V.row(F(i, 2)) - V.row(F(i, 0));
    Eigen::Matrix<typename DerivedV::Scalar, 1, 3> v3 =
        v1.cross(t).normalized();
    Eigen::Matrix<typename DerivedV::Scalar, 1, 3> v2 =
        v1.cross(v3).normalized();
