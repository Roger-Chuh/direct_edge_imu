...
namespace igl {
// Applies a two-axis valuator drag rotation (as seen in Maya/Studio max) to a
// given rotation. Inputs:
...
IGL_INLINE void
two_axis_valuator_fixed_up(const int w, const int h, const double speed,
                           const Eigen::Quaternion<Scalardown_quat> &down_quat,
                           const int down_x, const int down_y,
