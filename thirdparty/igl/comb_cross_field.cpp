...
namespace igl {
template <typename DerivedV, typename DerivedF> class Comb {
public:
...
private:
  ...
  K_PI_new(const Eigen::Matrix<typename DerivedV::Scalar, 3, 1> &a,
           const Eigen::Matrix<typename DerivedV::Scalar, 3, 1> &b,
           const Eigen::Matrix<typename DerivedV::Scalar, 3, 1> &n) {
    Eigen::Matrix<typename DerivedV::Scalar, 3, 1> c =
        (a.cross(n)).normalized();
...
IGL_INLINE void
igl::comb_cross_field(const Eigen::PlainObjectBase<DerivedV> &V,
                      const Eigen::PlainObjectBase<DerivedF> &F,
                      const Eigen::PlainObjectBase<DerivedV> &PD1,
                      const Eigen::PlainObjectBase<DerivedV> &PD2,
...
template void igl::comb_cross_field<Eigen::Matrix<double, -1, 3, 0, -1, 3>,
    ...
    Eigen::PlainObjectBase<Eigen::Matrix<double, -1, 3, 0, -1, 3>> &);
template void igl::comb_cross_field<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
