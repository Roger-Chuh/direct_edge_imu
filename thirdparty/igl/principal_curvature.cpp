...
public:
  ...
  public:
    ...
    IGL_INLINE static Quadric fit(std::vector<Eigen::Vector3d> &VV,
                                  bool zeroDetCheck, bool svd) {
      using namespace std;
      assert(VV.size() >= 5);
      if (VV.size() < 5) {
...
public:
  ...
  IGL_INLINE void fitQuadric(Eigen::Vector3d, std::vector<Eigen::Vector3d> ref,
                             const std::vector<int> &, Quadric *);
  IGL_INLINE void applyProjOnPlane(Eigen::Vector3d, std::vector<int>,
                                   std::vector<int> &);
  ...
  IGL_INLINE Eigen::Vector3d project(Eigen::Vector3d, Eigen::Vector3d,
                                     Eigen::Vector3d);
  IGL_INLINE void computeReferenceFrame(int, Eigen::Vector3d,
  ...
  IGL_INLINE static void rotateBackward(int nr, float *v0, float *v1,
                                        float *v2) {
    float t;
...
IGL_INLINE void
CurvatureCalculator::fitQuadric(Eigen::Vector3d v,
...
IGL_INLINE void
CurvatureCalculator::finalEigenStuff(int i, std::vector<Eigen::Vector3d> ref,
                                     Quadric q) {
...
IGL_INLINE void CurvatureCalculator::getSphere(const int start, const double r,
  ...
  std::priority_queue<std::pair<int, double>,
                      ...
                      comparer> *extra_candidates =
      new std::priority_queue<std::pair<int, double>,
...
IGL_INLINE Eigen::Vector3d CurvatureCalculator::project(Eigen::Vector3d v,
                                                        Eigen::Vector3d vp,
                                                        Eigen::Vector3d ppn) {
...
IGL_INLINE void
CurvatureCalculator::computeReferenceFrame(int i, Eigen::Vector3d normal,
                                           std::vector<Eigen::Vector3d> &ref) {
...
IGL_INLINE void CurvatureCalculator::getProjPlane(int j, std::vector<int> vv,
                                                  Eigen::Vector3d &ppn) {
  int nr;
  float a, b, c;
  float nx, ny, nz;
...
IGL_INLINE void CurvatureCalculator::applyProjOnPlane(Eigen::Vector3d ppn,
                                                      std::vector<int> vin,
                                                      std::vector<int> &vout) {
...
IGL_INLINE void CurvatureCalculator::applyMontecarlo(std::vector<int> &vin,
                                                     std::vector<int> *vout) {
...
igl::principal_curvature(const Eigen::PlainObjectBase<DerivedV> &V,
  ...
  if (radius < 2) {
    radius = 2;
    cout << "WARNING: igl::principal_curvature needs a radius >= 2, fixing it "
            "to 2."
         << endl;
  }
