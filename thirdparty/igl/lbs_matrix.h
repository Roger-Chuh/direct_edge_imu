...
IGL_INLINE void lbs_matrix(const Eigen::MatrixXd &V, const Eigen::MatrixXd &W,
                           Eigen::MatrixXd &M);
...
IGL_INLINE void lbs_matrix_column(const Eigen::MatrixXd &V,
                                  const Eigen::MatrixXd &W,
                                  Eigen::SparseMatrix<double> &M);
IGL_INLINE void lbs_matrix_column(const Eigen::MatrixXd &V,
                                  const Eigen::MatrixXd &W, Eigen::MatrixXd &M);
...
//     W(j,WI(i)) gives the ith most significant correspondence weight on vertex
//     j
// Output:
//   M  #V * dim by #handles * dim * (dim+1) matrix such that
//     new_V(:) = LBS(V,W,A) = reshape(M * A,size(V)), where A is a column
