...
TwType ReTwDefineEnum(const char *name, const TwEnumVal *enumValues,
...
struct ReTwCBItem {
  ...
  void *clientData;
  // Default constructor
  IGL_INLINE ReTwCBItem(const std::string _name, TwType _type,
                        TwSetVarCallback _setCallback,
                        TwGetVarCallback _getCallback, void *_clientData)
      : name(_name), type(_type), setCallback(_setCallback),
        getCallback(_getCallback), clientData(_clientData) {}
  // Shallow copy
  // I solemnly swear it's OK to copy clientData this way
  IGL_INLINE ReTwCBItem(const ReTwCBItem &that)
      : name(that.name), type(that.type), setCallback(that.setCallback),
        getCallback(that.getCallback), clientData(that.clientData) {}
  // Shallow assignment
  // I solemnly swear it's OK to copy clientData this way
...
public:
  ...
  IGL_INLINE int TwAddVarRW(const char *name, TwType type, void *var,
                            const char *def, const bool record = true);
  IGL_INLINE int TwAddVarCB(const char *name, TwType type,
                            TwSetVarCallback setCallback,
                            TwGetVarCallback getCallback, void *clientData,
                            const char *def, const bool record = true);
  // Wrappers for convenience (not recorded, just passed on)
  IGL_INLINE int TwAddVarRO(const char *name, TwType type, void *var,
                            const char *def);
  IGL_INLINE int TwAddButton(const char *name, TwButtonCallback buttonCallback,
                             void *clientData, const char *def);
  IGL_INLINE int TwSetParam(const char *varName, const char *paramName,
                            TwParamValueType paramValueType,
                            unsigned int inValueCount, const void *inValues);
  IGL_INLINE int TwGetParam(const char *varName, const char *paramName,
                            TwParamValueType paramValueType,
...
public:
  ...
  // What can you do...
  IGL_INLINE bool set_value_from_string(const char *name, TwType type,
...
//
// TW_API int      TW_CALL TwAddVarRW(TwBar *bar, const char *name, TwType type,
// void *var, const char *def); TW_API int      TW_CALL TwAddVarRO(TwBar *bar,
// const char *name, TwType type, const void *var, const char *def); TW_API int
// TW_CALL TwAddVarCB(TwBar *bar, const char *name, TwType type,
// TwSetVarCallback setCallback, TwGetVarCallback getCallback, void *clientData,
// const char *def); TW_API int      TW_CALL TwAddButton(TwBar *bar, const char
// *name, TwButtonCallback callback, void *clientData, const char *def); TW_API
// int TW_CALL TwAddSeparator(TwBar *bar, const char *name, const char *def);
// TW_API int      TW_CALL TwRemoveVar(TwBar *bar, const char *name); TW_API int
// TW_CALL TwRemoveAllVars(TwBar *bar);
