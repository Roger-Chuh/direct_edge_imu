...
// GLOBAL WRAPPERS
namespace {
std::map<TwType, std::pair<const char *, std::vector<TwEnumVal>>>
    ReTw_custom_types;
}
...
IGL_INLINE TwType igl::anttweakbar::ReTwDefineEnumFromString(
    const char *_Name, const char *_EnumString) {
  ...
  using namespace std;
  {
    ...
    const TwType type = ReTwDefineEnum(_Name, Vals.empty() ? NULL : &(Vals[0]),
...
IGL_INLINE
igl::anttweakbar::ReTwBar::ReTwBar(const igl::anttweakbar::ReTwBar &that)
    : bar(that.bar), name(that.name), rw_items(that.rw_items),
...
IGL_INLINE int igl::anttweakbar::ReTwBar::TwAddVarCB(
    const char *name, TwType type, TwSetVarCallback setCallback,
    TwGetVarCallback getCallback, void *clientData, const char *def,
    const bool record) {
  int ret = ::TwAddVarCB(this->bar, name, type, setCallback, getCallback,
                         clientData, def);
  if (ret && record) {
    cb_items.push_back(
        ReTwCBItem(name, type, setCallback, getCallback, clientData));
  }
  return ret;
}
...
IGL_INLINE int
igl::anttweakbar::ReTwBar::TwAddButton(const char *name,
                                       TwButtonCallback buttonCallback,
                                       void *clientData, const char *def) {
  int ret = ::TwAddButton(this->bar, name, buttonCallback, clientData, def);
...
IGL_INLINE int igl::anttweakbar::ReTwBar::TwSetParam(
    const char *varName, const char *paramName, TwParamValueType paramValueType,
    unsigned int inValueCount, const void *inValues) {
  // For now just pass these along
  return ::TwSetParam(this->bar, varName, paramName, paramValueType,
                      inValueCount, inValues);
}

IGL_INLINE int igl::anttweakbar::ReTwBar::TwGetParam(
    const char *varName, const char *paramName, TwParamValueType paramValueType,
    unsigned int outValueMaxCount, void *outValues) {
  return ::TwGetParam(this->bar, varName, paramName, paramValueType,
                      outValueMaxCount, outValues);
}
...
IGL_INLINE bool igl::anttweakbar::ReTwBar::save(const char *file_name) {
  ...
  for (std::vector<ReTwRWItem>::iterator it = rw_items.begin();
       it != rw_items.end(); it++) {
    std::string s = (*it).name;
    const char *name = s.c_str();
    TwType type = (*it).type;
    void *var = (*it).var;
    fprintf(fp, "%s: %s\n", name, get_value_as_string(var, type).c_str());
  }

  char var[REANTTWEAKBAR_MAX_CB_VAR_SIZE];
  // Print all CB variables
  for (std::vector<ReTwCBItem>::iterator it = cb_items.begin();
       it != cb_items.end(); it++) {
...
IGL_INLINE std::string
igl::anttweakbar::ReTwBar::get_value_as_string(void *var, TwType type) {
  ...
  case TW_TYPE_QUAT4D: {
    ...
    sstr << std::setprecision(15) << q[0] << " " << q[1] << " " << q[2] << " "
         << q[3];
    break;
  }
  case TW_TYPE_QUAT4F: {
  ...
  default: {
    using namespace std;
    std::map<TwType,
             std::pair<const char *, std::vector<TwEnumVal>>>::const_iterator
        iter = ReTw_custom_types.find(type);
    if (iter != ReTw_custom_types.end()) {
      sstr << (*iter).second.first << " ";
      int enum_val = *(static_cast<int *>(var));
      // try find display name for enum value
      std::vector<TwEnumVal>::const_iterator eit =
...
IGL_INLINE bool
igl::anttweakbar::ReTwBar::type_from_string(const char *type_str,
                                            TwType &type) {
...
bool igl::anttweakbar::ReTwBar::set_value_from_string(const char *name,
  ...
  case TW_TYPE_QUAT4D:
    ...
    {
      if (sscanf(value_str, " %lf %lf %lf %lf", &d[0], &d[1], &d[2], &d[3]) ==
          4) {
        value = &d;
      } else {
        printf("ERROR: Bad value format...\n");
  ...
  for (std::vector<ReTwRWItem>::iterator it = rw_items.begin();
    ...
    if (it->name == name) {
      ...
      default:
        // Try to find type in custom enum types
        std::map<TwType,
                 std::pair<const char *, std::vector<TwEnumVal>>>::iterator
            iter = ReTw_custom_types.find(type);
        if (iter != ReTw_custom_types.end()) {
          int *ivar = static_cast<int *>(var);
