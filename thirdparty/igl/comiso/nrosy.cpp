...
public:
  ...
  // roundseparately: round the integer variables one at a time, slower but
  // higher quality
  ...
  // v: direction to fix (in 3d)
  IGL_INLINE void setConstraintSoft(const int fid, const double w,
                                    const Eigen::Vector3d &v);

  // Set the ratio between smoothness and soft constraints (0 -> smoothness
  // only, 1 -> soft constr only)
  IGL_INLINE void setSoftAlpha(double alpha);

  // Reset constraints (at least one constraint must be present or solve will
  // fail)
...
igl::comiso::NRosyField::NRosyField(const Eigen::MatrixXd &_V,
...
void igl::comiso::NRosyField::prepareSystemMatrix(const int N) {
  ...
  for (unsigned r = 0; r < id_p.size(); ++r) {
    ...
    if (!isFixed_i) {
      ...
      if (isFixed_p)
        b(row) += -((4 * M_PI) / Nd) * p[eid];
      else
        T.push_back(Eigen::Triplet<double>(row, tag_p[eid], ((4 * M_PI) / Nd)));
      b(row) += -2 * k[eid];
    ...
    if (!isFixed_j) {
      ...
      if (isFixed_p)
        ...
        T.push_back(
            Eigen::Triplet<double>(row, tag_p[eid], -((4 * M_PI) / Nd)));
      b(row) += 2 * k[eid];
      assert(k[eid] == k[eid]);
      assert(b(row) == b(row));
    }
    ...
    if (!isFixed_p) {
      row = tag_p[eid];
      if (isFixed_i)
        b(row) += -(4 * M_PI) / Nd * hard[i];
      else
        T.push_back(Eigen::Triplet<double>(row, tag_t[i], (4 * M_PI) / Nd));
      if (isFixed_j)
        b(row) += (4 * M_PI) / Nd * hard[j];
      else
        T.push_back(Eigen::Triplet<double>(row, tag_t[j], -(4 * M_PI) / Nd));
      if (isFixed_p)
        b(row) += -(2 * pow(((2 * M_PI) / Nd), 2)) * p[eid];
      else
        T.push_back(Eigen::Triplet<double>(row, tag_p[eid],
                                           (2 * pow(((2 * M_PI) / Nd), 2))));
...
void igl::comiso::NRosyField::setConstraintHard(const int fid,
                                                const Eigen::Vector3d &v) {
  isHard[fid] = true;
  hard(fid) = convert3DtoLocal(fid, v);
}

void igl::comiso::NRosyField::setConstraintSoft(const int fid, const double w,
                                                const Eigen::Vector3d &v) {
  wSoft(fid) = w;
...
void igl::comiso::NRosyField::reduceSpace() {
  ...
  while (!q.empty()) {
    ...
    for (int i = 0; i < 3; ++i) {
      ...
      // skip borders
      if (fid != -1) {
        assert((EF(eid, 0) == c && EF(eid, 1) == fid) ||
               (EF(eid, 1) == c && EF(eid, 0) == fid));
...
double igl::comiso::NRosyField::convert3DtoLocal(unsigned fid,
                                                 const Eigen::Vector3d &v) {
  using namespace std;
  using namespace Eigen;
...
Eigen::Vector3d igl::comiso::NRosyField::convertLocalto3D(unsigned fid,
                                                          double a) {
  using namespace std;
  using namespace Eigen;
...
void igl::comiso::NRosyField::findCones(int N) {
  // Compute I0, see
  // http://www.graphics.rwth-aachen.de/media/papers/bommes_zimmer_2009_siggraph_011.pdf
  // for details
...
IGL_INLINE void
igl::comiso::nrosy(const Eigen::MatrixXd &V, const Eigen::MatrixXi &F,
                   const Eigen::VectorXi &b, const Eigen::MatrixXd &bc,
                   const Eigen::VectorXi &b_soft, const Eigen::VectorXd &w_soft,
                   const Eigen::MatrixXd &bc_soft, const int N,
                   const double soft, Eigen::MatrixXd &R, Eigen::VectorXd &S) {
  // Init solver
...
IGL_INLINE void igl::comiso::nrosy(const Eigen::MatrixXd &V,
                                   const Eigen::MatrixXi &F,
                                   const Eigen::VectorXi &b,
                                   const Eigen::MatrixXd &bc, const int N,
                                   Eigen::MatrixXd &R, Eigen::VectorXd &S) {
  // Init solver
  igl::comiso::NRosyField solver(V, F);
