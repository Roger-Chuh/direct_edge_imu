...
public:
  // Init
  IGL_INLINE FrameInterpolator(const Eigen::MatrixXd &_V,
  ...
  IGL_INLINE void frame2canonical(const Eigen::MatrixXd &TP,
                                  const Eigen::RowVectorXd &v, double &theta,
                                  Eigen::VectorXd &S);

  // Convert the canonical representation in a frame field
  IGL_INLINE void canonical2frame(const Eigen::MatrixXd &TP, const double theta,
                                  const Eigen::VectorXd &S,
                                  Eigen::RowVectorXd &v);

  IGL_INLINE Eigen::MatrixXd getFieldPerFace();

  IGL_INLINE void PolarDecomposition(Eigen::MatrixXd V, Eigen::MatrixXd &U,
                                     Eigen::MatrixXd &P);
...
private:
  ...
  IGL_INLINE double vector2theta(const Eigen::MatrixXd &TP,
                                 const Eigen::RowVectorXd &v);

  // Convert an angle in a vector in the tangent space
  IGL_INLINE Eigen::RowVectorXd theta2vector(const Eigen::MatrixXd &TP,
                                             const double theta);
...
FrameInterpolator::FrameInterpolator(const Eigen::MatrixXd &_V,
                                     const Eigen::MatrixXi &_F) {
  using namespace std;
...
double FrameInterpolator::vector2theta(const Eigen::MatrixXd &TP,
                                       const Eigen::RowVectorXd &v) {
...
Eigen::RowVectorXd FrameInterpolator::theta2vector(const Eigen::MatrixXd &TP,
                                                   const double theta) {
...
void FrameInterpolator::frame2canonical(const Eigen::MatrixXd &TP,
                                        const Eigen::RowVectorXd &v,
                                        double &theta, Eigen::VectorXd &S_v) {
  using namespace std;
  using namespace Eigen;
...
void FrameInterpolator::canonical2frame(const Eigen::MatrixXd &TP,
                                        const double theta,
                                        const Eigen::VectorXd &S_v,
                                        Eigen::RowVectorXd &v) {
  using namespace std;
  using namespace Eigen;
...
void FrameInterpolator::PolarDecomposition(Eigen::MatrixXd V,
                                           Eigen::MatrixXd &U,
                                           Eigen::MatrixXd &P) {
  using namespace std;
  using namespace Eigen;
  ...
  P = svd.matrixV() * svd.singularValues().asDiagonal() *
...
IGL_INLINE void
igl::comiso::frame_field(const Eigen::MatrixXd &V, const Eigen::MatrixXi &F,
                         const Eigen::VectorXi &b, const Eigen::MatrixXd &bc1,
                         const Eigen::MatrixXd &bc2, Eigen::MatrixXd &FF1,
                         Eigen::MatrixXd &FF2)

{
  ...
  for (unsigned i = 0; i < b.size(); ++i) {
    VectorXd t(6);
    t << bc1.row(i).transpose(), bc2.row(i).transpose();
