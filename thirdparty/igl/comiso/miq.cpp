...
//
// Copyright (C) 2014 Daniele Panozzo <daniele.panozzo@gmail.com>, Olga Diamanti
// <olga.diam@gmail.com>, Kevin Walliman <wkevin@student.ethz.ch>
//
...
template <typename DerivedV, typename DerivedF> class VertexIndexing {
public:
  // Input:
...
private:
  struct VertexInfo {
    ...
    int f0, k0; // face and local edge information of the edge that connects
                // this vertex to the previous vertex (previous in the vector)
    int f1, k1; // face and local edge information of the other face
                // corresponding to the same edge
    VertexInfo(int _v, int _f0, int _k0, int _f1, int _k1)
        : v(_v), f0(_f0), k0(_k0), f1(_f1), k1(_k1) {}
    bool operator==(VertexInfo const &other) { return other.v == v; }
  };

  IGL_INLINE void GetSeamInfo(const int f0, const int f1, const int indexE,
                              int &v0, int &v1, int &v0p, int &v1p,
                              unsigned char &_MMatch);

  IGL_INLINE std::vector<std::vector<VertexInfo>> GetVerticesPerSeam();
};

template <typename DerivedV, typename DerivedF> class PoissonSolver {

public:
  IGL_INLINE void
  SolvePoisson(Eigen::VectorXd Stiffness, double vector_field_scale = 0.1f,
               double grid_res = 1.f, bool direct_round = true,
               int localIter = 0, bool _integer_rounding = true,
...
public:
  IGL_INLINE
  MIQ_class(const Eigen::PlainObjectBase<DerivedV> &V_,
            ...
            Eigen::PlainObjectBase<DerivedF> &FUV, double GradientSize = 30.0,
            double Stiffness = 5.0, bool DirectRound = false, int iter = 5,
...
private:
  ...
  IGL_INLINE double LaplaceDistortion(const int f, double h,
                                      const Eigen::MatrixXd &WUV);

  IGL_INLINE bool
  updateStiffeningJacobianDistorsion(double grad_size,
                                     const Eigen::MatrixXd &WUV);
...
IGL_INLINE igl::comiso::SeamInfo::SeamInfo(int _v0, int _v0p, int _MMatch,
                                           int _integerVar) {
  v0 = _v0;
  v0p = _v0p;
...
IGL_INLINE igl::comiso::VertexIndexing<DerivedV, DerivedF>::VertexIndexing(
    ...
    const Eigen::Matrix<int, Eigen::Dynamic, 1> &_Handle_Singular,
    const Eigen::Matrix<int, Eigen::Dynamic, 3> &_Handle_Seams)
    : V(_V), F(_F), Vcut(_Vcut), Fcut(_Fcut), TT(_TT), TTi(_TTi),
      Handle_MMatch(_Handle_MMatch), Handle_Singular(_Handle_Singular),
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE void igl::comiso::VertexIndexing<DerivedV, DerivedF>::GetSeamInfo(
    const int f0, const int f1, const int indexE, int &v0, int &v1, int &v0p,
    int &v1p, unsigned char &_MMatch) {
  int edgef0 = indexE;
  v0 = Fcut(f0, edgef0);
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE std::vector<std::vector<
    typename igl::comiso::VertexIndexing<DerivedV, DerivedF>::VertexInfo>>
igl::comiso::VertexIndexing<DerivedV, DerivedF>::GetVerticesPerSeam() {
  // Return value
  std::vector<std::vector<VertexInfo>> verticesPerSeam;

  // for every vertex, keep track of their adjacent vertices on seams.
  // regular vertices have two neighbors on a seam, start- and endvertices may
  // have any other numbers of neighbors (e.g. 1 or 3)
  ...
  for (unsigned int i = 0; i < V.rows(); i++) {
    ...
    // vertices with two neighbors are regular vertices, unless the vertex is a
    // singularity, in which case it qualifies as a start vertex
  ...
  for (unsigned int i = 0; i < startVertexIndices.size(); i++) {
    ...
    // note: a vertex can never be a start vertex and a regular vertex
    // simultaneously
    for (unsigned int j = 0; j < neighborSize; j++) {
      ...
      auto startVertex =
          VertexInfo(startVertexIndices[i], -1, -1, -1,
                     -1); // -1 values are arbitrary (will never be used)
      ...
      while (true) {
        ...
        auto it = std::find(currentVertexNeighbors->begin(),
                            currentVertexNeighbors->end(), prevVertex);
        assert(it != currentVertexNeighbors->end());
        currentVertexNeighbors->erase(it);

        if (currentVertexNeighbors->size() == 1 &&
            !isStartVertex[currentVertex.v]) {
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE void
igl::comiso::VertexIndexing<DerivedV, DerivedF>::InitSeamInfo() {
  auto verticesPerSeam = GetVerticesPerSeam();
  Handle_SystemInfo.EdgeSeamInfo.clear();
  int integerVar = 0;
  // Loop over each seam
  for (auto seam : verticesPerSeam) {
    // choose initial side of the seam such that the start vertex corresponds to
    // Fcut(f, k) and the end vertex corresponds to Fcut(f, (k+1)%3) and not
    // vice versa.
    int priorVertexIdx;
    if (seam.size() > 2) {
      auto v1 = seam[1];
      auto v2 = seam[2];
      if (Fcut(v1.f0, (v1.k0 + 1) % 3) == Fcut(v2.f0, v2.k0) ||
          Fcut(v1.f0, (v1.k0 + 1) % 3) == Fcut(v2.f1, v2.k1)) {
        priorVertexIdx = Fcut(v1.f0, v1.k0);
      } else {
        priorVertexIdx = Fcut(v1.f1, v1.k1);
        assert(Fcut(v1.f1, (v1.k1 + 1) % 3) == Fcut(v2.f0, v2.k0) ||
               Fcut(v1.f1, (v1.k1 + 1) % 3) == Fcut(v2.f1, v2.k1));
      }
    } else {
      auto v1 = seam[1];
      priorVertexIdx = Fcut(v1.f0, v1.k0);
    ...
    for (auto it = seam.begin() + 1; it != seam.end(); ++it) {
      ...
      Handle_SystemInfo.EdgeSeamInfo.push_back(
          SeamInfo(vtx0, vtx0p, MM, integerVar));
      if (it == seam.end() - 1) {
        Handle_SystemInfo.EdgeSeamInfo.push_back(
            SeamInfo(vtx1, vtx1p, MM, integerVar));
      }
      priorVertexIdx = vtx1;
    }
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE void igl::comiso::PoissonSolver<DerivedV, DerivedF>::SolvePoisson(
    Eigen::VectorXd Stiffness, double vector_field_scale, double grid_res,
    bool direct_round, int localIter, bool _integer_rounding,
    bool _singularity_rounding, std::vector<int> roundVertices,
    std::vector<std::vector<int>> hardFeatures) {
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE igl::comiso::PoissonSolver<DerivedV, DerivedF>::PoissonSolver(
    const Eigen::PlainObjectBase<DerivedV> &_V,
    const Eigen::PlainObjectBase<DerivedF> &_F,
    const Eigen::PlainObjectBase<DerivedV> &_Vcut,
    const Eigen::PlainObjectBase<DerivedF> &_Fcut,
    ...
    const Eigen::Matrix<int, Eigen::Dynamic, 1> &_Handle_Singular,
    const MeshSystemInfo &_Handle_SystemInfo)
    : V(_V), F(_F), Vcut(_Vcut), Fcut(_Fcut), TT(_TT), TTi(_TTi), PD1(_PD1),
      PD2(_PD2), Handle_Singular(_Handle_Singular),
...
/// for a given missmatch interval
template <typename DerivedV, typename DerivedF>
IGL_INLINE std::complex<double>
igl::comiso::PoissonSolver<DerivedV, DerivedF>::GetRotationComplex(
    int interval) {
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE void
igl::comiso::PoissonSolver<DerivedV, DerivedF>::AddFixedVertex(int v) {
  n_fixed_vars++;
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE void
igl::comiso::PoissonSolver<DerivedV, DerivedF>::FindFixedVertField() {
  ...
  AddFixedVertex(0);
  UV.row(0) << 0, 0;
  std::cerr << "No vertices to fix, I am fixing the first vertex to the origin!"
            << std::endl;
}
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE void
igl::comiso::PoissonSolver<DerivedV, DerivedF>::FindFixedVert() {
  Hard_constraints.clear();
  FindFixedVertField();
}
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE int
igl::comiso::PoissonSolver<DerivedV, DerivedF>::GetFirstVertexIndex(int v) {
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE void
igl::comiso::PoissonSolver<DerivedV, DerivedF>::FixBlockedVertex() {
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE void
igl::comiso::PoissonSolver<DerivedV, DerivedF>::AddSingularityRound() {
  for (unsigned int v = 0; v < V.rows(); v++) {
    if (Handle_Singular(v)) {
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE void
igl::comiso::PoissonSolver<DerivedV, DerivedF>::AddToRoundVertices(
    std::vector<int> ids) {
  for (size_t i = 0; i < ids.size(); ++i) {
    if (ids[i] < 0 || ids[i] >= V.rows())
      std::cerr << "WARNING: Ignored round vertex constraint, vertex " << ids[i]
                << " does not exist in the mesh." << std::endl;
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE void
igl::comiso::PoissonSolver<DerivedV, DerivedF>::BuildLaplacianMatrix(
    double vfscale) {
  Eigen::VectorXi idx =
      Eigen::VectorXi::LinSpaced(Vcut.rows(), 0, 2 * Vcut.rows() - 2);
  Eigen::VectorXi idx2 =
      Eigen::VectorXi::LinSpaced(Vcut.rows(), 1, 2 * Vcut.rows() - 1);
  ...
  G2 = G.transpose() * dblA.replicate<3, 1>().asDiagonal() *
       Handle_Stiffness.replicate<3, 1>().asDiagonal();
  ...
  const Eigen::MatrixXd u = Eigen::Map<const Eigen::MatrixXd>(
      PD1.data(), Fcut.rows() * 3,
      1); // this mimics a reshape at the cost of a copy.
  const Eigen::MatrixXd v = Eigen::Map<const Eigen::MatrixXd>(
      PD2.data(), Fcut.rows() * 3,
      1); // this mimics a reshape at the cost of a copy.
...
IGL_INLINE void igl::comiso::PoissonSolver<DerivedV, DerivedF>::FindSizes() {
  ...
  num_constraint_equations =
      num_cut_constraint * 2 + n_fixed_vars * 2 + num_userdefined_constraint;
  ...
  if (DEBUGPRINT)
    printf("* NUM USER DEFINED CONSTRAINTS %d\n", num_userdefined_constraint);
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE void
igl::comiso::PoissonSolver<DerivedV, DerivedF>::AllocateSystem() {
...
/// set the constraints for the inter-range cuts
template <typename DerivedV, typename DerivedF>
IGL_INLINE void igl::comiso::PoissonSolver<
    DerivedV, DerivedF>::BuildSeamConstraintsExplicitTranslation() {
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE void
igl::comiso::PoissonSolver<DerivedV, DerivedF>::BuildUserDefinedConstraints() {
  ...
  for (unsigned int i = 0; i < num_userdefined_constraint; i++) {
    ...
    constraints_rhs[constr_row] =
        userdefined_constraints[i][userdefined_constraints[i].size() - 1];
    constr_row += 1;
  }
}
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE void
igl::comiso::PoissonSolver<DerivedV, DerivedF>::MixedIntegerSolve(
    double cone_grid_res, bool direct_round, int localIter) {
  ...
  /// matrix A
  gmm::col_matrix<gmm::wsvector<double>> A(SizeMatrix,
                                           SizeMatrix); // lhs matrix variables
  ...
  for (int k = 0; k < Constraints.outerSize(); ++k) {
    for (Eigen::SparseMatrix<double>::InnerIterator it(Constraints, k); it;
         ++it) {
      int row = it.row();
      int col = it.col();
      C(row, col) += it.value();
  ...
  std::vector<int>::iterator new_end =
      std::unique(ids_to_round.begin(), ids_to_round.end());
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE void
igl::comiso::PoissonSolver<DerivedV, DerivedF>::clearUserConstraint() {
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE void
igl::comiso::PoissonSolver<DerivedV, DerivedF>::addSharpEdgeConstraint(
    int fid, int vid) {
  // prepare constraint
  ...
  Eigen::Matrix<typename DerivedV::Scalar, 3, 1> e =
      Vcut.row(v2) - Vcut.row(v1);
  e = e.normalized();
...
template <typename DerivedV, typename DerivedF, typename DerivedU>
IGL_INLINE igl::comiso::MIQ_class<DerivedV, DerivedF, DerivedU>::MIQ_class(
    ...
    const Eigen::Matrix<int, Eigen::Dynamic, 1> &Handle_Singular,
    const Eigen::Matrix<int, Eigen::Dynamic, 3> &Handle_Seams,
    Eigen::PlainObjectBase<DerivedU> &UV, Eigen::PlainObjectBase<DerivedF> &FUV,
    double GradientSize, double Stiffness, bool DirectRound, int iter,
    int localIter, bool DoRound, bool SingularityRound,
    std::vector<int> roundVertices, std::vector<std::vector<int>> hardFeatures)
    : V(V_), F(F_) {
  ...
  VertexIndexing<DerivedV, DerivedF> VInd(
      V, F, Vcut, Fcut, TT, TTi, Handle_MMatch, Handle_Singular, Handle_Seams);

  VInd.InitSeamInfo();

  // Assemble the system and solve
  PoissonSolver<DerivedV, DerivedF> PSolver(
      V, F, Vcut, Fcut, TT, TTi, PD1_combed, PD2_combed, Handle_Singular,
  ...
  if (iter > 0) // do stiffening
  {
    for (int i = 0; i < iter; i++) {
      PSolver.SolvePoisson(Handle_Stiffness, GradientSize, 1.f, DirectRound,
                           localIter, DoRound, SingularityRound, roundVertices,
                           hardFeatures);
      int nflips = NumFlips(PSolver.WUV);
      bool folded =
          updateStiffeningJacobianDistorsion(GradientSize, PSolver.WUV);
...
template <typename DerivedV, typename DerivedF, typename DerivedU>
IGL_INLINE void igl::comiso::MIQ_class<DerivedV, DerivedF, DerivedU>::extractUV(
    Eigen::PlainObjectBase<DerivedU> &UV_out,
    Eigen::PlainObjectBase<DerivedF> &FUV_out) {
  UV_out = this->UV_out;
...
template <typename DerivedV, typename DerivedF, typename DerivedU>
IGL_INLINE int igl::comiso::MIQ_class<DerivedV, DerivedF, DerivedU>::NumFlips(
    const Eigen::MatrixXd &WUV) {
  int numFl = 0;
...
template <typename DerivedV, typename DerivedF, typename DerivedU>
IGL_INLINE double
igl::comiso::MIQ_class<DerivedV, DerivedF, DerivedU>::Distortion(
    int f, double h, const Eigen::MatrixXd &WUV) {
  assert(h > 0);
  ...
  Eigen::Matrix<typename DerivedV::Scalar, 3, 1> norm =
      (p1 - p0).cross(p2 - p0);
  ...
  if (area2 > 0) {
    ...
    Eigen::Matrix<typename DerivedV::Scalar, 3, 1> diffu =
        (neg_t0 * uv0(0) + neg_t1 * uv1(0) + neg_t2 * uv2(0)) * area2_inv;
    Eigen::Matrix<typename DerivedV::Scalar, 3, 1> diffv =
        (neg_t0 * uv0(1) + neg_t1 * uv1(1) + neg_t2 * uv2(1)) * area2_inv;
    ...
    double sqrtDet = sqrt(std::max(
        0.0, diffDiag * diffDiag + 4 * I01 * I01)); // guaranteed non-neg
...
template <typename DerivedV, typename DerivedF, typename DerivedU>
IGL_INLINE double
igl::comiso::MIQ_class<DerivedV, DerivedF, DerivedU>::LaplaceDistortion(
    const int f, double h, const Eigen::MatrixXd &WUV) {
...
template <typename DerivedV, typename DerivedF, typename DerivedU>
IGL_INLINE bool igl::comiso::MIQ_class<DerivedV, DerivedF, DerivedU>::
    updateStiffeningJacobianDistorsion(double grad_size,
                                       const Eigen::MatrixXd &WUV) {
...
template <typename DerivedV, typename DerivedF, typename DerivedU>
IGL_INLINE void
igl::comiso::miq(const Eigen::PlainObjectBase<DerivedV> &V,
                 const Eigen::PlainObjectBase<DerivedF> &F,
                 const Eigen::PlainObjectBase<DerivedV> &PD1_combed,
                 const Eigen::PlainObjectBase<DerivedV> &PD2_combed,
                 const Eigen::Matrix<int, Eigen::Dynamic, 3> &Handle_MMatch,
                 ...
                 const Eigen::Matrix<int, Eigen::Dynamic, 3> &Handle_Seams,
                 Eigen::PlainObjectBase<DerivedU> &UV,
                 Eigen::PlainObjectBase<DerivedF> &FUV, double GradientSize,
                 double Stiffness, bool DirectRound, int iter, int localIter,
                 bool DoRound, bool SingularityRound,
                 std::vector<int> roundVertices,
                 std::vector<std::vector<int>> hardFeatures) {
  GradientSize =
      GradientSize / (V.colwise().maxCoeff() - V.colwise().minCoeff()).norm();

  igl::comiso::MIQ_class<DerivedV, DerivedF, DerivedU> miq(
      V, F, PD1_combed, PD2_combed, Handle_MMatch, Handle_Singular,
      Handle_Seams, UV, FUV, GradientSize, Stiffness, DirectRound, iter,
      localIter, DoRound, SingularityRound, roundVertices, hardFeatures);

  miq.extractUV(UV, FUV);
}

template <typename DerivedV, typename DerivedF, typename DerivedU>
IGL_INLINE void igl::comiso::miq(const Eigen::PlainObjectBase<DerivedV> &V,
                                 const Eigen::PlainObjectBase<DerivedF> &F,
                                 const Eigen::PlainObjectBase<DerivedV> &PD1,
                                 const Eigen::PlainObjectBase<DerivedV> &PD2,
                                 Eigen::PlainObjectBase<DerivedU> &UV,
                                 Eigen::PlainObjectBase<DerivedF> &FUV,
                                 double GradientSize, double Stiffness,
                                 bool DirectRound, int iter, int localIter,
                                 bool DoRound, bool SingularityRound,
                                 std::vector<int> roundVertices,
                                 std::vector<std::vector<int>> hardFeatures) {
  ...
  igl::cross_field_missmatch(V, F, BIS1_combed, BIS2_combed, true,
                             Handle_MMatch);

  Eigen::Matrix<int, Eigen::Dynamic, 1> isSingularity, singularityIndex;
  igl::find_cross_field_singularities(V, F, Handle_MMatch, isSingularity,
                                      singularityIndex);
  ...
  igl::comb_frame_field(V, F, PD1, PD2, BIS1_combed, BIS2_combed, PD1_combed,
                        PD2_combed);

  igl::comiso::miq(V, F, PD1_combed, PD2_combed, Handle_MMatch, isSingularity,
                   Handle_Seams, UV, FUV, GradientSize, Stiffness, DirectRound,
                   iter, localIter, DoRound, SingularityRound, roundVertices,
                   hardFeatures);
}

#ifdef IGL_STATIC_LIBRARY
// Explicit template specialization
template void igl::comiso::miq<Eigen::Matrix<double, -1, 3, 0, -1, 3>,
    ...
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, 3, 0, -1, 3>> &, double,
    double, bool, int, int, bool, bool, std::vector<int, std::allocator<int>>,
    std::vector<std::vector<int, std::allocator<int>>,
                std::allocator<std::vector<int, std::allocator<int>>>>);
template void igl::comiso::miq<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
    ...
    Eigen::Matrix<int, -1, 3, 0, -1, 3> const &,
    Eigen::PlainObjectBase<Eigen::Matrix<double, -1, -1, 0, -1, -1>> &,
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, -1, 0, -1, -1>> &, double,
    double, bool, int, int, bool, bool, std::vector<int, std::allocator<int>>,
    std::vector<std::vector<int, std::allocator<int>>,
                std::allocator<std::vector<int, std::allocator<int>>>>);
template void igl::comiso::miq<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
