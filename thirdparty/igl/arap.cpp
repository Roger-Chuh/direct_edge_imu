...
IGL_INLINE bool igl::arap_solve(const Eigen::PlainObjectBase<Derivedbc> &bc,
  ...
  while (iter < data.max_iter) {
    ...
    for (int c = 0; c < data.dim; c++) {
      ...
      min_quad_with_fixed_solve(data.solver_data, Bc, bcc, Beq, Uc);
      U.col(c) = Uc;
    }
...
template bool igl::arap_solve<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
                              Eigen::Matrix<double, -1, -1, 0, -1, -1>>(
    Eigen::PlainObjectBase<Eigen::Matrix<double, -1, -1, 0, -1, -1>> const &,
    igl::ARAPData &,
    Eigen::PlainObjectBase<Eigen::Matrix<double, -1, -1, 0, -1, -1>> &);
template bool igl::arap_precomputation<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
