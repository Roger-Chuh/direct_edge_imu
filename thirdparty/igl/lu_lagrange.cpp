...
template <typename T>
IGL_INLINE bool igl::lu_lagrange(const Eigen::SparseMatrix<T> &ATA,
...
#if defined(_WIN32)
#pragma message(                                                               \
    "lu_lagrange has not yet been implemented for your Eigen Version")
#else
#warning lu_lagrange has not yet been implemented for your Eigen Version
  ...
  if (m == 0) {
    ...
    Eigen::SparseLLT<Eigen::SparseMatrix<T>> MTM_LLT(
        MTM.template triangularView<Eigen::Lower>());
...
template bool
igl::lu_lagrange<double>(Eigen::SparseMatrix<double, 0, int> const &,
