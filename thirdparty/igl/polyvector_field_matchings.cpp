...
IGL_INLINE void
igl::polyvector_field_matching(const Eigen::PlainObjectBase<DerivedS> &_ua,
  ...
  // 3) the vectors in b that match the ordered vectors in a (in this case
  // (0,3,2,1)_a ) must be a circular shift of the ccw ordered vectors in b  -
  // so we need to explicitely check only these matchings to find the best ccw
  // one, there are N of them
  int hN = _ua.cols() / 3;
  int N = 2 * hN;
  ...
  igl::sort_vectors_ccw(ua, na, order_a, false, sorted_unused, false,
                        inv_order_unused);
  igl::sort_vectors_ccw(ub, nb, order_b, false, sorted_unused, false,
                        inv_order_unused);

  // checking all possible circshifts of order_b as matches for order_a
  Eigen::Matrix<typename DerivedM::Scalar, Eigen::Dynamic, Eigen::Dynamic>
      all_matches(N, N);
  ...
  for (int s = 0; s < N; ++s) {
    ...
    for (int i = 0; i < N; ++i) {
      if (match_with_curl)
        current_score += fabs(ua.segment(order_a[i] * 3, 3).dot(e) -
                              ub.segment(order_b[i] * 3, 3).dot(e));
      else {
        Eigen::Matrix<typename DerivedS::Scalar, 3, 1> na_ = na.transpose();
        Eigen::Matrix<typename DerivedS::Scalar, 3, 1> nb_ = nb.transpose();

        Eigen::Matrix<typename DerivedS::Scalar, 1, 3> uaRot =
            igl::rotation_matrix_from_directions(na_, nb_) *
            ua.segment(order_a[i] * 3, 3).transpose();
  ...
  Eigen::Matrix<typename DerivedM::Scalar, 1, Eigen::Dynamic>
      best_matching_for_sorted_a;
  int best_i;
...
template <typename DerivedS, typename DerivedV, typename DerivedF,
          typename DerivedE, typename DerivedM, typename DerivedC>
IGL_INLINE typename DerivedC::Scalar igl::polyvector_field_matchings(
    ...
    const Eigen::PlainObjectBase<DerivedV> &FN, const Eigen::MatrixXi &E2F,
    bool match_with_curl, Eigen::PlainObjectBase<DerivedM> &match_ab,
  ...
  for (int ei = 0; ei < numEdges; ++ei) {
    ...
    Eigen::Matrix<typename DerivedV::Scalar, 1, Eigen::Dynamic> ce =
        (V.row(E(ei, 1)) - V.row(E(ei, 0)))
            .normalized()
            .template cast<typename DerivedV::Scalar>();

    Eigen::Matrix<typename DerivedM::Scalar, 1, Eigen::Dynamic> mab, mba;
    igl::polyvector_field_matching(sol3D.row(a).eval(), sol3D.row(b).eval(),
                                   FN.row(a).eval(), FN.row(b).eval(), ce,
                                   match_with_curl, mab, mba);
    ...
    for (int i = 0; i < half_degree; ++i) {
      ...
      matched.segment(i * 3, 3) =
          sign * sol3D.row(b).segment((mab[i] % half_degree) * 3, 3);
    }

    typename DerivedC::Scalar avgCurl = 0;
    for (int i = 0; i < half_degree; ++i)
      avgCurl += fabs(sol3D.row(a).segment(i * 3, 3).dot(ce) -
                      matched.segment(i * 3, 3).dot(ce));
...
template <typename DerivedS, typename DerivedV, typename DerivedF,
          typename DerivedM, typename DerivedC>
IGL_INLINE typename DerivedC::Scalar
igl::polyvector_field_matchings(const Eigen::PlainObjectBase<DerivedS> &sol3D,
                                const Eigen::PlainObjectBase<DerivedV> &V,
                                const Eigen::PlainObjectBase<DerivedF> &F,
                                bool match_with_curl,
  ...
  return igl::polyvector_field_matchings(
      sol3D, V, F, E, FN, E2F, match_with_curl, match_ab, match_ba, curl);
}

#ifdef IGL_STATIC_LIBRARY
// Explicit template specialization
template Eigen::Matrix<float, -1, 1, 0, -1, 1>::Scalar
igl::polyvector_field_matchings<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
    ...
    Eigen::PlainObjectBase<Eigen::Matrix<double, -1, -1, 0, -1, -1>> const &,
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, -1, 0, -1, -1>> const &, bool,
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, -1, 0, -1, -1>> &,
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, -1, 0, -1, -1>> &,
    Eigen::PlainObjectBase<Eigen::Matrix<float, -1, 1, 0, -1, 1>> &);
template Eigen::Matrix<double, -1, 1, 0, -1, 1>::Scalar
igl::polyvector_field_matchings<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
                                Eigen::Matrix<double, -1, -1, 0, -1, -1>,
                                Eigen::Matrix<int, -1, -1, 0, -1, -1>,
                                Eigen::Matrix<int, -1, -1, 0, -1, -1>,
