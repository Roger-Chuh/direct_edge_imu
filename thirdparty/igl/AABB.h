...
public:
  ...
  // int m_depth;
  AABB()
      : m_left(NULL), m_right(NULL), m_box(), m_primitive(-1)
  ...
  AABB(const AABB &other)
      : m_left(other.m_left ? new AABB(*other.m_left) : NULL),
        ...
        m_box(other.m_box), m_primitive(other.m_primitive)
  ...
  //
  inline void init(const Eigen::PlainObjectBase<DerivedV> &V,
                   const Eigen::MatrixXi &Ele, const Eigen::MatrixXi &SI,
  ...
  //     construct mesh.**
  //   Ele  #Ele by dim+1 list of mesh indices into #V. **Should be same as used
  //   to
  //     construct mesh.**
  ...
  inline std::vector<int> find(const Eigen::PlainObjectBase<DerivedV> &V,
                               const Eigen::MatrixXi &Ele,
  ...
  template <typename Derivedbb_mins, typename Derivedbb_maxs>
  inline void serialize(Eigen::PlainObjectBase<Derivedbb_mins> &bb_mins,
                        Eigen::PlainObjectBase<Derivedbb_maxs> &bb_maxs,
                        Eigen::VectorXi &elements, const int i = 0) const;
  ...
  // dimension.
  inline Scalar squared_distance(const Eigen::PlainObjectBase<DerivedV> &V,
                                 const Eigen::MatrixXi &Ele,
                                 const RowVectorDIMS &p, int &i,
                                 RowVectorDIMS &c) const;
  // private:
  inline Scalar squared_distance(const Eigen::PlainObjectBase<DerivedV> &V,
                                 const Eigen::MatrixXi &Ele,
                                 const RowVectorDIMS &p, const Scalar min_sqr_d,
                                 int &i, RowVectorDIMS &c) const;

public:
  template <typename DerivedP, typename DerivedsqrD, typename DerivedI,
            typename DerivedC>
  inline void squared_distance(const Eigen::PlainObjectBase<DerivedV> &V,
                               const Eigen::MatrixXi &Ele,
  ...
  template <typename Derivedother_V, typename DerivedsqrD, typename DerivedI,
            typename DerivedC>
  inline void
  squared_distance(const Eigen::PlainObjectBase<DerivedV> &V,
                   const Eigen::MatrixXi &Ele,
...
private:
  ...
  inline Scalar squared_distance_helper(
      const Eigen::PlainObjectBase<DerivedV> &V, const Eigen::MatrixXi &Ele,
      const AABB<Derivedother_V, DIM> *other,
      const Eigen::PlainObjectBase<Derivedother_V> &other_V,
      const Eigen::MatrixXi &other_Ele, const Scalar min_sqr_d,
      Eigen::PlainObjectBase<DerivedsqrD> &sqrD,
      Eigen::PlainObjectBase<DerivedI> &I,
      Eigen::PlainObjectBase<DerivedC> &C) const;
  // Helper function for leaves: works in-place on sqr_d
  inline void leaf_squared_distance(const Eigen::PlainObjectBase<DerivedV> &V,
                                    const Eigen::MatrixXi &Ele,
                                    const RowVectorDIMS &p, Scalar &sqr_d,
                                    int &i, RowVectorDIMS &c) const;
  inline void set_min(const RowVectorDIMS &p, const Scalar sqr_d_candidate,
                      const int i_candidate, const RowVectorDIMS &c_candidate,
                      Scalar &sqr_d, int &i, RowVectorDIMS &c) const;

public:
  static inline void barycentric_coordinates(const RowVectorDIMS &p,
...
inline void igl::AABB<DerivedV, DIM>::init(
    ...
    const Eigen::PlainObjectBase<Derivedbb_mins> &bb_mins,
    const Eigen::PlainObjectBase<Derivedbb_maxs> &bb_maxs,
    const Eigen::VectorXi &elements, const int i) {
  using namespace std;
...
template <typename DerivedV, int DIM>
inline void
igl::AABB<DerivedV, DIM>::init(const Eigen::PlainObjectBase<DerivedV> &V,
                               const Eigen::MatrixXi &Ele) {
  using namespace Eigen;
...
inline void igl::AABB<DerivedV, DIM>::init(
    const Eigen::PlainObjectBase<DerivedV> &V, const Eigen::MatrixXi &Ele,
    const Eigen::MatrixXi &SI, const Eigen::VectorXi &I) {
  using namespace Eigen;
  using namespace std;
...
inline std::vector<int> igl::AABB<DerivedV, DIM>::find(
    const Eigen::PlainObjectBase<DerivedV> &V, const Eigen::MatrixXi &Ele,
    const Eigen::PlainObjectBase<Derivedq> &q, const bool first) const {
  using namespace std;
  using namespace Eigen;
  assert(q.size() == DIM && "Query dimension should match aabb dimension");
  ...
  if (is_leaf()) {
    ...
    a3 /= sum;
    a4 /= sum;
    if (a1 >= -epsilon && a2 >= -epsilon && a3 >= -epsilon && a4 >= -epsilon) {
...
inline void igl::AABB<DerivedV, DIM>::serialize(
    ...
    Eigen::PlainObjectBase<Derivedbb_maxs> &bb_maxs, Eigen::VectorXi &elements,
    const int i) const {
  using namespace std;
  using namespace Eigen;
...
igl::AABB<DerivedV, DIM>::squared_distance(
    const Eigen::PlainObjectBase<DerivedV> &V, const Eigen::MatrixXi &Ele,
    const RowVectorDIMS &p, int &i, RowVectorDIMS &c) const {
  return squared_distance(V, Ele, p, std::numeric_limits<Scalar>::infinity(), i,
                          c);
}
...
igl::AABB<DerivedV, DIM>::squared_distance(
    const Eigen::PlainObjectBase<DerivedV> &V, const Eigen::MatrixXi &Ele,
    const RowVectorDIMS &p, Scalar min_sqr_d, int &i, RowVectorDIMS &c) const {
  using namespace Eigen;
  using namespace std;
  using namespace igl;
  ...
  if (is_leaf()) {
    ...
    const auto &look_left = [&]() {
      ...
      Scalar sqr_d_left =
          m_left->squared_distance(V, Ele, p, sqr_d, i_left, c_left);
...
inline void igl::AABB<DerivedV, DIM>::squared_distance(
    ...
    const AABB<Derivedother_V, DIM> &other,
    const Eigen::PlainObjectBase<Derivedother_V> &other_V,
    const Eigen::MatrixXi &other_Ele, Eigen::PlainObjectBase<DerivedsqrD> &sqrD,
    Eigen::PlainObjectBase<DerivedI> &I,
    Eigen::PlainObjectBase<DerivedC> &C) const {
  ...
  sqrD.setConstant(other_Ele.rows(), 1,
  ...
  squared_distance_helper(V, Ele, &other, other_V, other_Ele, min_sqr_d, sqrD,
                          I, C);
}
...
igl::AABB<DerivedV, DIM>::squared_distance_helper(
  ...
  //// Exact minimum squared distance between arbitary primitives inside this
  /// and / othre's bounding boxes
  ...
  if (this->is_leaf()) {
    ...
    if (true) {
      this->squared_distance_helper(V, Ele, other->m_left, other_V, other_Ele,
                                    0, sqrD, I, C);
      this->squared_distance_helper(V, Ele, other->m_right, other_V, other_Ele,
                                    0, sqrD, I, C);
    } else {
    ...
    // other->m_max_sqr_d =
    // std::max(other->m_left->m_max_sqr_d,other->m_right->m_max_sqr_d);
    return 0;
  }
  ...
  if (true) {
    ...
    {
      this->m_left->squared_distance_helper(V, Ele, other, other_V, other_Ele,
                                            0, sqrD, I, C);
      this->m_right->squared_distance_helper(V, Ele, other, other_V, other_Ele,
                                             0, sqrD, I, C);
    }
  } else {
    // this is never reached ... :-(
...
inline void igl::AABB<DerivedV, DIM>::leaf_squared_distance(
    const Eigen::PlainObjectBase<DerivedV> &V, const Eigen::MatrixXi &Ele,
    const RowVectorDIMS &p, Scalar &sqr_d, int &i, RowVectorDIMS &c) const {
  using namespace Eigen;
  using namespace igl;
  using namespace std;
  ...
  if (ss == 3 && Ele(m_primitive, 0) != Ele(m_primitive, 1) &&
    ...
    if (n_norm > 0) {
      ...
      const RowVectorDIMS bc =
          1. / 3. *
          (V.row(Ele(m_primitive, 0)) + V.row(Ele(m_primitive, 1)) +
      ...
      barycentric_coordinates(pp, V.row(Ele(m_primitive, 0)),
                              V.row(Ele(m_primitive, 1)),
                              V.row(Ele(m_primitive, 2)), b);
      inside_triangle =
          fabs(fabs(b(0)) + fabs(b(1)) + fabs(b(2)) - 1.) <= 1e-10;
    }
  }
...
template <typename DerivedV, int DIM>
inline void igl::AABB<DerivedV, DIM>::set_min(const RowVectorDIMS &
#ifndef NDEBUG
                                                  p
#endif
                                              ,
...
template <typename DerivedV, int DIM>
inline void igl::AABB<DerivedV, DIM>::barycentric_coordinates(
    const RowVectorDIMS &p, const RowVectorDIMS &a, const RowVectorDIMS &b,
    const RowVectorDIMS &c, Eigen::Matrix<Scalar, 1, 3> &bary) {
