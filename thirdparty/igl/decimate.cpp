...
IGL_INLINE bool igl::decimate(const Eigen::MatrixXd &V,
                              const Eigen::MatrixXi &F, const size_t max_m,
                              Eigen::MatrixXd &U, Eigen::MatrixXi &G) {
  const auto &max_m_faces =
      [&max_m,
       &F](const Eigen::MatrixXd &, const Eigen::MatrixXi &,
...
IGL_INLINE bool igl::decimate(
    ...
    const std::function<
        bool(const Eigen::MatrixXd &, const Eigen::MatrixXi &,
             const Eigen::MatrixXi &, const Eigen::VectorXi &,
             const Eigen::MatrixXi &, const Eigen::MatrixXi &,
             const std::set<std::pair<double, int>> &,
             const std::vector<std::set<std::pair<double, int>>::iterator> &,
  ...
  while (true) {
    ...
    int e, e1, e2, f1, f2;
    if (collapse_edge(cost_and_placement, V, F, E, EMAP, EF, EI, Q, Qit, C, e,
                      e1, e2, f1, f2)) {
      if (stopping_condition(V, F, E, EMAP, EF, EI, Q, Qit, C, e, e1, e2, f1,
                             f2)) {
        clean_finish = true;
        break;
      }
