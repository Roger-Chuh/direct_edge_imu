...
IGL_INLINE int igl::tetgen::tetrahedralize(
    ...
    const std::vector<std::vector<int>> &F, const std::string switches,
    std::vector<std::vector<REAL>> &TV, std::vector<std::vector<int>> &TT,
...
template <typename DerivedV, typename DerivedF, typename DerivedTV,
          typename DerivedTT, typename DerivedTF>
IGL_INLINE int
igl::tetgen::tetrahedralize(const Eigen::PlainObjectBase<DerivedV> &V,
...
template int
igl::tetgen::tetrahedralize<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
