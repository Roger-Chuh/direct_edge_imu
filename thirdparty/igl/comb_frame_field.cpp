...
IGL_INLINE void
igl::comb_frame_field(const Eigen::PlainObjectBase<DerivedV> &V,
  ...
  for (unsigned i = 0; i < PD1.rows(); ++i) {
    ...
    DIRs << PD1.row(i), -PD1.row(i), PD2.row(i), -PD2.row(i);

    std::vector<double> a(4);

    double a_combed = atan2(B2.row(i).dot(BIS1_combed.row(i)),
                            B1.row(i).dot(BIS1_combed.row(i)));

    // center on the combed sector center
    for (unsigned j = 0; j < 4; ++j) {
      a[j] = atan2(B2.row(i).dot(DIRs.row(j)), B1.row(i).dot(DIRs.row(j))) -
             a_combed;
...
template void igl::comb_frame_field<Eigen::Matrix<double, -1, 3, 0, -1, 3>,
    ...
    Eigen::PlainObjectBase<Eigen::Matrix<double, -1, 3, 0, -1, 3>> &);
template void igl::comb_frame_field<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
