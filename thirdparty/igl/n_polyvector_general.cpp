...
private:
  ...
  IGL_INLINE void getGeneralCoeffConstraints(
      ...
      const Eigen::Matrix<typename DerivedV::Scalar, Eigen::Dynamic,
                          Eigen::Dynamic> &cfW,
      int k, const Eigen::VectorXi &rootsIndex,
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE void
igl::GeneralPolyVectorFieldFinder<DerivedV,
                                  DerivedF>::precomputeInteriorEdges() {
  // Flag border edges
  numInteriorEdges = 0;
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE void
igl::GeneralPolyVectorFieldFinder<DerivedV, DerivedF>::minQuadWithKnownMini(
    const Eigen::SparseMatrix<std::complex<typename DerivedV::Scalar>> &Q,
    const Eigen::SparseMatrix<std::complex<typename DerivedV::Scalar>> &f,
    const Eigen::VectorXi isConstrained,
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE bool igl::GeneralPolyVectorFieldFinder<DerivedV, DerivedF>::solve(
    const Eigen::VectorXi &isConstrained,
    const Eigen::Matrix<typename DerivedV::Scalar, Eigen::Dynamic,
                        Eigen::Dynamic> &cfW,
    const Eigen::VectorXi &rootsIndex,
  ...
  for (int i = 0; i < n; ++i) {
    ...
    Eigen::Matrix<std::complex<typename DerivedV::Scalar>, Eigen::Dynamic, 1>
        Ck;
    getGeneralCoeffConstraints(isConstrained, cfW, i, rootsIndex, Ck);
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE void igl::GeneralPolyVectorFieldFinder<DerivedV, DerivedF>::
    getGeneralCoeffConstraints(
        const Eigen::VectorXi &isConstrained,
        const Eigen::Matrix<typename DerivedV::Scalar, Eigen::Dynamic,
                            Eigen::Dynamic> &cfW,
        int k, const Eigen::VectorXi &rootsIndex,
        Eigen::Matrix<std::complex<typename DerivedV::Scalar>, Eigen::Dynamic,
                      1> &Ck) {
...
template <typename DerivedV, typename DerivedF>
IGL_INLINE void
igl::GeneralPolyVectorFieldFinder<DerivedV, DerivedF>::computek() {
  K.setZero(numE);
  // For every non-border edge
  for (unsigned eid = 0; eid < numE; ++eid) {
    if (!isBorderEdge[eid]) {
...
IGL_INLINE void
igl::n_polyvector_general(const Eigen::MatrixXd &V, const Eigen::MatrixXi &F,
                          const Eigen::VectorXi &b, const Eigen::MatrixXd &bc,
                          const Eigen::VectorXi &I, Eigen::MatrixXd &output) {
  ...
  int n = I.rows();
  igl::GeneralPolyVectorFieldFinder<Eigen::MatrixXd, Eigen::MatrixXi> pvff(V, F,
                                                                           n);
