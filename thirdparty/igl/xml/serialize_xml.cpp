...
template <typename T>
IGL_INLINE void serialize_xml(const T &obj, const std::string &objectName,
...
IGL_INLINE void serialize_xml(const T &obj, const std::string &objectName,
                              ...
                              tinyxml2::XMLElement *element, bool binary) {
  static_assert(serialization_xml::is_serializable<T>::value,
  ...
  if (binary) {
    ...
    std::string data = serialization_xml::base64_encode(
...
template <typename T>
IGL_INLINE void deserialize_xml(T &obj, const std::string &objectName,
  ...
  if (error != tinyxml2::XML_NO_ERROR) {
    ...
    tinyxml2::XMLElement *element = doc->FirstChildElement("serialization");
    if (element == NULL) {
      std::cerr << "Name of object not found! Initialized with default value."
                << std::endl;
      obj = T();
    } else {
...
IGL_INLINE void deserialize_xml(T &obj, const std::string &objectName,
  ...
  static_assert(serialization::is_serializable<T>::value,
                "'igl::xml::deserialize_xml': type is not deserializable");
  ...
  if (child != NULL) {
    ...
    if (attr != NULL) {
      ...
      std::vector<char> buffer;
      std::copy(decoded.c_str(), decoded.c_str() + decoded.length(),
...
IGL_INLINE bool XMLSerializable::PreSerialization() const { return true; }

IGL_INLINE void XMLSerializable::PostSerialization() const {}

IGL_INLINE bool XMLSerializable::PreDeserialization() { return true; }
...
IGL_INLINE void
XMLSerializable::Serialize(tinyxml2::XMLDocument *doc,
...
IGL_INLINE void
XMLSerializable::Deserialize(const tinyxml2::XMLDocument *doc,
                             const tinyxml2::XMLElement *element) {
  if (this->PreDeserialization()) {
    if (initialized == false) {
      objects.clear();
...
IGL_INLINE XMLSerializable::XMLSerializable() { initialized = false; }
...
template <typename T>
IGL_INLINE typename std::enable_if<std::is_fundamental<T>::value>::type
serialize(const T &obj, tinyxml2::XMLDocument *doc,
          tinyxml2::XMLElement *element, const std::string &name) {
...
template <typename T>
IGL_INLINE typename std::enable_if<std::is_fundamental<T>::value>::type
deserialize(T &obj, const tinyxml2::XMLDocument *doc,
            const tinyxml2::XMLElement *element, const std::string &name) {
  const tinyxml2::XMLElement *child = element->FirstChildElement(name.c_str());
  if (child != NULL) {
...
IGL_INLINE void serialize(const std::string &obj, tinyxml2::XMLDocument *doc,
...
IGL_INLINE void deserialize(std::string &obj, const tinyxml2::XMLDocument *doc,
                            const tinyxml2::XMLElement *element,
                            const std::string &name) {
  const tinyxml2::XMLElement *child = element->FirstChildElement(name.c_str());
  if (child != NULL) {
...
template <typename T>
IGL_INLINE typename std::enable_if<
    std::is_base_of<XMLSerializableBase, T>::value>::type
serialize(const T &obj, tinyxml2::XMLDocument *doc,
          tinyxml2::XMLElement *element, const std::string &name) {
  // Serialize object implementing Serializable interface
  const XMLSerializableBase &object =
      dynamic_cast<const XMLSerializableBase &>(obj);
...
IGL_INLINE void
serialize(const std::pair<T1, T2> &obj, tinyxml2::XMLDocument *doc,
          tinyxml2::XMLElement *element, const std::string &name) {
...
template <typename T1, typename T2>
IGL_INLINE void
deserialize(std::pair<T1, T2> &obj, const tinyxml2::XMLDocument *doc,
            const tinyxml2::XMLElement *element, const std::string &name) {
  const tinyxml2::XMLElement *child = element->FirstChildElement(name.c_str());
  if (child != NULL) {
...
template <typename T1, typename T2>
IGL_INLINE void
serialize(const std::vector<T1, T2> &obj, tinyxml2::XMLDocument *doc,
          tinyxml2::XMLElement *element, const std::string &name) {
...
template <typename T1, typename T2>
IGL_INLINE void
deserialize(std::vector<T1, T2> &obj, const tinyxml2::XMLDocument *doc,
            const tinyxml2::XMLElement *element, const std::string &name) {
  obj.clear();
...
template <typename T>
IGL_INLINE void serialize(const std::set<T> &obj, tinyxml2::XMLDocument *doc,
                          tinyxml2::XMLElement *element,
                          const std::string &name) {
...
template <typename T>
IGL_INLINE void deserialize(std::set<T> &obj, const tinyxml2::XMLDocument *doc,
                            const tinyxml2::XMLElement *element,
                            const std::string &name) {
  obj.clear();
...
template <typename T1, typename T2>
IGL_INLINE void
serialize(const std::map<T1, T2> &obj, tinyxml2::XMLDocument *doc,
          tinyxml2::XMLElement *element, const std::string &name) {
...
template <typename T1, typename T2>
IGL_INLINE void
deserialize(std::map<T1, T2> &obj, const tinyxml2::XMLDocument *doc,
            const tinyxml2::XMLElement *element, const std::string &name) {
  obj.clear();
...
IGL_INLINE void serialize(const Eigen::Matrix<T, R, C, P, MR, MC> &obj,
                          tinyxml2::XMLDocument *doc,
                          tinyxml2::XMLElement *element,
                          const std::string &name) {
...
template <typename T, int R, int C, int P, int MR, int MC>
IGL_INLINE void deserialize(Eigen::Matrix<T, R, C, P, MR, MC> &obj,
                            const tinyxml2::XMLDocument *doc,
                            const tinyxml2::XMLElement *element,
                            const std::string &name) {
  const tinyxml2::XMLElement *child = element->FirstChildElement(name.c_str());
...
IGL_INLINE void
serialize(const Eigen::SparseMatrix<T, P, I> &obj, tinyxml2::XMLDocument *doc,
          tinyxml2::XMLElement *element, const std::string &name) {
  tinyxml2::XMLElement *matrix = getElement(doc, element, name.c_str());
...
template <typename T, int P, typename I>
IGL_INLINE void
deserialize(Eigen::SparseMatrix<T, P, I> &obj, const tinyxml2::XMLDocument *doc,
            const tinyxml2::XMLElement *element, const std::string &name) {
  const tinyxml2::XMLElement *child = element->FirstChildElement(name.c_str());
  bool initialized = false;
  if (child != NULL) {
...
template <typename T>
IGL_INLINE typename std::enable_if<std::is_pointer<T>::value>::type
serialize(const T &obj, tinyxml2::XMLDocument *doc,
          tinyxml2::XMLElement *element, const std::string &name) {
...
deserialize(T &obj, const tinyxml2::XMLDocument *doc,
  ...
  if (child != NULL) {
    ...
    if (isNullPtr) {
      if (obj != NULL) {
        std::cout << "deserialization: possible memory leak for '"
...
IGL_INLINE tinyxml2::XMLElement *getElement(tinyxml2::XMLDocument *doc,
                                            tinyxml2::XMLElement *element,
                                            const std::string &name) {
  tinyxml2::XMLElement *child = element->FirstChildElement(name.c_str());
  if (child == NULL) {
...
IGL_INLINE void getAttribute(const char *src, std::string &dest) { dest = src; }
...
template <typename T>
IGL_INLINE typename std::enable_if<std::is_integral<T>::value &&
                                   std::is_unsigned<T>::value>::type
getAttribute(const char *src, T &dest) {
  unsigned int val;
...
IGL_INLINE void replaceSubString(std::string &str, const std::string &search,
...
std::string base64_decode(std::string const &encoded_string) {
  ...
  while (in_len-- && (encoded_string[in_] != '=') &&
    ...
    if (i == 4) {
      ...
      // char_array_4[i] = base64_chars.find(char_array_4[i]); // original
      // version
...
template void
igl::xml::serialize_xml<std::vector<float, std::allocator<float>>>(
    ...
    bool, bool);
template void
igl::xml::deserialize_xml<std::vector<float, std::allocator<float>>>(
