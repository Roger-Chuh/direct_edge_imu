...
//   binary     set to true to serialize the object in binary format (faster for
//   big data) overwrite  set to true to overwrite an existing xml file element
//   tinyxml2 virtual representation of the current xml node
// Outputs:
...
// deserializes the given data from a xml file or doc data back to the provided
// object
//
// Templates:
//   T  type of the object to serialize
...
//   filename   name of the file containing the serialization
//   binary     set to true to serialize the object in binary format (faster for
//   big data) overwrite  set to true to overwrite an existing xml file doc
//   contains current tinyxml2 virtual representation of the xml data element
//   tinyxml2 virtual representation of the current xml node
// Outputs:
...
struct XMLSerializableBase : public SerializableBase {
  ...
  virtual void Serialize(tinyxml2::XMLDocument *doc,
                         tinyxml2::XMLElement *element) const = 0;
...
private:
  ...
  struct XMLSerializationObject : public XMLSerializableBase {
    ...
    void Serialize(tinyxml2::XMLDocument *doc,
...
public:
  ...
  IGL_INLINE void Serialize(tinyxml2::XMLDocument *doc,
                            tinyxml2::XMLElement *element) const;
  IGL_INLINE void Deserialize(const tinyxml2::XMLDocument *doc,
                              const tinyxml2::XMLElement *element);
...
IGL_INLINE std::string base64_encode(unsigned char const *bytes_to_encode,
                                     unsigned int in_len);
...
template <typename T> struct is_serializable {
  ...
  static const bool value =
      std::is_fundamental<T0>::value || std::is_same<std::string, T0>::value ||
      std::is_base_of<XMLSerializableBase, T0>::value ||
