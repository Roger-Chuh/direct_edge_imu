...
//    IGL_INLINE bool save_ReAntTweakBar(::igl::anttweakbar::ReTwBar* bar, const
//    char* file_name); IGL_INLINE bool
//    save_ReAntTweakBar(::igl::anttweakbar::ReTwBar* bar,
//    tinyxml2::XMLDocument* doc); IGL_INLINE bool
//    load_ReAntTweakBar(::igl::anttweakbar::ReTwBar* bar, const char
//    *file_name); IGL_INLINE bool
//    load_ReAntTweakBar(::igl::anttweakbar::ReTwBar* bar,
//    tinyxml2::XMLDocument* doc);

IGL_INLINE bool save_ReAntTweakBar(::igl::anttweakbar::ReTwBar *bar,
                                   const char *file_name) {
  const char *name_chars = TwGetBarName(bar->bar);
  std::string name = std::string(name_chars) + "_AntTweakBar";

  const std::vector<::igl::anttweakbar::ReTwRWItem> &rw_items =
      bar->get_rw_items();
  for (std::vector<::igl::anttweakbar::ReTwRWItem>::const_iterator it =
           rw_items.begin();
       it != rw_items.end(); it++) {
  ...
  const std::vector<::igl::anttweakbar::ReTwCBItem> &cb_items =
      bar->get_cb_items();
  for (std::vector<::igl::anttweakbar::ReTwCBItem>::const_iterator it =
           cb_items.begin();
       it != cb_items.end(); it++) {
    TwType type = it->type;
...
tinyxml2::XMLDocument* doc)
{
  ...
  const std::vector< ::igl::anttweakbar::ReTwRWItem>& rw_items =
bar->get_rw_items(); for(std::vector<
::igl::anttweakbar::ReTwRWItem>::const_iterator it = rw_items.begin(); it !=
rw_items.end(); it++)
  {
    std::string val = bar->get_value_as_string(it->var,it->type);
  ...
  // Print all CB variables
  const std::vector< ::igl::anttweakbar::ReTwCBItem>& cb_items =
bar->get_cb_items(); for(std::vector<
::igl::anttweakbar::ReTwCBItem>::const_iterator it = cb_items.begin(); it !=
cb_items.end(); it++)
  {
    TwType type = it->type;
    //TwSetVarCallback setCallback = it->setCallback;
...
IGL_INLINE bool load_ReAntTweakBar(::igl::anttweakbar::ReTwBar *bar,
                                   const char *file_name) {
...
Skipping...\n",iter->first.c_str()); continue;
    ...
    if(!bar->type_from_string(type_str,type))
    {
      printf("ERROR: Type '%s' of '%s' not found...
Skipping...\n",type_str,iter->first.c_str()); continue;
    }
