...
IGL_INLINE bool igl::mosek::mosek_linprog(
    const Eigen::VectorXd &c, const Eigen::SparseMatrix<double> &A,
    const Eigen::VectorXd &lc, const Eigen::VectorXd &uc,
    const Eigen::VectorXd &lx, const Eigen::VectorXd &ux, Eigen::VectorXd &x) {
...
IGL_INLINE bool
igl::mosek::mosek_linprog(const Eigen::VectorXd &c,
                          const Eigen::SparseMatrix<double> &A,
                          const Eigen::VectorXd &lc, const Eigen::VectorXd &uc,
                          const Eigen::VectorXd &lx, const Eigen::VectorXd &ux,
                          const MSKenv_t &env, Eigen::VectorXd &x) {
  ...
  const auto &key = [](const double lxj, const double uxj) -> MSKboundkeye {
  ...
  for (int j = 0; j < n; j++) {
    ...
    const double lxj =
        lx.size() > 0 ? lx[j] : -numeric_limits<double>::infinity();
    const double uxj =
        ux.size() > 0 ? ux[j] : numeric_limits<double>::infinity();
    mosek_guarded(MSK_putvarbound(task, j, key(lxj, uxj), lxj, uxj));

    if (m > 0) {
      // Input column j of A
      mosek_guarded(MSK_putacol(task, j, vAcp[j + 1] - vAcp[j], &vAri[vAcp[j]],
                                &vAv[vAcp[j]]));
    }
  }
  ...
  for (int i = 0; i < m; i++) {
    ...
    const double lci =
        lc.size() > 0 ? lc[i] : -numeric_limits<double>::infinity();
    const double uci =
        uc.size() > 0 ? uc[i] : numeric_limits<double>::infinity();
