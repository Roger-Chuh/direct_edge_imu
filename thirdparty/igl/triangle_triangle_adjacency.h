...
//   adjacent to the j edge of triangle i TTi  #F by #3 adjacent matrix, the
//   element i,j is the id of edge of the triangle TT(i,j) that is adjacent with
//   triangle i
...
IGL_INLINE void
triangle_triangle_adjacency(const Eigen::PlainObjectBase<Scalar> &V,
...
IGL_INLINE void
triangle_triangle_adjacency(const Eigen::PlainObjectBase<DerivedF> &F,
...
template <typename Scalar, typename Index>
IGL_INLINE void
triangle_triangle_adjacency_preprocess(const Eigen::PlainObjectBase<Scalar> &V,
                                       const Eigen::PlainObjectBase<Index> &F,
...
IGL_INLINE void
triangle_triangle_adjacency_extractTT(const Eigen::PlainObjectBase<DerivedF> &F,
                                      std::vector<std::vector<int>> &TTT,
                                      Eigen::PlainObjectBase<DerivedTT> &TT);
...
// Outputs:
//   TT  #F by 3 list of lists so that TT[i][c] --> {j,k,...} means that faces j
//   and
