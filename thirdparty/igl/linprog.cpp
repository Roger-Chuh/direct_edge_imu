...
IGL_INLINE bool igl::linprog(const Eigen::VectorXd &c,
                             ...
                             const Eigen::VectorXd &b, const int k,
                             Eigen::VectorXd &x) {
  ...
  while (phase < 3) {
    ...
    while (true) {
      if (MAXIT > 0 && it >= MAXIT) {
        ...
        cerr << "linprog: warning! maximum iterations without convergence."
             << endl;
#endif
        success = false;
        break;
...
IGL_INLINE bool igl::linprog(const Eigen::VectorXd &f, const Eigen::MatrixXd &A,
                             const Eigen::VectorXd &b, const Eigen::MatrixXd &B,
  ...
  P << In, -In, MatrixXd::Zero(n, m), MatrixXd::Zero(m, 2 * n), Im;
  MatrixXd ASP = AS * P;
