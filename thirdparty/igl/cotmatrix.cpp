...
IGL_INLINE void igl::cotmatrix(const Eigen::PlainObjectBase<DerivedV> &V,
  ...
  if (simplex_size == 3) {
    // This is important! it could decrease the comptuation time by a factor of
    // 2 Laplacian for a closed 2d manifold mesh will have on average 7 entries
    // per row
    L.reserve(10 * V.rows());
    edges.resize(3, 2);
    edges << 1, 2, 2, 0, 0, 1;
...
template void igl::cotmatrix<Eigen::Matrix<double, -1, 3, 0, -1, 3>,
