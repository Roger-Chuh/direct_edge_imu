...
IGL_INLINE Eigen::Matrix<Scalar, 3, 3>
igl::rotation_matrix_from_directions(const Eigen::Matrix<Scalar, 3, 1> v0,
  ...
  if ((v0 + v1).norm() < epsilon) {
    rotM = -Eigen::Matrix<Scalar, 3, 3>::Identity();
    rotM(0, 0) = 1.;
    std::cerr << "igl::rotation_matrix_from_directions: rotating around x axis "
                 "by 180o"
              << std::endl;
    return rotM;
  }
...
igl::rotation_matrix_from_directions<double>(
    const Eigen::Matrix<double, 3, 1, 0, 3, 1>,
    const Eigen::Matrix<double, 3, 1, 0, 3, 1>, const bool);
#endif
