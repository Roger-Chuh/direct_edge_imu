...
public:
  ...
  IGL_INLINE void init(const Eigen::MatrixXd &_V, const Eigen::MatrixXi &_F,
                       const Eigen::MatrixXd &_D1, const Eigen::MatrixXd &_D2,
                       double _Lambda, double _perturb_rotations,
                       int _fixed = 1);
  ...
  IGL_INLINE void precompute_ARAP(Eigen::SparseMatrix<double> &Lff,
                                  Eigen::MatrixXd &LfcVc);

  // Precomputation for regularization
  IGL_INLINE void precompute_SMOOTH(Eigen::SparseMatrix<double> &MS,
                                    Eigen::MatrixXd &bS);

  // extracts a r x c block from sparse matrix mat into sparse matrix m1
  // (r0,c0) is upper left entry of block
  IGL_INLINE void extractBlock(Eigen::SparseMatrix<double> &mat, int r0, int c0,
                               int r, int c, Eigen::SparseMatrix<double> &m1);
  ...
  IGL_INLINE void
  compute_idealWarp(std::vector<Eigen::Matrix<double, 3, 3>> &WW);
  ...
  std::vector<Eigen::Matrix<double, 3, 2>>
      FF; // frame field FF in 3D (parallel to m.F)
  std::vector<Eigen::Matrix<double, 3, 3>>
      WW; // warping matrices to make a cross field (parallel to m.F)
  std::vector<Eigen::Matrix<double, 3, 2>>
      XF; // pseudo-cross field from solution (parallel to m.F)
  ...
  // Numerics
  int nfree, nconst; // number of free/constrained vertices in the mesh -
                     // default all-but-1/1
  Eigen::MatrixXd C; // cotangent matrix of m
  Eigen::SparseMatrix<double> L; // Laplacian matrix of m

  Eigen::SparseMatrix<double>
      M;               // matrix for global optimization - pre-conditioned
  Eigen::MatrixXd RHS; // pre-computed part of known term in global optimization
  std::vector<Eigen::Matrix<double, 3, 3>>
      RW; // optimal rotation-warping matrices (parallel to m.F) -- INCORPORATES
          // WW
  Eigen::SimplicialCholesky<Eigen::SparseMatrix<double>>
      solver; // solver for linear system in global opt.
...
IGL_INLINE void
Frame_field_deformer::init(const Eigen::MatrixXd &_V, const Eigen::MatrixXi &_F,
                           const Eigen::MatrixXd &_D1,
                           const Eigen::MatrixXd &_D2, double _Lambda,
                           double _perturb_rotations, int _fixed) {
  V = _V;
  F = _F;
...
IGL_INLINE void Frame_field_deformer::reset_opt() {
  ...
  for (unsigned i = 0; i < V_w.rows(); ++i)
    for (unsigned j = 0; j < V_w.cols(); ++j)
      V_w(i, j) +=
          (double(rand()) / double(RAND_MAX)) * 10e-4 * perturb_rotations;
}
...
IGL_INLINE void Frame_field_deformer::precompute_opt() {
  ...
  igl::vertex_triangle_adjacency(V, F, VT,
                                 VTi); // compute vertex to face relationship

  igl::cotmatrix_entries(V, F,
                         C); // cotangent matrix for opt. rotations - global
  ...
  RHS = (1 - Lambda) * LfcVc +
        Lambda * bS; // RHS (partial) for linear system - global
...
IGL_INLINE void
Frame_field_deformer::precompute_ARAP(Eigen::SparseMatrix<double> &Lff,
                                      Eigen::MatrixXd &LfcVc) {
  using namespace Eigen;
...
IGL_INLINE void
Frame_field_deformer::precompute_SMOOTH(Eigen::SparseMatrix<double> &MS,
                                        Eigen::MatrixXd &bS) {
  using namespace Eigen;
...
IGL_INLINE void
Frame_field_deformer::extractBlock(Eigen::SparseMatrix<double> &mat, int r0,
                                   int c0, int r, int c,
    ...
    for (Eigen::SparseMatrix<double>::InnerIterator it(mat, k); it; ++it) {
      if (it.row() >= r0 && it.row() < r0 + r)
        tripletList.push_back(
            Eigen::Triplet<double>(it.row() - r0, it.col() - c0, it.value()));
    }
...
IGL_INLINE void Frame_field_deformer::compute_optimal_rotations() {
  ...
  for (int i = 0; i < F.rows(); i++) {
    ...
    // cotangents
    D << C(i, 2), 0, 0, 0, C(i, 0), 0, 0, 0, C(i, 1);

    S = PP * D * P.transpose();
    Eigen::JacobiSVD<Matrix<double, 3, 3>> svd(S, Eigen::ComputeFullU |
                                                      Eigen::ComputeFullV);
...
IGL_INLINE void Frame_field_deformer::compute_optimal_positions() {
  ...
  for (int i = 0; i < nfree; i++) {
    ...
    for (int k = 0; k < (int)VT[i].size(); k++) // for all incident triangles
    {
      ...
      vi = (i == F(t, 0))
               ? 0
               : (i == F(t, 1)) ? 1 : (i == F(t, 2)) ? 2 : 3; // index of i in t
      assert(vi != 3);
      i1 = F(t, (vi + 1) % 3);
      i2 = F(t, (vi + 2) % 3);
      b.row(i) +=
          (C(t, (vi + 2) % 3) * RW[t] * (V.row(i1) - V.row(i)).transpose())
              .transpose();
      b.row(i) +=
          (C(t, (vi + 1) % 3) * RW[t] * (V.row(i2) - V.row(i)).transpose())
              .transpose();
    }
  }
  b /= 2.0;
...
IGL_INLINE void Frame_field_deformer::computeXField(
...
// computes in WW the ideal warp at each tri to make the frame field a cross
IGL_INLINE void Frame_field_deformer::compute_idealWarp(
  ...
  for (int i = 0; i < (int)FF.size(); i++) {
    ...
    // polar decomposition to discard rotational component (unnecessary but
    // makes it easier)
    Eigen::JacobiSVD<Matrix<double, 3, 3>> svd(AI, Eigen::ComputeFullU |
                                                       Eigen::ComputeFullV);
...
IGL_INLINE void
igl::frame_field_deformer(const Eigen::MatrixXd &V, const Eigen::MatrixXi &F,
                          const Eigen::MatrixXd &FF1,
                          const Eigen::MatrixXd &FF2, Eigen::MatrixXd &V_d,
                          Eigen::MatrixXd &FF1_d, Eigen::MatrixXd &FF2_d,
                          const int iterations, const double lambda,
