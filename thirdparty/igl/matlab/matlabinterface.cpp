...
// (no need to call it directly since it is automatically invoked by any other
// command)
...
IGL_INLINE void igl::matlab::mlsetmatrix(Engine **mlengine, std::string name,
...
// Send a matrix to MATLAB
IGL_INLINE void igl::matlab::mlsetmatrix(
    Engine **mlengine, std::string name,
...
IGL_INLINE void igl::matlab::mlgetmatrix(Engine **mlengine, std::string name,
                                         Eigen::MatrixXd &M) {
  if (*mlengine == 0)
    mlinit(mlengine);
...
// Receive a matrix from MATLAB
IGL_INLINE void igl::matlab::mlgetmatrix(
    Engine **mlengine, std::string name,
...
IGL_INLINE void igl::matlab::mlsetscalar(Engine **mlengine, std::string name,
                                         double s) {
  if (*mlengine == 0)
    mlinit(mlengine);
...
IGL_INLINE double igl::matlab::mlgetscalar(Engine **mlengine,
                                           std::string name) {
  if (*mlengine == 0)
    mlinit(mlengine);
...
IGL_INLINE std::string igl::matlab::mleval(Engine **mlengine,
                                           std::string code) {
  if (*mlengine == 0)
    mlinit(mlengine);
...
IGL_INLINE void igl::matlab::mlsetmatrix(Engine **mlengine, std::string name,
  ...
  std::string temp =
      name + " = sparse(temp93765(:,1),temp93765(:,2),temp93765(:,3)," +
