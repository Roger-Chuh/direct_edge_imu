...
IGL_INLINE void igl::mvc(const Eigen::MatrixXd &V, const Eigen::MatrixXd &C,
                         Eigen::MatrixXd &W) {
  ...
  bool V_flat =
      (V.cols() == 3) && (std::sqrt((V.col(3)).dot(V.col(3))) < 1e-10);
  bool C_flat =
      (C.cols() == 3) && (std::sqrt((C.col(3)).dot(C.col(3))) < 1e-10);
  ...
  for (int i = 0; i < C1.rows(); ++i) {
    ...
    double edge_length =
        std::sqrt((C1.row(i) - C1.row(i_next)).dot(C1.row(i) - C1.row(i_next)));
    for (int j = 0; j < V1.rows(); ++j) {
      ...
      // distance from V to every C, where dist_C_V(i,j) is the distance from
      // domain vertex j to handle i
      ...
      WW(i, j) =
          (std::tan(a_prev / 2.0) + std::tan(a_next / 2.0)) / dist_C_V(i, j);

      if (dist_C_V(i, j) < EPS)
        on_corner.push_back(std::make_pair(j, i));
      else
          // only in case of no-corner (no need for checking for multiple
          // segments afterwards -- should only be on one segment (otherwise
          // must be on a corner and we already handled that) domain vertex j is
          // on the segment from i to i+1 if the distances from vj to pi and
          // pi+1 are about
          if (std::abs((dist_C_V(i, j) + dist_C_V_next) / edge_length - 1) <
              EPS)
