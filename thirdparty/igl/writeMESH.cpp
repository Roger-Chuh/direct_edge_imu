...
IGL_INLINE bool igl::writeMESH(const std::string str,
  ...
  for (int i = 0; i < number_of_tetrahedra; i++) {
    ...
    fprintf(mesh_file, "%d %d %d %d 1\n", (int)T(i, 0) + 1, (int)T(i, 1) + 1,
            (int)T(i, 2) + 1, (int)T(i, 3) + 1);
  }
  fclose(mesh_file);
  return true;
}
...
// template bool igl::writeMESH<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
