...
void igl::polyvector_field_one_ring_matchings(
    ...
    const int vector_to_match, Eigen::VectorXi &mvi, Eigen::VectorXi &fi) {
  ...
  while (next_face != fstart) {
    ...
    // check its orientation to determine whether match_ab or match_ba should be
    // used
...
IGL_INLINE void igl::polyvector_field_singularities_from_matchings(
  ...
  igl::polyvector_field_singularities_from_matchings(
      V, F, V_border, VF, TT, E2F, F2E, match_ab, match_ba, singularities);
}
...
IGL_INLINE void igl::polyvector_field_singularities_from_matchings(
  ...
  for (int vector_to_match = 0; vector_to_match < half_degree;
       ++vector_to_match) {
    for (int vi = 0; vi < numV; ++vi) {
      ...
      igl::polyvector_field_one_ring_matchings(V, F, VF, E2F, F2E, TT, match_ab,
                                               match_ba, vi, vector_to_match,
                                               mvi, fi);
...
template void igl::polyvector_field_singularities_from_matchings<
