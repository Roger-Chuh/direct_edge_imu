...
IGL_INLINE bool igl::opengl::create_shader_program(
    const std::string &geom_source, const std::string &vert_source,
    const std::string &frag_source, const std::map<std::string, GLuint> &attrib,
    GLuint &id) {
  using namespace std;
  if (vert_source == "" && frag_source == "") {
    cerr << "create_shader_program() could not create shader program,"
  ...
  if (geom_source != "") {
    // load vertex shader
    GLuint g =
        igl::opengl::load_shader(geom_source.c_str(), GL_GEOMETRY_SHADER_EXT);
    if (g == 0) {
  ...
  if (frag_source != "") {
    ...
    GLuint f =
        igl::opengl::load_shader(frag_source.c_str(), GL_FRAGMENT_SHADER);
    if (f == 0) {
  ...
  for (std::map<std::string, GLuint>::const_iterator ait = attrib.begin();
       ait != attrib.end(); ait++) {
    glBindAttribLocation(id, (*ait).second, (*ait).first.c_str());
  }
  // Link program
  glLinkProgram(id);
...
IGL_INLINE bool igl::opengl::create_shader_program(
    ...
    const std::map<std::string, GLuint> &attrib, GLuint &prog_id) {
