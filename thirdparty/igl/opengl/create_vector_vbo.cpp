...
IGL_INLINE void igl::opengl::create_vector_vbo(
  ...
  if (V.Options & Eigen::RowMajor) {
    glBufferData(GL_ARRAY_BUFFER, sizeof(T) * V.size(), V.data(),
                 GL_STATIC_DRAW);
  } else {
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(T) * V.size(), VT.data(),
                 GL_STATIC_DRAW);
  }
...
template void igl::opengl::create_vector_vbo<int>(
    Eigen::Matrix<int, -1, -1, 0, -1, -1> const &, unsigned int &);
// generated by autoexplicit.sh
template void igl::opengl::create_vector_vbo<double>(
    Eigen::Matrix<double, -1, -1, 0, -1, -1> const &, unsigned int &);
#endif
