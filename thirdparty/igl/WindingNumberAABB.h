...
public:
  ...
  inline WindingNumberAABB(const Eigen::MatrixXd &V, const Eigen::MatrixXi &F);
  inline WindingNumberAABB(const WindingNumberTree<Point> &parent,
...
template <typename Point>
inline void igl::WindingNumberAABB<Point>::set_mesh(const Eigen::MatrixXd &V,
                                                    const Eigen::MatrixXi &F) {
  igl::WindingNumberTree<Point>::set_mesh(V, F);
  init();
}

template <typename Point> inline void igl::WindingNumberAABB<Point>::init() {
...
inline igl::WindingNumberAABB<Point>::WindingNumberAABB(
    ...
    : WindingNumberTree<Point>(V, F), min_corner(), max_corner(),
      total_positive_area(std::numeric_limits<double>::infinity()),
      split_method(MEDIAN_ON_LONGEST_AXIS) {
  init();
}
...
template <typename Point>
inline igl::WindingNumberAABB<Point>::WindingNumberAABB(
    const WindingNumberTree<Point> &parent, const Eigen::MatrixXi &F)
    : WindingNumberTree<Point>(parent, F), min_corner(), max_corner(),
      total_positive_area(std::numeric_limits<double>::infinity()),
      split_method(MEDIAN_ON_LONGEST_AXIS) {
  init();
}

template <typename Point> inline void igl::WindingNumberAABB<Point>::grow() {
  using namespace std;
  using namespace Eigen;
  ...
  // Base cases
  if (this->getF().rows() <=
          (WindingNumberAABB_MIN_F > 0 ? WindingNumberAABB_MIN_F : 0) ||
  ...
  WindingNumberAABB<Point> *leftWindingNumberAABB =
      new WindingNumberAABB<Point>(*this, leftF);
  leftWindingNumberAABB->grow();
  this->children.push_back(leftWindingNumberAABB);
  WindingNumberAABB<Point> *rightWindingNumberAABB =
      new WindingNumberAABB<Point>(*this, rightF);
...
inline void igl::WindingNumberAABB<Point>::compute_min_max_corners() {
  ...
  for (int i = 0; i < this->getF().rows(); i++) {
    for (int j = 0; j < this->getF().cols(); j++) {
      for (int d = 0; d < min_corner.size(); d++) {
        min_corner[d] = this->getV()(this->getF()(i, j), d) < min_corner[d]
                            ? this->getV()(this->getF()(i, j), d)
                            : min_corner[d];
        max_corner[d] = this->getV()(this->getF()(i, j), d) > max_corner[d]
                            ? this->getV()(this->getF()(i, j), d)
                            : max_corner[d];
      }
  ...
  // cout<<"Diag center: "<<((this->max_corner +
  // this->min_corner)*0.5).transpose()<<endl; cout<<endl;
...
template <typename Point>
inline double
igl::WindingNumberAABB<Point>::max_abs_winding_number(const Point &p) const {
  using namespace std;
...
template <typename Point>
inline double igl::WindingNumberAABB<Point>::max_simple_abs_winding_number(
    const Point &p) const {
  using namespace std;
  using namespace Eigen;
  // Only valid if not inside
  ...
  BV << min_corner[0], min_corner[1], min_corner[2], min_corner[0],
      min_corner[1], max_corner[2], min_corner[0], max_corner[1], min_corner[2],
      min_corner[0], max_corner[1], max_corner[2], max_corner[0], min_corner[1],
      min_corner[2], max_corner[0], min_corner[1], max_corner[2], max_corner[0],
      max_corner[1], min_corner[2], max_corner[0], max_corner[1], max_corner[2];
  MatrixXi BF(2 * 2 * 3, 3);
  BF << 0, 6, 4, 0, 2, 6, 0, 3, 2, 0, 1, 3, 2, 7, 6, 2, 3, 7, 4, 6, 7, 4, 7, 5,
  ...
  igl::winding_number_3(BV.data(), BV.rows(), PBF.data(), PBF.rows(), p.data(),
                        1, &w);
  return w;
}
