...
public:
  ...
  // m_V(V.template cast<ExactScalar>()),m_F(F)
  {
    ...
    m_J = VectorJ::LinSpaced(m_number_of_birth_faces, 0,
                             m_number_of_birth_faces - 1);
  }
  // Returns reference to resulting mesh vertices m_V in exact scalar
  // representation
  const MatrixX3E &V() const { return m_V; }
  // Returns mesh vertices in the desired output type, casting when
  // appropriate to floating precision.
  template <typename DerivedV> Eigen::PlainObjectBase<DerivedV> cast_V() const {
