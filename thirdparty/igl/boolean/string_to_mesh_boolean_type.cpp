...
IGL_INLINE bool
igl::boolean::string_to_mesh_boolean_type(const std::string &s,
                                          MeshBooleanType &type) {
  using namespace std;
  string eff_s = s;
  transform(eff_s.begin(), eff_s.end(), eff_s.begin(), ::tolower);
  const auto &find_any = [](const vector<string> &haystack,
                            const string &needle) -> bool {
  ...
  } else if (find_any({"minus", "subtract", "difference", "relative complement",
                       "m", "\\"},
                      eff_s)) {
...
IGL_INLINE igl::boolean::MeshBooleanType
igl::boolean::string_to_mesh_boolean_type(const std::string &s) {
  MeshBooleanType type;
#ifndef NDEBUG
  const bool ret =
