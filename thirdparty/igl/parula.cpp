...
template <typename T> IGL_INLINE void igl::parula(const T x, T *rgb) {
  return parula(x, rgb[0], rgb[1], rgb[2]);
}

template <typename T> IGL_INLINE void igl::parula(const T f, T &r, T &g, T &b) {
  // clamp to (0,1)
...
IGL_INLINE void igl::parula(const Eigen::PlainObjectBase<DerivedZ> &Z,
...
template void igl::parula<Eigen::Matrix<double, -1, 1, 0, -1, 1>,
