...
template <typename AT, typename DerivedB, typename Derivedknown,
          typename DerivedY, typename AeqT, typename DerivedBeq, typename AieqT,
          typename DerivedBieq, typename Derivedlx, typename Derivedux,
          typename DerivedZ>
IGL_INLINE igl::SolverStatus igl::active_set(
    const Eigen::SparseMatrix<AT> &A, const Eigen::PlainObjectBase<DerivedB> &B,
    ...
    const igl::active_set_params &params, Eigen::PlainObjectBase<DerivedZ> &Z) {
...
#define TRUE 1
#define FALSE 0
  Matrix<BOOL, Dynamic, 1> as_lx =
      Matrix<BOOL, Dynamic, 1>::Constant(n, 1, FALSE);
  Matrix<BOOL, Dynamic, 1> as_ux =
      Matrix<BOOL, Dynamic, 1>::Constant(n, 1, FALSE);
  Matrix<BOOL, Dynamic, 1> as_ieq =
      Matrix<BOOL, Dynamic, 1>::Constant(Aieq.rows(), 1, FALSE);
  ...
  while (true) {
    ...
    if (known_i.size() == A.rows()) {
...
#endif
      if (!min_quad_with_fixed_precompute(A, known_i, Aeq_i, params.Auu_pd,
                                          data)) {
        cerr << "Error: min_quad_with_fixed precomputation failed." << endl;
        if (iter > 0 && Aeq_i.rows() > Aeq.rows()) {
          cerr
              << "  *Are you sure rows of [Aeq;Aieq] are linearly independent?*"
              << endl;
        }
      ...
      cout << "  post" << endl;
#endif
      // Computing Lagrange multipliers needs to be adjusted slightly if A is
      // not symmetric
      assert(data.Auu_sym);
    }
    ...
    for (int u = 0; u < as_ux_count; u++) {
      if (Lambda_known_i(nk + as_lx_count + u) < params.inactive_threshold) {
...
template igl::SolverStatus
igl::active_set<double, Eigen::Matrix<double, -1, 1, 0, -1, 1>,
