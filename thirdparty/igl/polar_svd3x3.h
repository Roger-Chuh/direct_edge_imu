...
//
template <typename Mat> IGL_INLINE void polar_svd3x3(const Mat &A, Mat &R);
#ifdef __SSE__
template <typename T>
IGL_INLINE void polar_svd3x3_sse(const Eigen::Matrix<T, 3 * 4, 3> &A,
                                 Eigen::Matrix<T, 3 * 4, 3> &R);
#endif
#ifdef __AVX__
template <typename T>
IGL_INLINE void polar_svd3x3_avx(const Eigen::Matrix<T, 3 * 8, 3> &A,
