...
// and shouldn't cause race conditions. (This is a hack, but I think it's
// "safe")
...
template <typename Point> class WindingNumberTree {
public:
...
public:
  ...
  // For root
  inline WindingNumberTree(const Eigen::MatrixXd &V, const Eigen::MatrixXi &F);
  // For chilluns
  inline WindingNumberTree(const WindingNumberTree<Point> &parent,
                           const Eigen::MatrixXi &F);
  inline virtual ~WindingNumberTree();
  inline virtual void set_mesh(const Eigen::MatrixXd &V,
                               const Eigen::MatrixXi &F);
  // Set method
...
public:
  ...
  inline virtual double cached_winding_number(const WindingNumberTree &that,
                                              const Point &p) const;
};
} // namespace igl
...
// WindingNumberMethod WindingNumberTree<Point>::method =
// EXACT_WINDING_NUMBER_METHOD; template <typename Point> double
// WindingNumberTree<Point>::min_max_w = 0;
template <typename Point>
std::map<std::pair<const igl::WindingNumberTree<Point> *,
                   const igl::WindingNumberTree<Point> *>,
         double>
    igl::WindingNumberTree<Point>::cached;

template <typename Point>
inline igl::WindingNumberTree<Point>::WindingNumberTree()
    : method(EXACT_WINDING_NUMBER_METHOD), parent(NULL), V(igl::dummyV), SV(),
      F(),
      // boundary(igl::boundary_facets<Eigen::MatrixXi,Eigen::MatrixXi>(F))
      cap(), radius(std::numeric_limits<double>::infinity()), center(0, 0, 0) {}
...
template <typename Point>
inline void igl::WindingNumberTree<Point>::set_mesh(const Eigen::MatrixXd &_V,
...
template <typename Point>
inline igl::WindingNumberTree<Point>::WindingNumberTree(
    const igl::WindingNumberTree<Point> &parent, const Eigen::MatrixXi &_F)
    : method(parent.method), parent(&parent), V(parent.V), SV(), F(_F),
...
template <typename Point>
inline void
igl::WindingNumberTree<Point>::set_method(const WindingNumberMethod &m) {
  this->method = m;
...
template <typename Point> inline void igl::WindingNumberTree<Point>::grow() {
  // Don't grow
  return;
}
...
template <typename Point>
inline double
igl::WindingNumberTree<Point>::winding_number(const Point &p) const {
  using namespace std;
  // cout<<"+"<<boundary.rows();
  ...
  if (inside(p)) {
    ...
    if (children.size() > 0) {
      ...
      double sum = 0;
      for (typename list<WindingNumberTree<Point> *>::const_iterator cit =
               children.begin();
...
template <typename Point>
inline double
igl::WindingNumberTree<Point>::winding_number_all(const Point &p) const {
  double w = 0;
  igl::winding_number_3(V.data(), V.rows(), F.data(), F.rows(), p.data(), 1,
                        &w);
  return w;
}
...
template <typename Point>
inline double
igl::WindingNumberTree<Point>::winding_number_boundary(const Point &p) const {
  using namespace Eigen;
  using namespace std;

  double w = 0;
  // `cap` is already flipped inside out, so we don't need to flip sign of w
  igl::winding_number_3(V.data(), V.rows(), cap.data(), cap.rows(), &p[0], 1,
                        &w);
  return w;
}
...
inline void igl::WindingNumberTree<Point>::print(const char *tab) {
  ...
  // Print children
  for (typename list<WindingNumberTree<Point> *>::iterator cit =
           children.begin();
       cit != children.end(); cit++) {
    cout << "," << endl;
...
template <typename Point>
inline double igl::WindingNumberTree<Point>::max_abs_winding_number(
    const Point & /*p*/) const {
  return std::numeric_limits<double>::infinity();
}

template <typename Point>
inline double igl::WindingNumberTree<Point>::max_simple_abs_winding_number(
    const Point & /*p*/) const {
  using namespace std;
...
inline double igl::WindingNumberTree<Point>::cached_winding_number(
    const igl::WindingNumberTree<Point> &that, const Point &p) const {
  using namespace std;
  ...
  if (far) {
    double a =
        atan2(that.radius - this->radius, (that.center - this->center).norm());
    assert(a > 0);
    far = (a < PI / 8.0);
  }
  ...
  if (far) {
    // Not implemented yet
    pair<const WindingNumberTree *, const WindingNumberTree *> this_that(this,
                                                                         &that);
    // Need to compute it for first time?
    if (cached.count(this_that) == 0) {
      cached[this_that] = that.winding_number_boundary(this->center);
    }
