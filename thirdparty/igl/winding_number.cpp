...
IGL_INLINE void igl::winding_number(const Eigen::MatrixXd &V,
  ...
  switch (F.cols()) {
  case 2:
    return winding_number_2(V.data(), V.rows(), F.data(), F.rows(), O.data(),
                            O.rows(), W.data());
  case 3: {
...
IGL_INLINE void
igl::winding_number_3(const Scalar *V, const int n, const DerivedF *F,
                      const int m, const Scalar *O, const int no, Scalar *S) {
  // Initialize output
  // loop over origins
#pragma omp parallel for if (no > IGL_WINDING_NUMBER_OMP_MIN_VALUE)
  for (int o = 0; o < no; o++) {
...
#pragma omp parallel for if (m > IGL_WINDING_NUMBER_OMP_MIN_VALUE && no > 1)
  ...
  for (int f = 0; f < m; f++) {
    ...
    for (int o = 0; o < no; o++) {
      ...
      // printf("\n");
      // Compute determinant
      Scalar detf = v[0][0] * v[1][1] * v[2][2] + v[1][0] * v[2][1] * v[0][2] +
                    v[2][0] * v[0][1] * v[1][2] - v[2][0] * v[1][1] * v[0][2] -
                    v[1][0] * v[0][1] * v[2][2] - v[0][0] * v[2][1] * v[1][2];
      ...
      Scalar val = atan2(detf, vl[0] * vl[1] * vl[2] + dp[0] * vl[0] +
                                   dp[1] * vl[1] + dp[2] * vl[2]) /
                   (2. * igl::PI);
#pragma omp atomic
      S[o] += val;
...
IGL_INLINE void
igl::winding_number_2(const double *V, const int n, const DerivedF *F,
                      const int m, const double *O, const int no, double *S) {
  // Initialize output
  // loop over origins
#pragma omp parallel for if (no > IGL_WINDING_NUMBER_OMP_MIN_VALUE)
  for (int o = 0; o < no; o++) {
...
#pragma omp parallel for if (m > IGL_WINDING_NUMBER_OMP_MIN_VALUE && no > 1)
  ...
  for (int f = 0; f < m; f++) {
    ...
    for (int o = 0; o < no; o++) {
      ...
      double val = atan2(o2vd[0] * o2vs[1] - o2vd[1] * o2vs[0],
                         o2vd[0] * o2vs[0] + o2vd[1] * o2vs[1]) /
                   (2. * igl::PI);
#pragma omp atomic
      S[o] += val;
    }
...
template void igl::winding_number_2<double>(double const *, int, double const *,
                                            int, double const *, int, double *);
template void igl::winding_number_3<double>(double const *, int, double const *,
                                            int, double const *, int, double *);
template void igl::winding_number_3<double, int>(double const *, int,
                                                 int const *, int,
