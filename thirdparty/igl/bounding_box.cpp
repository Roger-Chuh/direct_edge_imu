...
IGL_INLINE void igl::bounding_box(const Eigen::PlainObjectBase<DerivedV> &V,
  ...
  const std::function<void(const int, const int, int *, int)> combos =
      [&BV, &minV, &maxV, &combos](const int dim, const int i, int *X,
                                   const int pre_index) {
        for (X[i] = 0; X[i] < 2; X[i]++) {
  ...
  case 3:
    BF.resize(12, 3);
    BF << 2, 0, 6, 0, 4, 6, 5, 4, 0, 5, 0, 1, 6, 4, 5, 5, 7, 6, 3, 0, 2, 1, 0,
        3, 3, 2, 6, 6, 7, 3, 5, 1, 3, 3, 7, 5;
    break;
  default:
...
template void igl::bounding_box<Eigen::Matrix<double, -1, 3, 0, -1, 3>,
